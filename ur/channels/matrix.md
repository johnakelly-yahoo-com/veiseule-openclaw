...
Multi-account سپورٹ: فی اکاؤنٹ credentials اور اختیاری `name` کے ساتھ `channels.matrix.accounts` استعمال کریں۔ مشترکہ پیٹرن کے لیے [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) دیکھیں۔

ہر اکاؤنٹ کسی بھی homeserver پر ایک علیحدہ Matrix صارف کے طور پر چلتا ہے۔ Per-account کنفیگریشن
اوپر کی سطح کے `channels.matrix` سیٹنگز سے inherit کرتی ہے اور کسی بھی آپشن کو override کر سکتی ہے
(DM پالیسی، گروپس، encryption وغیرہ)۔
...
- Env variables (`MATRIX_HOMESERVER`, `MATRIX_ACCESS_TOKEN`, وغیرہ) صرف **default** اکاؤنٹ پر لاگو ہوتے ہیں۔
- بنیادی چینل سیٹنگز (DM پالیسی، گروپ پالیسی، mention gating، وغیرہ) تمام اکاؤنٹس پر لاگو ہوتی ہیں جب تک کہ فی اکاؤنٹ override نہ کی جائیں۔
- ہر اکاؤنٹ کو مختلف agent سے منسلک کرنے کے لیے `bindings[].match.accountId` استعمال کریں۔
- Crypto state ہر اکاؤنٹ + access token کے مطابق محفوظ کی جاتی ہے (ہر اکاؤنٹ کے لیے علیحدہ key stores)۔
...
- display names یا صرف localparts استعمال نہ کریں (مثال: `"Alice"` یا `"alice"`)۔ یہ مبہم ہوتے ہیں اور allowlist matching کے لیے نظرانداز کر دیے جاتے ہیں۔ مکمل `@user:server` IDs استعمال کریں۔
