...
بڑی فائلیں ہر فائل کی بنیاد پر `agents.defaults.bootstrapMaxChars` (ڈیفالٹ `20000` حروف) استعمال کرتے ہوئے truncate کی جاتی ہیں۔ OpenClaw فائلوں میں مجموعی bootstrap انجیکشن کی حد کو بھی `agents.defaults.bootstrapTotalMaxChars` (ڈیفالٹ `24000` حروف) کے ذریعے نافذ کرتا ہے۔ `/context` **raw بمقابلہ injected** سائزز دکھاتا ہے اور یہ بھی کہ truncation ہوئی یا نہیں۔
