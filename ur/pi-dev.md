...
State، OpenClaw کی state ڈائریکٹری کے تحت محفوظ ہوتا ہے۔ ڈیفالٹ `~/.openclaw` ہے۔ اگر `OPENCLAW_STATE_DIR` سیٹ ہو تو اس کے بجائے وہی ڈائریکٹری استعمال کریں۔
...
اگر آپ صرف سیشنز ری سیٹ کرنا چاہتے ہیں تو اس ایجنٹ کے لیے `agents/<agentId>/sessions/` اور `agents/<agentId>/sessions.json` حذف کر دیں۔ اگر آپ دوبارہ توثیق نہیں کرنا چاہتے تو `credentials/` کو برقرار رکھیں۔
