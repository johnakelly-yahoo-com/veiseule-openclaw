...
| فراہم کنندہ                           | فوائد                                | نقصانات                                 | API کی کلید                                  |
| ------------------------------------- | ------------------------------------ | --------------------------------------- | -------------------------------------------- |
| **Brave** (ڈیفالٹ) | تیز، ساختہ نتائج، مفت درجۂ آغاز      | روایتی سرچ نتائج                        | `BRAVE_API_KEY`                              |
...
| ماڈل                                               | وضاحت                                       | بہترین استعمال |
| -------------------------------------------------- | ------------------------------------------- | -------------- |
| `perplexity/sonar`                                 | ویب سرچ کے ساتھ تیز Q&A | فوری تلاشیں    |
| `perplexity/sonar-pro` (ڈیفالٹ) | ویب سرچ کے ساتھ کثیر مرحلہ استدلال          | پیچیدہ سوالات  |
...
- `freshness` (اختیاری، صرف Brave): دریافت کے وقت کے مطابق فلٹر (`pd`، `pw`، `pm`، `py`، یا `YYYY-MM-DDtoYYYY-MM-DD`)
  - Brave: `pd`, `pw`, `pm`, `py`, یا `YYYY-MM-DDtoYYYY-MM-DD`
