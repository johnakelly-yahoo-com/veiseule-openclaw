...
Ollama is a local LLM runtime that makes it easy to run open-source models on your machine. OpenClaw، Ollama کے مقامی API (`/api/chat`) کے ساتھ انضمام کرتا ہے، اسٹریمنگ اور ٹول کالنگ کی سپورٹ فراہم کرتا ہے، اور جب آپ `OLLAMA_API_KEY` (یا کسی auth پروفائل) کے ساتھ آپٹ اِن کرتے ہیں اور `models.providers.ollama` کی واضح انٹری متعین نہیں کرتے تو یہ **ٹول کی صلاحیت رکھنے والے ماڈلز کو خودکار طور پر دریافت** کر سکتا ہے۔
...
OpenClaw کا Ollama انضمام بطور ڈیفالٹ **مقامی Ollama API** (`/api/chat`) استعمال کرتا ہے، جو بیک وقت اسٹریمنگ اور ٹول کالنگ کی مکمل سپورٹ فراہم کرتا ہے۔ کوئی خاص کنفیگریشن درکار نہیں ہے۔

#### لیگیسی OpenAI-مطابقت موڈ

اگر آپ کو اس کے بجائے OpenAI-مطابقت رکھنے والا اینڈ پوائنٹ استعمال کرنا ہو (مثلاً ایسے پراکسی کے پیچھے جو صرف OpenAI فارمیٹ کو سپورٹ کرتا ہو)، تو `api: "openai-completions"` کو واضح طور پر سیٹ کریں:
...
نوٹ: OpenAI-مطابقت رکھنے والا اینڈ پوائنٹ ممکن ہے کہ بیک وقت اسٹریمنگ + ٹول کالنگ کی سپورٹ نہ کرے۔ آپ کو ماڈل کنفیگ میں `params: { streaming: false }` کے ذریعے اسٹریمنگ غیر فعال کرنے کی ضرورت پڑ سکتی ہے۔
