...
- پول نوٹ:
  - OpenClaw تیز رفتار یونٹ شارڈز کے لیے Node 22/23 پر Vitest `vmForks` استعمال کرتا ہے۔
  - Node 24+ پر، OpenClaw خودکار طور پر باقاعدہ `forks` پر واپس آ جاتا ہے تاکہ Node VM لنکنگ کی خرابیوں (`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`) سے بچا جا سکے۔
  - دستی طور پر اووررائیڈ کریں `OPENCLAW_TEST_VM_FORKS=0` (`forks` پر مجبور کریں) یا `OPENCLAW_TEST_VM_FORKS=1` (`vmForks` پر مجبور کریں)۔
...
- دائرۂ کار:
  ...
  - کنسول I/O اوورہیڈ کم کرنے کے لیے ڈیفالٹ طور پر سائلنٹ موڈ میں چلتا ہے۔
- توقعات:
