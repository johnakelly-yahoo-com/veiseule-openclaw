...
```powershell
# install.ps1 میں ابھی تک مخصوص -Verbose فلیگ موجود نہیں ہے۔
Set-PSDebug -Trace 1
...
`channels.telegram.allowFrom` **انسانی بھیجنے والے کا Telegram صارف ID** ہے (عددی شکل میں)۔ 48. یہ bot username نہیں ہوتا۔

آن بورڈنگ وزارڈ `@username` ان پٹ قبول کرتا ہے اور اسے عددی ID میں تبدیل کرتا ہے، لیکن OpenClaw کی اجازت دہی صرف عددی IDs استعمال کرتی ہے۔
...
1. Copilot ٹوکنز `COPILOT_GITHUB_TOKEN` سے پڑھے جاتے ہیں (نیز `GH_TOKEN` / `GITHUB_TOKEN`)۔
