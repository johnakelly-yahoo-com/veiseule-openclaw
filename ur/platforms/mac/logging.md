...
- تفصیل کی سطح: **ڈیبگ پین → لاگز → ایپ لاگنگ → تفصیل کی سطح**
- فعال کریں: **ڈیبگ پین → لاگز → ایپ لاگنگ → "رولنگ تشخیصی لاگ (JSONL) لکھیں"**
- مقام: `~/Library/Logs/OpenClaw/diagnostics.jsonl` (خودکار طور پر روٹیٹ ہوتا ہے؛ پرانی فائلوں کے آخر میں `.1`، `.2`، … لگ جاتا ہے)
- صاف کریں: **ڈیبگ پین → لاگز → ایپ لاگنگ → "صاف کریں"**
