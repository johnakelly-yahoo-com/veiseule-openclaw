...
آپ اس deny فہرست کو `gateway.tools` کے ذریعے حسبِ ضرورت تبدیل کر سکتے ہیں:

```json5
{
  gateway: {
    tools: {
      // HTTP /tools/invoke کے ذریعے بلاک کرنے کے لیے اضافی ٹولز
      deny: ["browser"],
      // ڈیفالٹ deny فہرست سے ٹولز ہٹائیں
      allow: ["gateway"],
    },
  },
}
...
- `200` → `{ ok: true, result }`
- `400` → `{ ok: false, error: { type, message } }` (غلط درخواست یا ٹول اِن پٹ کی خرابی)
- `401` → غیر مجاز
- `429` → auth ریٹ لمٹڈ (`Retry-After` سیٹ ہے)
- `404` → ٹول دستیاب نہیں (نہ ملا یا اجازت فہرست میں شامل نہیں)
- `405` → طریقہ کار کی اجازت نہیں
- `500` → `{ ok: false, error: { type, message } }` (ٹول کے نفاذ کے دوران غیر متوقع خرابی؛ صاف شدہ پیغام)
