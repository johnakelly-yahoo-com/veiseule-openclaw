...
- اگر `gateway.auth.rateLimit` کنفیگر کیا گیا ہو اور بہت زیادہ auth ناکامیاں ہوں، تو اینڈپوائنٹ `429` کے ساتھ `Retry-After` واپس کرتا ہے۔
...
- درخواستیں محفوظ طریقے سے کنٹرول کی جاتی ہیں (DNS ریزولوشن، پرائیویٹ IP بلاکنگ، ری ڈائریکٹ حدود، ٹائم آؤٹس)۔
- ہر ان پٹ ٹائپ کے لیے اختیاری hostname allowlists کی معاونت دستیاب ہے (`files.urlAllowlist`, `images.urlAllowlist`)۔
  - عین host: `"cdn.example.com"`
  - وائلڈ کارڈ سب ڈومینز: `"*.assets.example.com"` (apex سے میچ نہیں کرتا)
...
- URL allowlists کو fetch سے پہلے اور redirect کے ہر مرحلے پر نافذ کیا جاتا ہے۔
- کسی hostname کو allowlist کرنے سے نجی/اندرونی IP بلاکنگ کو بائی پاس نہیں کیا جا سکتا۔
- انٹرنیٹ پر ایکسپوزڈ gateways کے لیے، ایپ لیول حفاظتی اقدامات کے ساتھ نیٹ ورک egress کنٹرولز بھی نافذ کریں۔
  [Security](/gateway/security) دیکھیں۔
