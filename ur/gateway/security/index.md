...
- Canvas host: `/__openclaw__/canvas/` اور `/__openclaw__/a2ui/` (من مانی HTML/JS؛ اسے غیر معتبر مواد سمجھیں)
...
- نیا راز بنائیں/سیٹ کریں (`gateway.auth.token` یا `OPENCLAW_GATEWAY_PASSWORD`)۔
- Gateway ری اسٹارٹ کریں (یا macOS ایپ ری اسٹارٹ کریں اگر وہ Gateway کی نگرانی کرتی ہو)۔
...
- `gateway.auth.mode: "trusted-proxy"`: صارفین کی توثیق کے لیے ایک identity-aware reverse proxy پر اعتماد کریں اور شناخت کو ہیڈرز کے ذریعے منتقل کریں (دیکھیں [Trusted Proxy Auth](/gateway/trusted-proxy-auth))۔
...
**Security rule:** do not forward these headers from your own reverse proxy. اگر
آپ gateway کے سامنے TLS ختم کرتے ہیں یا proxy استعمال کرتے ہیں، تو
`gateway.auth.allowTailscale` کو غیر فعال کریں اور اس کے بجائے token/password auth (یا [Trusted Proxy Auth](/gateway/trusted-proxy-auth)) استعمال کریں۔
...
اضافی سخت حفاظتی اختیارات:

- `tools.exec.applyPatch.workspaceOnly: true` (ڈیفالٹ): یقینی بناتا ہے کہ `apply_patch` sandbox بند ہونے کی صورت میں بھی workspace ڈائریکٹری سے باہر لکھ یا حذف نہیں کر سکتا۔ صرف اسی صورت میں `false` پر سیٹ کریں جب آپ جان بوجھ کر چاہتے ہوں کہ `apply_patch` workspace سے باہر فائلوں میں تبدیلی کرے۔
- `tools.fs.workspaceOnly: true` (اختیاری): `read`/`write`/`edit`/`apply_patch` کے راستوں کو workspace ڈائریکٹری تک محدود کرتا ہے (مفید اگر آپ آج absolute paths کی اجازت دیتے ہیں اور ایک واحد حفاظتی حد چاہتے ہیں)۔
...
1. ٹائم اسٹیمپ، gateway ہوسٹ OS + OpenClaw ورژن
2. سیشن ٹرانسکرپٹ(س) + مختصر لاگ ٹیل (ریڈیکشن کے بعد)
3. حملہ آور نے کیا بھیجا + ایجنٹ نے کیا کیا
...
```mermaid
flowchart TB
    A["Owner (Peter)"] -- مکمل اعتماد --> B["AI (Clawd)"]
    B -- اعتماد لیکن تصدیق کریں --> C["Friends in allowlist"]
    C -- محدود اعتماد --> D["Strangers"]
    D -- کوئی اعتماد نہیں --> E["Mario asking for find ~"]
    E -- بالکل بھی اعتماد نہیں 😏 --> F[" "]

     %% سب سے نیچے والے لیبل کو درست دکھانے کے لیے شفاف باکس ضروری ہے
