...
- « Gateway start blocked: set gateway.mode=local » Correction : définissez `gateway.mode="local"` dans votre configuration (ou exécutez `openclaw configure`). Si vous exécutez OpenClaw via Podman en utilisant l’utilisateur dédié `openclaw`, la configuration se trouve dans `~openclaw/.openclaw/openclaw.json`.
