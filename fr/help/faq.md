...
Réponses rapides ainsi qu’un dépannage approfondi pour des configurations réelles (développement local, VPS, multi-agent, OAuth/clé API, bascule de modèle). Pour les diagnostics d’exécution, consultez [Troubleshooting](/gateway/troubleshooting). Pour la référence complète de configuration, consultez [Configuration](/gateway/configuration).
...
1. **Rappels et suivi :** coups de cron ou coups de coeur et listes de contrôle.
2. Gardez un contexte important dans l'espace de travail et demandez au bot de le lire.
...
Commencez une nouvelle session pour que l’instantané des compétences se rafraîchisse.
...
L’assistant d’onboarding accepte une entrée `@username` et la résout en identifiant numérique, mais l’autorisation OpenClaw utilise uniquement des identifiants numériques.
...
**Rotation du profil d'authentification** dans le même fournisseur.
...
1. Sélection des **examens de santé** et des **compétences**
...
3. Pas intégré aujourd'hui.

   ```markdown
   Que fait l'assistant d'intégration
   ```

4. **Fournisseurs** (WhatsApp, Telegram, Discord, Mattermost (plugin), Signal, iMessage)
...
Métadonnées de session (par agent)
...
## # Par défaut, l'agent par défaut configuré (omit --agent)l'ordre d'authentification des modèles openclaw get --provider anthropic# Verrouiller la rotation sur un seul profil (n'essayez que celui-ci)openclaw modèles auth ordre --provider anthropic:default# Ou définir un ordre explicite (repli dans le provider)modèle openclaw auth protocole --provider anthropic:work anthropic:work anthropic:default# Forcer la substitution (revenir à config auth. rder / rond-robin)modèles d'authentification openclaw clair --provider anthropique
...
| Activer la messagerie inter-fournisseur pour l'agent: | Exécutez `openclaw agents add &lt;id&gt;` et configurez l'authentification pendant l'assistant.                                             |
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `$OPENCLAW_STATE_DIR/openclaw.json`                                   | **Approuver le nœud** sur la passerelle :                                                                                             |
| Faire des threads de groupe partager le contexte avec des DMs         | Options :                                                                                                                             |
| `$OPENCLAW_STATE_DIR/agents/&lt;agentId&gt;/agent/auth-profiles.json`       | Installez une deuxième passerelle uniquement lorsque vous avez besoin de **l'isolation dure** ou de deux robots complètement séparés. |
| `$OPENCLAW_STATE_DIR/agents/&lt;agentId&gt;/agent/auth.json`                | **Recherche et rédaction :** recherche rapide, résumés et premiers brouillons pour les e-mails ou les documentations. |
| `$OPENCLAW_STATE_DIR/credentials/`                                    | L'application macOS surveille le fichier de configuration et bascule les modes en temps réel lorsque ces valeurs changent.            |
| `$OPENCLAW_STATE_DIR/agents/`                                         | **Q : "Quel est le modèle par défaut pour Anthropic avec une clé API ?"**                                                             |
| `$OPENCLAW_STATE_DIR/agents/&lt;agentId&gt;/sessions/`                      | Si vous utilisez allowlists, ajoutez `web_search`/`web_fetch` ou `group:web`.                                                         |
| `$OPENCLAW_STATE_DIR/agents/&lt;agentId&gt;/sessions/sessions.json`         | Ne pas ouvrir le port WS dans un onglet normal.                                                                                       |
...
- Historique et état de la conversation (par agent)
...
1. **La demande de LLM a rejeté la signature de pensée de message nécessitant l'antigravité de Google**
2. Pourquoi est-ce que je vois la requête LLM rejetée messages NcontentXtooluseinput Champ requis
3. Aucun fichier `.env` ne remplace des variables d’environnement existantes.
4. Quels sont les identifiants de profil typiques
5. `opus` → `anthropique/claude-opus-4-6`
...
- \* Gardez la passerelle sur un VPS, mais exécutez Chrome localement et contrôlez le relais
...
1. Docs : [routage multi-agents](/concepts/multi-agent), [Sub-agents](/tools/subagents), [Agents CLI](/cli/agents).
...
2. Si vous utilisez le CLI ou TUI, l'URL devrait ressembler à :
   - Configuration rapide (recommandée) :

3. Si vous n'avez pas encore de jeton : `docteur openclaw --generate-gateway-token`.
   - `followup` - lance un message à la fois

4. Option 1 (plus rapide) : logs de queue et envoi d'un message de test dans le groupe :
...
1. action de traitement:kill sessionId:XXX
...
3. Un agent de codage local peut souvent reconstruire une configuration fonctionnelle à partir des logs ou de l'historique.
...
1. Quel port la passerelle utilise-t-elle
2. Aucun pont TCP séparé n'est requis; les nœuds se connectent à travers la passerelle WebSocket.
...
1. Quelle est la stratégie de sauvegarde recommandée

   ```
   COPILOT_GITHUB_TOKEN=...
   ```

2. Quel est le format de la configuration Où est il

3. OpenClaw prend en charge les deux:
...
### Mise à jour vers **2026.1.12** (ou depuis la source `main`), puis redémarrez la passerelle.
...
**Assurez-vous que le VPS + Mac est sur la même queue**.
...
Si la télécommande, est-ce que le tunnel/échelle de queue est en haut?
...
Import OAuth hérité (copié dans les profils d'authentification lors de la première utilisation)
...
- Configuration principale (JSON5)

### **Bascule de modele** vers le modele suivant dans `agents.defaults.model.fallbacks`.
...
1. Profils d'authentification : ce qu'ils sont et comment les gérer
2. `steer` - les nouveaux messages redirigent la tâche actuelle
3. Telegram → Passerelle → Agent → `node.*` → Noeud → Passerelle → Telegram
4. Run:
...
1. clawhub installe <skill-slug>
   clawhub update --all
2. Y a-t-il un avantage à utiliser un nœud sur mon ordinateur portable personnel au lieu de SSH d'un VPS
...
```json5
Etat par agent (agentDir + sessions)
```

openclaw cron exécute <jobId> --force
openclaw cron exécute --id <jobId> --limit 50

- Profils d'authentification (OAuth + Clés API)
- Cherchez `chatId` (ou `from`) se terminant par `@g.us`, comme:
  `1234567890-1234567890@g.us`.

### Mettez le wrapper sur `PATH` sur l'hôte Linux (par exemple `~/bin/memo`).
...
Ouvrez l'application macOS localement et connectez-vous en mode **Remote via SSH** (ou direct tailnet)
...
Si l'authentification est activée, incluez le jeton/mot de passe dans le cadre `connect`.
...
1. **Tâches Cron**: les tâches isolées peuvent définir une substitution de `modèle` par tâche.
2. Relatif : [/concepts/oauth](/concepts/oauth) (OAuth flows, token storage, multi-account patterns)
3. Modèle d'exemple (exécuté à partir d'une machine qui peut atteindre la passerelle cible):
...
Etat du fournisseur (par exemple `whatsapp/<accountId>/creds.json`)

**Confirmer où vivent les profils d'authentification** (nouveaux vs chemins existants)

```powershell
Utilisez l'id exact du modèle (sensible à la casse) : `minimax/MiniMax-M2.1` ou
...
```powershell
Assurez-vous que MiniMax est configuré (assistant ou JSON), ou qu'une clé API MiniMax
...
- Remplacer les métadonnées de la compétence (espace de travail ou `~/.openclaw/skills`) pour autoriser Linux:
...
4. **State dir (`~/.openclaw`)**: config, credentials, auth profiles, sessions, logs,
...
### Cache d'authentification d'exécution (géré automatiquement)
...
- Restaurer à partir d'une sauvegarde (git ou un `~/.openclaw/openclaw.json` copié).
  ...
  - Activer l'importation shell (opt-in convenience) :
...
### Ou ajoutez-le à votre bloc `env` de configuration (ne s'applique qu'en cas de manquance).
...
Ou activez l'importation du shell (`env.shellEnv.enabled: true`).
...
Créer un wrapper SSH pour le binaire (exemple : `memo` pour les notes d'Apple ) :
...
Utilisez l'URL WS: `ws://<host>:18789` (ou `wss://...` si HTTPS).
...
`$OPENCLAW_STATE_DIR/identifiants/oauth.json`
...
Activer le plugin: `openclaw plugins enable google-gemini-cli-auth`

### Mettez le jeton dans `~/.openclaw/.env`:
...
Mettez les clés manquantes dans `~/.openclaw/.env` afin qu'elles soient ramassées même si le service n'hérite pas votre shell env.
