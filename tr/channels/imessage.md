...
flowchart TB
 ...
 subgraph Tailscale[" "]
    direction LR
      ...
      Mac["<b>Messages + imsg bulunan Mac<br></b><br>Messages oturum açılmış<br>Remote Login etkin"]
  end
    Gateway -- SSH (imsg rpc) --> Mac
    Mac -- SCP (ekler) --> Gateway
    direction BT
    User["user@gateway-host"] -- "Tailscale tailnet (hostname veya 100.x.y.z)" --> Gateway
end
```
