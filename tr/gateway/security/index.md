...
- **Politika sapması/yanlış yapılandırma** (sandbox docker ayarları yapılandırılmış ancak sandbox modu kapalı; etkisiz `gateway.nodes.denyCommands` desenleri; genel `tools.profile="minimal"` ayarının ajan bazlı profiller tarafından geçersiz kılınması; geniş izinli araç politikası altında extension eklenti araçlarına erişilebilir olması).
...
- OpenResponses URL girdileri (`input_file` / `input_image`) için sıkı `gateway.http.endpoints.responses.files.urlAllowlist` ve `gateway.http.endpoints.responses.images.urlAllowlist` ayarlayın ve `maxUrlParts` değerini düşük tutun.
...
Canvas içeriğini normal bir tarayıcıda yüklüyorsanız, diğer tüm güvenilmeyen web sayfaları gibi değerlendirin:

- Canvas host’u güvenilmeyen ağlara/kullanıcılara açmayın.
- Etkilerini tamamen anlamadan, canvas içeriğinin ayrıcalıklı web yüzeyleriyle aynı origin’i paylaşmasına izin vermeyin.
...
- `gateway.auth.mode: "trusted-proxy"`: kullanıcıları kimlik doğrulamak ve kimliği header’lar aracılığıyla iletmek için kimlik farkındalıklı bir reverse proxy’ye güvenin (bkz. [Trusted Proxy Auth](/gateway/trusted-proxy-auth)).
...
- `tools.exec.applyPatch.workspaceOnly: true` (varsayılan): sandbox kapalı olsa bile `apply_patch` işleminin çalışma alanı dizini dışına yazmasını/silmesini engeller. `apply_patch`’in çalışma alanı dışındaki dosyalara dokunmasını bilinçli olarak istiyorsanız yalnızca `false` olarak ayarlayın.
- `tools.fs.workspaceOnly: true` (isteğe bağlı): `read`/`write`/`edit`/`apply_patch` yollarını çalışma alanı diziniyle sınırlar (bugün mutlak yolları izinli tutuyor ve tek bir güvenlik bariyeri istiyorsanız kullanışlıdır).
