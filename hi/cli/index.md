...
- `--custom-base-url <url>` (non-interactive; `--auth-choice custom-api-key` के साथ उपयोग किया जाता है)
- `--custom-model-id <id>` (non-interactive; `--auth-choice custom-api-key` के साथ उपयोग किया जाता है)
- `--custom-api-key <key>` (non-interactive; वैकल्पिक; `--auth-choice custom-api-key` के साथ उपयोग किया जाता है; न देने पर `CUSTOM_API_KEY` पर fallback करता है)
- `--custom-provider-id <id>` (non-interactive; वैकल्पिक custom provider id)
- `--custom-compatibility <openai|anthropic>` (non-interactive; वैकल्पिक; डिफ़ॉल्ट `openai`)
- `--gateway-port <port>`
