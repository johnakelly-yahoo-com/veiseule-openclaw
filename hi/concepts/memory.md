...
- खोजें `memory.qmd.searchMode` (डिफ़ॉल्ट `qmd search --json`; साथ ही
  `vsearch` और `query` समर्थित) के माध्यम से चलाई जाती हैं। यदि चयनित मोड आपके
  QMD बिल्ड पर फ़्लैग स्वीकार नहीं करता, तो OpenClaw `qmd query` के साथ पुनः प्रयास करता है। यदि QMD विफल हो जाए या उसका बाइनरी
  अनुपलब्ध हो, तो OpenClaw स्वतः ही अंतर्निहित SQLite मैनेजर पर वापस आ जाता है ताकि
  memory टूल्स काम करते रहें।
...
- **पहली खोज धीमी हो सकती है**: QMD पहली `qmd query` रन पर लोकल GGUF मॉडल (री-रैंकर/क्वेरी
  ...
  - यदि आप मॉडल्स को मैन्युअली प्री-डाउनलोड करना चाहते हैं (और वही इंडेक्स वार्म करना चाहते हैं जिसे OpenClaw
    ...
    ```bash
    # वही state dir चुनें जिसका उपयोग OpenClaw करता है
    STATE_DIR="${OPENCLAW_STATE_DIR:-$HOME/.openclaw}"

    export XDG_CONFIG_HOME="$STATE_DIR/agents/main/qmd/xdg-config"
    export XDG_CACHE_HOME="$STATE_DIR/agents/main/qmd/xdg-cache"

    # (वैकल्पिक) इंडेक्स रीफ़्रेश + embeddings को मजबूर करें
    qmd update
    qmd embed

    # वार्म अप करें / पहली बार मॉडल डाउनलोड ट्रिगर करें
...
- `scope`: [`session.sendPolicy`](/gateway/configuration#session) जैसा ही स्कीमा।
  डिफ़ॉल्ट DM-only है (`deny` सभी, `allow` डायरेक्ट चैट्स); इसे ढीला करें ताकि समूहों/चैनलों में QMD हिट्स दिखाई दें।
  - `match.keyPrefix` **normalized** session key (लोअरकेस, और किसी भी
    प्रारंभिक `agent:<id>:` को हटाकर) से मेल खाता है। उदाहरण: `discord:channel:`।
  - `match.rawKeyPrefix` **raw** session key (लोअरकेस) से मेल खाता है, जिसमें
    `agent:<id>:` शामिल होता है। उदाहरण: `agent:main:discord:`।
  - Legacy: `match.keyPrefix: "agent:..."` को अभी भी raw-key prefix के रूप में माना जाता है,
    लेकिन स्पष्टता के लिए `rawKeyPrefix` का उपयोग करना बेहतर है।
...
- डिफ़ॉल्ट रूप से अक्षम। बड़े-कॉर्पस इंडेक्सिंग (OpenAI, Gemini, और Voyage) के लिए सक्षम करने हेतु `agents.defaults.memorySearch.remote.batch.enabled = true` सेट करें।
