...
- Typenspezifische Überschreibungen (optional): `resetByType` ermöglicht es Ihnen, die Richtlinie für `direct`-, `group`- und `thread`-Sitzungen zu überschreiben (thread = Slack/Discord-Threads, Telegram-Themen, Matrix-Threads, sofern vom Connector bereitgestellt werden).
