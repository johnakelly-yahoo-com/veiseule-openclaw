...
Echte Projekte aus der Community. Sehen Sie, was Menschen mit OpenClaw bauen.

<Info>
**Möchten Sie vorgestellt werden?** Teilen Sie Ihr Projekt in [#showcase auf Discord](https://discord.gg/clawd) oder [markieren Sie @openclaw auf X](https://x.com/openclaw).
</Info>
...
OpenCode schließt die Änderung ab → öffnet einen PR → OpenClaw prüft den Diff und antwortet in Telegram mit „kleinen Vorschlägen“ sowie einem klaren Merge‑Urteil (einschließlich kritischer Fixes, die zuerst anzuwenden sind).
...
„Robby“ (@openclaw) nach einem lokalen Weinkeller‑Skill gefragt. Er fordert einen Beispiel‑CSV‑Export sowie den Speicherort an und baut/testet den Skill dann schnell (962 Flaschen im Beispiel).
...
Wöchentlicher Essensplan → Stammartikel → Lieferzeitfenster buchen → Bestellung bestätigen. Keine APIs, nur Browser‑Steuerung.
...
Tastenkürzel für einen Bildschirmbereich → Gemini Vision → sofortiges Markdown in der Zwischenablage.
...
Kapselt papla.media‑TTS und sendet Ergebnisse als Telegram‑Sprachnachrichten (kein lästiges Autoplay).
...
Per Homebrew installierter Helfer zum Auflisten/Inspektieren/Beobachten lokaler OpenAI‑Codex‑Sitzungen (CLI + VS Code).
...
Echtzeit‑Abfahrten, Störungen, Aufzugsstatus und Routenplanung für den Wiener öffentlichen Verkehr.
...
Komplette iOS‑App mit Karten und Sprachaufzeichnung gebaut und vollständig per Telegram‑Chat zu TestFlight bereitgestellt.
...
Claude Code entdeckte und bestätigte die Steuerungen des Luftreinigers; anschließend übernimmt OpenClaw die Verwaltung der Raumluftqualität.
...
Ausgelöst durch eine Dachkamera: OpenClaw wird gebeten, jedes Mal ein Himmelsfoto aufzunehmen, wenn es hübsch aussieht — er entwarf einen Skill und machte die Aufnahme.
