...
För enklare daglig Docker-hantering, installera `ClawDock`:

```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/openclaw/openclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```

**Lägg till i din shell-konfiguration (zsh):**

```bash
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Använd sedan `clawdock-start`, `clawdock-stop`, `clawdock-dashboard` osv. Kör `clawdock-help` för alla kommandon.

Se [`ClawDock` Helper README](https://github.com/openclaw/openclaw/blob/main/scripts/shell-helpers/README.md) för mer information.
...
1. **Persist Playwright-webbläsarnedladdningar**:
