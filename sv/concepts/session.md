...
- Åsidosättningar per typ (valfritt): `resetByType` låter dig åsidosätta policyn för `direct`-, `group`- och `thread`-sessioner (thread = Slack/Discord-trådar, Telegram-ämnen, Matrix-trådar när de tillhandahålls av anslutningen).
