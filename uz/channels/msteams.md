...
1. OpenClaw’ni sozlashdan oldin Azure Bot resursini yaratishingiz kerak.
...
1. **Variant B: Teams (ilova o‘rnatilgandan keyin)**
...
1. Kanal/guruh **rasm yoki fayl tarkibi** (payload faqat HTML stubni o‘z ichiga oladi).
2. SharePoint/OneDrive’da saqlangan biriktirmalarni yuklab olish.
...
- Entra ID (Azure AD) **App Registration** ichida Microsoft Graph **Application permissions** qo‘shing:
- SharePoint/OneDrive’da saqlangan fayl biriktirmalarini yuklab olish.
- Teams ilovasi **manifest versiyasini** oshiring, qayta yuklang va **ilovani Teams’da qayta o‘rnating**.
...
**Foydalanuvchi mentionlari uchun qo‘shimcha ruxsat:** Foydalanuvchi @mentionlari suhbatdagi foydalanuvchilar uchun avtomatik ishlaydi. Biroq, agar joriy suhbatda **mavjud bo‘lmagan** foydalanuvchilarni dinamik qidirib mention qilmoqchi bo‘lsangiz, `User.Read.All` (Application) ruxsatini qo‘shing va admin roziligini bering.
...
- `channels.msteams.teams.<teamId>``channels.msteams.teams.<teamId>
...
- `channels.msteams.teams.<teamId>
  .replyStyle`: har bir jamoa uchun alohida sozlama..channels.<conversationId>.replyStyle\`: per-channel override.
...
1. FileConsentCard oqimi (SharePoint’siz ham ishlaydi)
...
| ----------------------------------------------- | ----------------- | --------------------------------------- |
| Botni o‘rnatish                                 | Yes               | Cheklangan                              |
| Real-time messages (webhook) | Yes               | Ishlamasligi mumkin                     |
| RSC ruxsatlari                                  | Yes               | Boshqacha ishlashi mumkin               |
| @mentionlar                        | Yes               | Agar botga kirish mumkin bo‘lsa         |
| Graph API tarixi                                | Yes               | Ha (ruxsatlar bilan) |
