...
- Cross-platform: Unix socket + token auth (pagkakatugma sa macOS/Linux/Windows).
...
- **Estado ng Node host:** `~/.openclaw/node.json` (node id + pairing token).
