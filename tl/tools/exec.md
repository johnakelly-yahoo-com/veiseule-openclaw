...
- `tools.exec.host` (paunang halaga: `sandbox`)
- `tools.exec.security` (default: `deny` para sa sandbox, `allowlist` para sa gateway + node kapag unset)
- `tools.exec.ask` (paunang halaga: `on-miss`)
- `tools.exec.node` (paunang halaga: hindi nakatakda)
