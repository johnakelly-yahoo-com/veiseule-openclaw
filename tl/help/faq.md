...
```powershell
# Wala pang nakalaang -Verbose flag ang install.ps1.
Set-PSDebug -Trace 1
...
Ang `channels.telegram.allowFrom` ay **Telegram user ID ng taong nagpadala** (numeric). It is not the bot username.
...
- Nag-e-edit ka ng isang config file habang ibang config ang ginagamit ng tumatakbong service (madalas na `--profile` / `OPENCLAW_STATE_DIR` mismatch).
...
- Mga log ng service/supervisor (kapag tumatakbo ang gateway sa pamamagitan ng launchd/systemd):
