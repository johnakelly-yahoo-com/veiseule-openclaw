...
4. Tinutulay ng `subscribeEmbeddedPiSession` ang mga pi-agent-core event papunta sa OpenClaw `agent` stream:
   ...
   - mga delta ng assistant => `stream: "assistant"`
...
### Mga internal na hook (Gateway hooks)
...
### Mga hook ng plugin (agent + gateway lifecycle)
...
## Pag-compact + Mga Muling Pagsubok
