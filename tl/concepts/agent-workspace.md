...
- Paunang setting: `~/.openclaw/workspace`
...
- `~/.openclaw/openclaw.json` (configurasyon)
- `~/.openclaw/credentials/` (mga OAuth token, mga API key)
- `~/.openclaw/agents/<agentId>/sessions/` (mga transcript ng session + metadata)
- `~/.openclaw/skills/` (mga pinamamahalaang skill)
