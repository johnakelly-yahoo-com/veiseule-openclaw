...
2. Совпадение `parentPeer` (наследование треда)
3. `guildId + roles` (маршрутизация ролей Discord)
4. `guildId` (Discord)
5. `teamId` (Slack)
6. Совпадение `accountId` для канала
7. Совпадение на уровне канала (`accountId: "*"`)
8. Откат к агенту по умолчанию (`agents.list[].default`, иначе первый элемент списка, по умолчанию: `main`)

Если привязка задает несколько полей соответствия (например, `peer` + `guildId`), требуются все указанные поля (логика `AND`).
