...
- Настройте поиск памяти в разделе `agents.defaults.memorySearch` (а не в корневом
  `memorySearch`).
...
- Коллекции создаются через `qmd collection add` из `memory.qmd.paths`
  ...
  по умолчанию 5 м).
- Gateway теперь инициализирует менеджер QMD при запуске, поэтому таймеры периодического обновления
  активируются ещё до первого вызова `memory_search`.
...
- Поиск выполняется через `qmd query --json`. Если выбранный режим отклоняет флаги в вашей
  сборке QMD, OpenClaw повторяет попытку с `qmd query`. Если QMD падает или бинарник отсутствует,
...
- `scope`: та же схема, что и [`session.sendPolicy`](/gateway/configuration#session).
  ...
  - `match.keyPrefix` сопоставляется с **нормализованным** ключом сессии (в нижнем регистре, с удалённым
    ведущим `agent:<id>:`). Пример: `discord:channel:`.
  - `match.rawKeyPrefix` сопоставляется с **сырым** ключом сессии (в нижнем регистре), включая
    `agent:<id>:`. Пример: `agent:main:discord:`.
  - Устаревшее: `match.keyPrefix: "agent:..."` по-прежнему рассматривается как префикс сырого ключа,
    однако для ясности предпочтительнее использовать `rawKeyPrefix`.
