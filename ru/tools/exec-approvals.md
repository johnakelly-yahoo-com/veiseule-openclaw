...
Safe bins также принудительно обрабатывают токены argv как **буквальный текст** во время выполнения (без globbing
и без расширения `$VARS`) для сегментов только со stdin, поэтому шаблоны вроде `*` или `$HOME/...` не могут
использоваться для скрытого чтения файлов.
