...
- Если настроен `gateway.auth.rateLimit` и происходит слишком много неудачных попыток аутентификации, endpoint возвращает `429` с `Retry-After`.
...
Gateway HTTP также по умолчанию применяет жёсткий deny list (даже если политика сессии разрешает инструмент):
...
Вы можете настроить этот deny list через `gateway.tools`:
...
```json5
{
  gateway: {
    tools: {
      ...
      deny: ["browser"],
      // Удалить инструменты из deny list по умолчанию
      allow: ["gateway"],
    },
  },
}
...
- `401` → не авторизован
- `429` → превышен лимит попыток аутентификации (`Retry-After` установлен)
