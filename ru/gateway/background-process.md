...
- `tools.exec.notifyOnExitEmptySuccess` (по умолчанию false): если true, также ставит в очередь события завершения для успешно выполненных фоновых задач без вывода.
...
- Если оба параметра `offset` и `limit` опущены, возвращаются последние 200 строк и добавляется подсказка о пагинации.
- Если указан `offset`, а `limit` опущен, возвращается вывод от `offset` до конца (без ограничения в 200 строк).
