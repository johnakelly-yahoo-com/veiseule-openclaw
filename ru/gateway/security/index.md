...
- Для URL‑входов OpenResponses (`input_file` / `input_image`) установите строгие значения
  `gateway.http.endpoints.responses.files.urlAllowlist` и
  `gateway.http.endpoints.responses.images.urlAllowlist`, а также держите `maxUrlParts` низким.
...
- Control UI (ресурсы SPA) (базовый путь по умолчанию `/`)
- Хост canvas: `/__openclaw__/canvas/` и `/__openclaw__/a2ui/` (произвольный HTML/JS; рассматривайте как недоверенный контент)

Если вы загружаете контент canvas в обычном браузере, относитесь к нему как к любой другой недоверенной веб‑странице:

- Не открывайте доступ к хосту canvas для недоверенных сетей/пользователей.
- Не размещайте контент canvas в том же origin, что и привилегированные веб‑поверхности, если вы полностью не понимаете возможные последствия.
...
- `gateway.auth.mode: "trusted-proxy"`: доверять identity-aware reverse proxy для аутентификации пользователей и передачи их идентификации через заголовки (см. [Trusted Proxy Auth](/gateway/trusted-proxy-auth)).
...
Дополнительные параметры усиления безопасности:

- `tools.exec.applyPatch.workspaceOnly: true` (по умолчанию): гарантирует, что `apply_patch` не может записывать/удалять файлы вне каталога workspace, даже если изоляция отключена. Устанавливайте `false` только если вы намеренно хотите, чтобы `apply_patch` работал с файлами вне workspace.
- `tools.fs.workspaceOnly: true` (необязательно): ограничивает пути `read`/`write`/`edit`/`apply_patch` каталогом workspace (полезно, если вы сейчас разрешаете абсолютные пути и хотите добавить единое ограничение).
...
```mermaid
flowchart TB
    A["Владелец (Peter)"] -- Полное доверие --> B["AI (Clawd)"]
    B -- Доверяй, но проверяй --> C["Друзья в allowlist"]
    C -- Ограниченное доверие --> D["Незнакомцы"]
    D -- Нет доверия --> E["Mario просит выполнить find ~"]
    E -- Точно никакого доверия 😏 --> F[" "]

     %% Прозрачный блок нужен, чтобы корректно отобразить самую нижнюю метку
     F:::Class_transparent_box
    classDef Class_transparent_box fill:transparent, stroke:transparent
```
