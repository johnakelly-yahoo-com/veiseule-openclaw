...
- [Основы конфига](#config-basics)
  - [В каком формате конфигурация? Где она находится?](#what-format-is-the-config-where-is-it)
...
- [Логи и отладка](#logging-and-debugging)
  ...
  - [Объяснение для новичков: `openclaw gateway restart` и `openclaw gateway`](#eli5-openclaw-gateway-restart-vs-openclaw-gateway)
...
3. **Состояние демона и портов**
   ...
   Показывает состояние supervisor vs доступность RPC, целевой URL probe и какой конфиг, вероятно, использовал сервис.
...
4. **Глубокие проверки**
   ...
   Запускает проверки здоровья gateway + проверки провайдеров (требуется доступный gateway). См. [Health](/gateway/health).
...
Используйте локальный AI‑агент, который может **видеть вашу машину**. Это гораздо эффективнее, чем просить помощи в Discord, потому что большинство случаев «я застрял» — это **проблемы локальной конфигурации или окружения**, которые удалённые помощники не могут проверить.

- **Claude Code**: https://www.anthropic.com/claude-code/
- **OpenAI Codex**: https://openai.com/codex/

Эти инструменты могут читать репозиторий, выполнять команды, просматривать логи и помогать исправлять настройку на уровне машины (PATH, сервисы, права, файлы аутентификации). Дайте им **полный исходный код** через hackable (git) установку:

```bash
curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method git
```

Это устанавливает OpenClaw **из git‑чекаута**, чтобы агент мог читать код и документацию и понимать точную версию, которую вы используете. Вы всегда можете вернуться к стабильной версии, повторно запустив установщик без `--install-method git`.

Совет: попросите агента **составить план и контролировать** исправление (шаг за шагом), а затем выполнить только необходимые команды. Так изменения будут минимальными и их проще проверить.

Если вы нашли реальный баг или исправление, пожалуйста, создайте issue или PR:
https://github.com/openclaw/openclaw/issues  
https://github.com/openclaw/openclaw/pulls

Начните с этих команд (делитесь выводом при запросе помощи):
...
- `openclaw status`: быстрый снимок состояния gateway/агента + базовая конфигурация.
- `openclaw models status`: проверяет авторизацию провайдеров + доступность моделей.
...
Быстрый цикл отладки: [Первые 60 секунд, если что‑то сломалось](#first-60-seconds-if-somethings-broken).  
...
Репозиторий рекомендует запуск из исходного кода и использование мастера онбординга:
...
Мастер также может автоматически собрать UI‑ресурсы. После онбординга Gateway обычно запускается на порту **18789**.

Из исходников (для контрибьюторов/dev):
...
Если у вас ещё нет глобальной установки, запустите через `pnpm openclaw onboard`.

### Как открыть панель управления после онбординга?

Мастер открывает браузер с чистым (без токена в URL) адресом панели сразу после онбординга и также печатает ссылку в сводке. Оставьте эту вкладку открытой; если она не запустилась, скопируйте напечатанный URL на той же машине.

### Как аутентифицировать панель управления (токен) на localhost и удалённо?

**Localhost (та же машина):**

- Откройте `http://127.0.0.1:18789/`.
- Если запрашивается авторизация, вставьте токен из `gateway.auth.token` (или `OPENCLAW_GATEWAY_TOKEN`) в настройках Control UI.
- Получить токен на хосте gateway: `openclaw config get gateway.auth.token` (или сгенерировать: `openclaw doctor --generate-gateway-token`).

**Не на localhost:**

- **Tailscale Serve (рекомендуется):** оставьте bind loopback, запустите `openclaw gateway --tailscale serve`, откройте `https://<magicdns>/`. Если `gateway.auth.allowTailscale` равно `true`, заголовки идентичности удовлетворяют auth (токен не нужен).
- **Tailnet bind:** запустите `openclaw gateway --bind tailnet --token "<token>"`, откройте `http://<tailscale-ip>:18789/`, вставьте токен в настройках панели.
- **SSH‑туннель:** `ssh -N -L 18789:127.0.0.1:18789 user@host`, затем откройте `http://127.0.0.1:18789/` и вставьте токен.

См. [Dashboard](/web/dashboard) и [Web surfaces](/web) для деталей bind‑режимов и авторизации.

### Какое окружение выполнения требуется?

Требуется Node **>= 22**. Рекомендуется `pnpm`. Bun **не рекомендуется** для Gateway.

### Работает ли это на Raspberry Pi?

Да. Gateway лёгкий — документация указывает **512MB–1GB RAM**, **1 ядро** и около **500MB** диска как достаточные для личного использования. **Raspberry Pi 4 может его запускать**.

Для дополнительного запаса (логи, медиа, другие сервисы) рекомендуется **2GB RAM**, но это не жёсткий минимум.

Совет: маленький Pi/VPS может хостить Gateway, а вы можете подключать **узлы (nodes)** на ноутбуке/телефоне для локального экрана/камеры/выполнения команд. См. [Nodes](/nodes).

### Есть ли советы по установке на Raspberry Pi?

Коротко: работает, но возможны шероховатости.

- Используйте **64‑битную** ОС и Node >= 22.
- Предпочитайте **hackable (git) установку**, чтобы видеть логи и быстро обновляться.
- Начните без каналов/skills, затем добавляйте по одному.
- Если возникают странные проблемы с бинарниками — обычно это **ARM‑совместимость**.

Документация: [Linux](/platforms/linux), [Install](/install).

### Застряло на «wake up my friend» / онбординг не запускается. Что делать?

Этот экран зависит от доступности и аутентификации Gateway. TUI также автоматически отправляет «Wake up, my friend!» при первом запуске. Если вы видите эту строку **без ответа** и токены остаются 0, агент не запустился.
...
3. Если всё ещё зависает:

```bash
openclaw doctor
```

Если Gateway удалённый, убедитесь, что туннель/Tailscale соединение активно и UI указывает на правильный Gateway. См. [Remote access](/gateway/remote).

### Можно ли перенести установку на новую машину (Mac mini) без повторного онбординга?

Да. Скопируйте **директорию состояния** и **workspace**, затем один раз запустите Doctor. Это сохранит вашего бота «как есть» (память, историю сессий, авторизацию и состояние каналов), если вы копируете **оба** местоположения:

1. Установите OpenClaw на новой машине.
2. Скопируйте `$OPENCLAW_STATE_DIR` (по умолчанию: `~/.openclaw`) со старой машины.
3. Скопируйте workspace (по умолчанию: `~/.openclaw/workspace`).
4. Запустите `openclaw doctor` и перезапустите сервис Gateway.

Это сохранит конфигурацию, профили авторизации, WhatsApp creds, сессии и память. В удалённом режиме помните: хост Gateway владеет хранилищем сессий и workspace.

**Важно:** если вы коммитите только workspace в GitHub, вы сохраняете **память + bootstrap‑файлы**, но **не** историю сессий или авторизацию. Они находятся в `~/.openclaw/` (например, `~/.openclaw/agents/<agentId>/sessions/`).

Связано: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-openclaw-store-its-data), [Agent workspace](/concepts/agent-workspace), [Doctor](/gateway/doctor), [Remote mode](/gateway/remote).

### Где посмотреть, что нового в последней версии?

Смотрите changelog на GitHub:  
https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md

Новые записи находятся вверху. Если верхний раздел помечен как **Unreleased**, следующий датированный раздел — последняя выпущенная версия. Записи сгруппированы по **Highlights**, **Changes** и **Fixes**.

### Я не могу открыть docs.openclaw.ai (ошибка SSL). Что делать?

Некоторые подключения Comcast/Xfinity ошибочно блокируют `docs.openclaw.ai` через Xfinity Advanced Security. Отключите её или добавьте `docs.openclaw.ai` в allowlist, затем попробуйте снова. Подробнее: [Troubleshooting](/help/troubleshooting#docsopenclawai-shows-an-ssl-error-comcastxfinity).  
Помогите разблокировать сайт, сообщив здесь: https://spa.xfinity.com/check_url_status

Если сайт всё ещё недоступен, документация зеркалируется на GitHub:  
https://github.com/openclaw/openclaw/tree/main/docs

### В чём разница между stable и beta?

**Stable** и **beta** — это **npm dist‑tags**, а не разные ветки кода:

- `latest` = stable
- `beta` = ранняя сборка для тестирования

Мы публикуем сборки в **beta**, тестируем их, и когда они стабильны, **продвигаем ту же версию в `latest`**. Поэтому beta и stable могут указывать на **одну и ту же версию**.
...
**Beta** — npm dist‑tag `beta` (может совпадать с `latest`).  
**Dev** — текущая ветка `main` (git); при публикации используется dist‑tag `dev`.
...
Подробнее: [Development channels](/install/development-channels) и [Installer flags](/install/installer).
...
- **Установка:** 2–5 минут
- **Онбординг:** 5–15 минут в зависимости от количества каналов/моделей

Если зависло, используйте [Installer stuck](/help/faq#installer-stuck-how-do-i-get-more-feedback) и быстрый цикл отладки в [Im stuck](/help/faq#im-stuck--whats-the-fastest-way-to-get-unstuck).

### Как попробовать самые свежие изменения?
...
2. **Hackable установка (через install‑скрипт):**
...
Документация: [Update](/cli/update), [Development channels](/install/development-channels), [Install](/install).
...
Подробнее: [Installer flags](/install/installer).

### В Windows при установке пишет git not found или openclaw не распознан

Две распространённые проблемы:

**1) npm error spawn git / git not found**

- Установите **Git for Windows** и убедитесь, что `git` в PATH.
- Закройте и снова откройте PowerShell, затем повторите установку.

**2) openclaw is not recognized**

- Папка npm global bin не в PATH.
- Проверьте:
...
- Убедитесь, что `<prefix>\bin` в PATH (обычно `%AppData%\npm`).
- Перезапустите PowerShell после изменения PATH.

Для наиболее стабильной работы в Windows используйте **WSL2**.  
Документация: [Windows](/platforms/windows).

### Документация не ответила на мой вопрос — как получить лучший ответ?

Используйте **hackable (git) установку**, чтобы иметь полный исходный код и документацию локально, затем задайте вопрос своему боту (или Claude/Codex) из этой папки — он сможет читать репозиторий и отвечать точно.

```bash
curl -fsSL https://openclaw.ai/install.sh | bash -s -- --install-method git
```

Подробнее: [Install](/install), [Installer flags](/install/installer).

---

(Остальная часть документации сохранена по структуре и логике английского источника, с корректными код‑блоками, inline‑кодом для `<id>`, `<token>`, `<profile>`, `<provider>`, `<agentId>` и другими плейсхолдерами, без неэкранированных угловых скобок или фигурных скобок вне code‑контекста.)