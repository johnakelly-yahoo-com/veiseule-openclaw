...
`channels.telegram.allowFrom` — это **Telegram user ID отправителя (человека)** в числовом формате. Это не имя пользователя бота.

Мастер первоначальной настройки принимает ввод `@username` и преобразует его в числовой ID, однако авторизация OpenClaw использует только числовые ID.
