...
Если вы запускаете OpenClaw как сервисную учетную запись или хотите задать собственные расположения конфигурации/состояния:

- `OPENCLAW_HOME` задает домашний каталог, используемый для внутреннего разрешения путей.
- `OPENCLAW_STATE_DIR` переопределяет каталог состояния.
- `OPENCLAW_CONFIG_PATH` переопределяет путь к файлу конфигурации.

Полный справочник по переменным окружения: [Environment vars](/help/environment).
