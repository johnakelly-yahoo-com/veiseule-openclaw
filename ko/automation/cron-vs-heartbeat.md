...
| 매일 정확히 오전 9 시에 보고서 전송   | Cron (격리)         | 정확한 타이밍 필요               |
| 다가오는 일정 이벤트 모니터링        | Heartbeat                            | 주기적 인지에 자연스럽게 적합         |
| 매주 심층 분석 실행             | Cron (격리)         | 독립 작업, 다른 모델 사용 가능       |
| Remind me in 20 minutes | Cron (메인, `--at`) | 정확한 타이밍의 단발성 작업          |
| 백그라운드 프로젝트 상태 점검        | Heartbeat                            | 기존 사이클을 활용함              |
