...
- Bonjour/mDNS TXT 레코드는 **unauthenticated** 상태입니다. 클라이언트는 TXT 값을 UX 힌트로만 취급해야 합니다.
- 라우팅(호스트/포트)은 TXT에서 제공된 `lanHost`, `tailnetDns`, `gatewayPort`보다 **resolved service endpoint**(SRV + A/AAAA)를 우선해야 합니다.
- TLS pinning은 광고된 `gatewayTlsSha256`이 이전에 저장된 핀을 재정의하도록 절대 허용해서는 안 됩니다.
- iOS/Android 노드는 discovery 기반 직접 연결을 **TLS-only**로 처리해야 하며, 최초 핀을 저장하기 전에 명시적인 “trust this fingerprint” 확인(아웃오브밴드 검증)을 요구해야 합니다.
