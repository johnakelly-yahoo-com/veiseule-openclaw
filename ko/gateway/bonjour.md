...
- 클라이언트는 확인된 서비스 엔드포인트(SRV + A/AAAA)를 사용하여 라우팅해야 합니다. `lanHost`, `tailnetDns`, `gatewayPort`, `gatewayTlsSha256`은 힌트로만 취급하세요.
- TLS 핀닝에서는 광고된 `gatewayTlsSha256`이 이전에 저장된 핀을 절대 덮어쓰지 못하도록 해야 합니다.
- iOS/Android 노드는 디스커버리 기반 직접 연결을 **TLS 전용**으로 처리해야 하며, 최초 지문을 신뢰하기 전에 명시적인 사용자 확인을 요구해야 합니다.
