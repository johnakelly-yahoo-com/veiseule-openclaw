...
- **정책 드리프트/구성 오류** (sandbox docker 설정이 구성되었지만 sandbox 모드가 꺼져 있음; 효과가 없는 `gateway.nodes.denyCommands` 패턴; 전역 `tools.profile="minimal"`이 에이전트별 프로필에 의해 재정의됨; 확장 플러그인 도구가 느슨한 도구 정책 하에서 접근 가능).
...
- OpenResponses URL 입력(`input_file` / `input_image`)의 경우, 엄격한
  `gateway.http.endpoints.responses.files.urlAllowlist` 및
  `gateway.http.endpoints.responses.images.urlAllowlist`를 설정하고 `maxUrlParts`를 낮게 유지하세요.
...
- Control UI (SPA 자산) (기본 경로 `/`)
- Canvas 호스트: `/__openclaw__/canvas/` 및 `/__openclaw__/a2ui/` (임의의 HTML/JS; 신뢰할 수 없는 콘텐츠로 취급)

일반 브라우저에서 canvas 콘텐츠를 로드하는 경우, 다른 신뢰할 수 없는 웹 페이지와 동일하게 취급하세요:

- 신뢰할 수 없는 네트워크/사용자에게 canvas 호스트를 노출하지 마세요.
- canvas 콘텐츠가 권한이 있는 웹 표면과 동일한 origin을 공유하지 않도록 하세요. 그 영향에 대해 충분히 이해한 경우에만 예외로 하세요.
...
- `gateway.auth.mode: "trusted-proxy"`: 사용자를 인증하고 헤더를 통해 신원을 전달하는 identity-aware 리버스 프록시를 신뢰합니다([Trusted Proxy Auth](/gateway/trusted-proxy-auth) 참고).
...
- `tools.exec.applyPatch.workspaceOnly: true` (기본값): sandboxing이 꺼져 있어도 `apply_patch`가 워크스페이스 디렉터리 외부에 쓰기/삭제하지 못하도록 보장합니다. 의도적으로 `apply_patch`가 워크스페이스 외부의 파일을 수정하도록 하려는 경우에만 `false`로 설정하세요.
- `tools.fs.workspaceOnly: true` (선택 사항): `read`/`write`/`edit`/`apply_patch` 경로를 워크스페이스 디렉터리로 제한합니다(현재 절대 경로를 허용하고 있고 단일 보호 장치를 추가하려는 경우 유용).
...
flowchart TB
    ...
    B -- 신뢰하되 검증 --> C["허용 목록의 친구들"]
    C -- 제한적 신뢰 --> D["낯선 사람들"]
    D -- 신뢰 없음 --> E["~ 찾기를 요청하는 Mario"]
    E -- 절대적으로 신뢰 안 함 😏 --> F[" "]
