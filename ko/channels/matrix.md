...
다중 계정 지원: 계정별 자격 증명과 선택적 `name`을 사용해 `channels.matrix.accounts`를 사용하세요. 공통 패턴은 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)을 참고하세요.

각 계정은 모든 homeserver에서 별도의 Matrix 사용자로 실행됩니다. 계정별 구성
최상위 `channels.matrix` 설정을 상속하며 모든 옵션(DM 정책, 그룹, 암호화 등)을 재정의할 수 있습니다.
...
- 동시 모듈 import로 인한 경쟁 상태를 방지하기 위해 계정 시작은 직렬화됩니다.
- 환경 변수(`MATRIX_HOMESERVER`, `MATRIX_ACCESS_TOKEN` 등)는 **기본** 계정에만 적용됩니다.
- 기본 채널 설정(DM 정책, 그룹 정책, 멘션 게이팅 등)은 계정별로 재정의하지 않는 한 모든 계정에 적용됩니다.
- 각 계정을 서로 다른 에이전트로 라우팅하려면 `bindings[].match.accountId`를 사용하세요.
- 암호화 상태는 계정 + access token별로 저장됩니다(계정마다 별도의 키 저장소).
...
- 표시 이름이나 로컬파트만 있는 형식(예: `"Alice"` 또는 `"alice"`)은 사용하지 마세요. 이들은 모호하며 allowlist 매칭에 사용되지 않습니다. 전체 `@user:server` ID를 사용하세요.
