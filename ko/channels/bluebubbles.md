...
- 항상 웹훅 비밀번호를 설정하세요. 리버스 프록시(Tailscale Serve/Funnel, nginx, Cloudflare Tunnel, ngrok)를 통해 게이트웨이를 노출하는 경우, 프록시가 루프백을 통해 게이트웨이에 연결할 수 있습니다. BlueBubbles 웹훅 핸들러는 전달 헤더가 포함된 요청을 프록시된 요청으로 간주하며, 비밀번호 없는 웹훅은 허용하지 않습니다.
...
- `channels.bluebubbles.mediaLocalRoots`: 외부로 전송되는 로컬 미디어 경로에 대해 허용되는 절대 로컬 디렉터리의 명시적 허용 목록입니다. 기본적으로 이 설정이 구성되지 않으면 로컬 경로 전송은 거부됩니다. 계정별 재정의: `channels.bluebubbles.accounts.<accountId> .mediaLocalRoots`.
