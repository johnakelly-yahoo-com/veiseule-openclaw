...
Private/LAN ship URL (고급):

기본적으로 OpenClaw는 이 플러그인에 대해 프라이빗/내부 호스트명 및 IP 범위를 차단합니다 (SSRF 강화).
ship URL이 프라이빗 네트워크에 있는 경우 (예: `http://192.168.1.50:8080` 또는 `http://localhost:8080`),
명시적으로 허용해야 합니다:
