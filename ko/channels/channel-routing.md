...
2. **상위 peer 일치**(스레드 상속).
3. `guildId` + `roles`를 통한 **Guild + 역할 일치**(Discord).
4. **길드 매칭** (Discord) — `guildId` 를 통해.
5. **팀 매칭** (Slack) — `teamId` 를 통해.
6. **계정 매칭** (채널의 `accountId`).
7. **채널 매칭** (해당 채널의 모든 계정).
8. **기본 에이전트** (`agents.list[].default`, 그렇지 않으면 목록의 첫 번째 항목, 최종 대안으로 `main`).
