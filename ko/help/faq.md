...
`channels.telegram.allowFrom`은 **사람 발신자의 Telegram 사용자 ID**(숫자형)입니다. 봇 사용자 이름이 아닙니다.

온보딩 마법사는 `@username` 입력을 받아 이를 숫자 ID로 변환하지만, OpenClaw 권한 부여는 숫자 ID만 사용합니다.
