...
- 기대 사항:
  ...
  - 빠르고 안정적이어야 함
- 풀 참고:
  - OpenClaw는 더 빠른 유닛 샤드를 위해 Node 22/23에서 Vitest `vmForks`를 사용합니다.
  - Node 24 이상에서는 Node VM 링크 오류(`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`)를 방지하기 위해 OpenClaw가 자동으로 일반 `forks`로 대체합니다.
  - `OPENCLAW_TEST_VM_FORKS=0`(`forks` 강제) 또는 `OPENCLAW_TEST_VM_FORKS=1`(`vmForks` 강제)로 수동 설정할 수 있습니다.
...
- 파일: `src/**/*.e2e.test.ts`
- 런타임 기본값:
  - 더 빠른 파일 시작을 위해 Vitest `vmForks`를 사용합니다.
  - 적응형 워커를 사용합니다(CI: 2-4, 로컬: 4-8).
  - 콘솔 I/O 오버헤드를 줄이기 위해 기본적으로 무음 모드로 실행됩니다.
- 유용한 재정의 옵션:
  - 워커 수를 강제하려면 `OPENCLAW_E2E_WORKERS=<n>`을 사용하세요(최대 16).
  - 자세한 콘솔 출력을 다시 활성화하려면 `OPENCLAW_E2E_VERBOSE=1`을 사용하세요.
- 범위:
...
- Cerebras: `cerebras/`…(접근 권한이 있는 경우) (if you have access) (if you have access)
