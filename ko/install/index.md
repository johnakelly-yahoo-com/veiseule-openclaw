...
<CardGroup cols={2}>
  ...
  <Card title="Podman" href="/install/podman" icon="container">
    루트리스 컨테이너: `setup-podman.sh`를 한 번 실행한 후, 런치 스크립트를 실행하세요.
...
사용자 정의 런타임 경로가 필요한 경우 다음을 사용하세요:

- 홈 디렉터리 기반 내부 경로에는 `OPENCLAW_HOME`
- 변경 가능한 상태 위치에는 `OPENCLAW_STATE_DIR`
- 설정 파일 위치에는 `OPENCLAW_CONFIG_PATH`

우선순위와 전체 세부 정보는 [Environment vars](/help/environment)를 참조하세요.
