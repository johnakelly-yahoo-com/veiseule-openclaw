...
그룹 채팅에 `requireMention: true`가 설정된 경우, OpenClaw는 이제 멘션을 확인하기 **전에** 오디오를 전사합니다. 이 기능을 사용하면 음성 메모에 멘션이 포함된 경우에도 처리할 수 있습니다.

**작동 방식:**

1. 음성 메시지에 텍스트 본문이 없고 그룹에서 멘션이 필요한 경우, OpenClaw는 "프리플라이트" 전사를 수행합니다.
2. 전사된 텍스트는 멘션 패턴(예: `@BotName`, 이모지 트리거)을 확인합니다.
3. 멘션이 발견되면 메시지는 전체 응답 파이프라인을 거칩니다.
4. 멘션 감지를 위해 전사 텍스트가 사용되므로 음성 메모도 멘션 게이트를 통과할 수 있습니다.

**대체 동작:**

- 프리플라이트 중 전사에 실패하면(타임아웃, API 오류 등) 텍스트 전용 멘션 감지를 기준으로 메시지가 처리됩니다.
- 이를 통해 텍스트와 오디오가 함께 포함된 메시지가 잘못 누락되는 일을 방지합니다.

**예시:** 사용자가 `requireMention: true`가 설정된 Telegram 그룹에서 "Hey @Claude, what's the weather?"라고 말하는 음성 메모를 보냅니다. 음성 메모가 전사되고 멘션이 감지되면 에이전트가 응답합니다.
...
- 프리플라이트 전사는 멘션 감지를 위해 **첫 번째** 오디오 첨부 파일만 처리합니다. 추가 오디오는 메인 미디어 이해 단계에서 처리됩니다.
