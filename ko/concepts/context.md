...
큰 파일은 파일별로 `agents.defaults.bootstrapMaxChars` 를 사용해 잘립니다 (기본값 `20000` 문자). OpenClaw는 파일 전반에 걸쳐 총 부트스트랩 주입 한도를 `agents.defaults.bootstrapTotalMaxChars`(기본값 `24000`자)로 적용합니다. `/context` 는 **원본 대비 주입된** 크기와 잘림 발생 여부를 표시합니다.
