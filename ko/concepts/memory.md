...
- 메모리 검색은 최상위 `memorySearch`가 아닌 `agents.defaults.memorySearch` 아래에서 구성하세요.
...
- 이제 gateway는 시작 시 QMD 매니저를 초기화하므로, 첫 `memory_search` 호출 이전에도 주기적 업데이트 타이머가 설정됩니다.
- 이제 부팅 시 새로고침은 기본적으로 백그라운드에서 실행되어
  채팅 시작이 차단되지 않습니다. 이전의 차단 동작을 유지하려면
  `memory.qmd.update.waitForBootSync = true` 을 설정하십시오.
- 검색은 `qmd query --json` 를 통해 실행됩니다. 선택한 모드가 현재 QMD 빌드에서 플래그를 허용하지 않으면, OpenClaw는 `qmd query`로 다시 시도합니다. QMD 가 실패하거나 바이너리가
...
- `searchMode`(기본값 `search`): 어떤 QMD 명령이 `memory_search`를 처리할지 선택합니다 (`search`, `vsearch`, `query`).
...
- `scope`: [`session.sendPolicy`](/gateway/configuration#session)과 동일한 스키마입니다.
  ...
  - `match.keyPrefix`는 **정규화된** 세션 키(소문자로 변환되고, 앞에 있는 `agent:<id>:`가 제거된 값)와 일치합니다. 예: `discord:channel:`.
  - `match.rawKeyPrefix`는 `agent:<id>:`를 포함한 **원본** 세션 키(소문자 변환)와 일치합니다. 예: `agent:main:discord:`.
  - 레거시: `match.keyPrefix: "agent:..."`도 여전히 원본 키 접두사로 처리되지만, 명확성을 위해 `rawKeyPrefix` 사용을 권장합니다.
