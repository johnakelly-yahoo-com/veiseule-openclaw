...
- Nadpisania dla poszczególnych typów (opcjonalne): `resetByType` pozwala nadpisać politykę dla sesji `direct`, `group` i `thread` (thread = wątki w Slack/Discord, tematy w Telegramie, wątki w Matrix, jeśli są udostępniane przez konektor).
