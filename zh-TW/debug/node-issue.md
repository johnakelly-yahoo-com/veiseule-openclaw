...
- `tsx` 使用 esbuild 來轉換 TS/ESM。esbuild 的 `keepNames` 會產生一個 `__name` 輔助函式，並以 `__name(...)` 包裝函式定義。 `tsx` 使用 esbuild 來轉換 TS/ESM。esbuild 的 `keepNames` 會產生一個 `__name` 輔助函式，並以 `__name(...)` 包裝函式定義。 esbuild’s `keepNames` emits a `__name` helper and wraps function definitions with `__name(...)`.
