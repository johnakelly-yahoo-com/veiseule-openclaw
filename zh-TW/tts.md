...
- `maxTextLength`：TTS 輸入的硬性上限（字元）。超過時 `/tts audio` 會失敗。 `maxTextLength`：TTS 輸入的硬性上限（字元）。超過時 `/tts audio` 會失敗。 `/tts audio` fails if exceeded.
- `timeoutMs`：請求逾時（毫秒）。
...
- **Edge TTS**：使用 `edge.outputFormat`（預設 `audio-24khz-48kbitrate-mono-mp3`）。
  - `node-edge-tts` 可接受 `outputFormat`，但並非所有格式都可由 Edge 服務提供。citeturn2search0 citeturn2search0 citeturn2search0
  - 輸出格式值遵循 Microsoft Speech 輸出格式（包含 Ogg/WebM Opus）。citeturn1search0 citeturn1search0 citeturn1search0
  - Telegram 的 `sendVoice` 接受 OGG/MP3/M4A；若需要保證的 Opus 語音訊息，請使用 OpenAI/ElevenLabs。citeturn1search1 citeturn1search1 citeturn1search1
...
`tts` 工具會將文字轉為語音並回傳一個 `MEDIA:` 路徑。當結果與 Telegram 相容時，該工具會包含 `[[audio_as_voice]]`，使 Telegram 送出語音氣泡。 `tts` 工具會將文字轉為語音並回傳一個 `MEDIA:` 路徑。當結果與 Telegram 相容時，該工具會包含 `[[audio_as_voice]]`，使 Telegram 送出語音氣泡。 When the
