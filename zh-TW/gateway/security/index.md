...
OpenClaw is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **不存在「完美安全」的設定。** 目標是有意識地考量：
...
- **外掛**（存在未經明確允許清單的擴充）。
- **策略漂移／設定錯誤**（已設定 sandbox docker 參數但未啟用 sandbox 模式；無效的 `gateway.nodes.denyCommands` 模式；全域 `tools.profile="minimal"` 被個別 agent 設定覆寫；在寬鬆工具策略下仍可存取擴充外掛工具）。
...
identity. 如果你啟用 `gateway.controlUi.allowInsecureAuth`，UI 會回退為 **僅權杖驗證**，並在省略裝置身分時跳過裝置配對。 This is a security
downgrade—prefer HTTPS (Tailscale Serve) or open the UI on `127.0.0.1`. This is a security
...
boundary and lock down permissions on `~/.openclaw` (see the audit section below). 如果你需要在代理之間有更強的隔離，請在不同的作業系統使用者或不同的主機下執行它們。
...
斜線指令與指示只會對 **已授權的傳送者** 生效。 23. 授權來源於頻道允許清單／配對，以及 `commands.useAccessGroups`（請參閱 [Configuration](/gateway/configuration) 與 [Slash commands](/tools/slash-commands)）。 24. 如果頻道允許清單為空或包含 `"*"`，該頻道的指令實際上是開放的。

`/exec` 是提供給已授權操作人員的僅限工作階段便利功能。 26. 它 **不會** 寫入設定或變更其他工作階段。
...
- `pairing` (default): unknown senders receive a short pairing code and the bot ignores their message until approved. 配對碼在 1 小時後過期；重複的私訊在建立新的請求前不會重新傳送配對碼。 35. 35. 待處理的請求預設每個頻道上限為 **3 個**。
- `allowlist`：封鎖未知寄件者（無配對流程）。
- `open`：允許任何人 DM（公開）。**需要**頻道允許清單包含 `"*"`（明確選擇加入）。 `open`：允許任何人 DM（公開）。**需要**頻道允許清單包含 `"*"`（明確選擇加入）。 **Requires** the channel allowlist to include `"*"` (explicit opt-in).
- `disabled`：完全忽略入站 DMs。
...
39. 如果你在同一個頻道上執行多個帳號，請改用 `per-account-channel-peer`。 如果你在同一個頻道上執行多個帳號，請改用 `per-account-channel-peer`。 如果你在同一頻道上執行多個帳戶，請改用 `per-account-channel-peer`。如果同一個人透過多個頻道聯絡你，請使用 `session.identityLinks` 將那些 DM 工作階段合併為一個標準身分。請參閱 [工作階段管理](/concepts/session) 與 [設定](/gateway/configuration)。 40. 請參閱 [Session Management](/concepts/session) 與 [Configuration](/gateway/configuration)。
...
即使有強力的系統提示，**提示注入仍未被解決**。 26. 系統提示的防護欄僅是軟性指引；真正的硬性約束來自工具政策、exec 核准、沙箱化，以及頻道允許清單（而且操作員可依設計將這些關閉）。 27. 實務上有幫助的是：
...
- **Model choice matters:** older/legacy models can be less robust against prompt injection and tool misuse. Prefer modern, instruction-hardened models for any bot with tools. 我們建議使用 Anthropic Opus 4.6（或最新的 Opus），因為它在辨識提示注入方面表現出色（請參閱 [“A step forward on safety”](https://www.anthropic.com/news/claude-opus-4-5)）。
...
即使 **只有你** 能傳訊息給機器人，提示注入仍可能透過機器人讀取的任何 **不受信任內容** 發生（網路搜尋／抓取結果、瀏覽器頁面、電子郵件、文件、附件、貼上的日誌／程式碼）。 15. 換言之：發送者不是唯一的威脅面；**內容本身**也可能攜帶對抗性指令。
...
- 對於 OpenResponses 的 URL 輸入（`input_file` / `input_image`），請設定嚴格的
...
Prompt injection resistance is **not** uniform across model tiers. 較小／較便宜的模型通常更容易遭受工具濫用與指令劫持，特別是在對抗性提示下。
...
`/reasoning` 與 `/verbose` 可能會暴露原本不打算公開到頻道的內部推理或工具輸出。 45. 在群組情境中，請將它們視為 **僅供除錯**，除非你明確需要，否則保持關閉。
...
36. **教訓：** 不要讓陌生人（或朋友！） **教訓：** 不要讓陌生人（或朋友！） manipulate your AI into exploring the filesystem.
...
此 HTTP 介面包含 Control UI 與 canvas host：

- Control UI（SPA 靜態資產）（預設基礎路徑為 `/`）
- Canvas host：`/__openclaw__/canvas/` 和 `/__openclaw__/a2ui/`（可包含任意 HTML/JS；應視為不受信任內容）

如果你在一般瀏覽器中載入 canvas 內容，請將其視為任何其他不受信任的網頁：

- 不要將 canvas host 暴露給不受信任的網路／使用者。
- 除非你完全理解其影響，否則不要讓 canvas 內容與具有特權的 Web 介面共用相同的來源（origin）。

綁定模式控制 Gateway 閘道器的監聽位置：

- `gateway.bind: "loopback"`（預設）：僅本機用戶端可連線。
- 38. 非回送位址的綁定（`"lan"`, `"tailnet"`, `"custom"`）會擴大攻擊面。 非回送位址的綁定（`"lan"`, `"tailnet"`, `"custom"`）會擴大攻擊面。 Only use them with a shared token/password and a real firewall.
...
在最小模式下，Gateway 閘道器仍會廣播足夠的資訊以供裝置探索（`role`、`gatewayPort`、`transport`），但會省略 `cliPath` 與 `sshPort`。需要 CLI 路徑資訊的應用程式可改由已驗證的 WebSocket 連線取得。 Apps that need CLI path information can fetch it via the authenticated WebSocket connection instead. Apps that need CLI path information can fetch it via the authenticated WebSocket connection instead.
...
- `gateway.auth.mode: "trusted-proxy"`：信任具備身分識別能力的反向代理來驗證使用者，並透過標頭傳遞身分資訊（請參閱 [Trusted Proxy Auth](/gateway/trusted-proxy-auth)）。
...
and matching it to the header. OpenClaw verifies the identity by resolving the
`x-forwarded-for` address through the local Tailscale daemon (`tailscale whois`)
...
- `tools.exec.applyPatch.workspaceOnly: true`（預設）：確保即使未啟用 sandbox，`apply_patch` 也無法在工作區目錄之外寫入或刪除檔案。 僅在你有意讓 `apply_patch` 存取工作區之外的檔案時，才將其設為 `false`。
- `tools.fs.workspaceOnly: true`（選用）：將 `read`／`write`／`edit`／`apply_patch` 的路徑限制在工作區目錄內（若你目前允許使用絕對路徑，並希望加入單一道防護機制，這會很有幫助）。
...
single container/workspace. `scope: "shared"` uses a
...
發現 OpenClaw 的漏洞嗎？請負責任地回報： Please report responsibly: Please report responsibly:
