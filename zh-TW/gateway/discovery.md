...
Bonjour 為盡力而為，且不會跨網路。 Bonjour 為盡力而為，且不會跨網路。 僅用於「同一個 LAN」的便利性。
...
- Bonjour/mDNS TXT 記錄為**未經驗證**。 用戶端必須僅將 TXT 值視為使用者體驗提示。
- 路由（host/port）應優先使用**已解析的服務端點**（SRV + A/AAAA），而非 TXT 提供的 `lanHost`、`tailnetDns` 或 `gatewayPort`。
- TLS 釘選絕不可允許已宣告的 `gatewayTlsSha256` 覆寫先前儲存的釘選值。
- iOS/Android 節點應將基於探索的直接連線視為**僅限 TLS**，並在儲存首次釘選前要求明確的「信任此指紋」確認（帶外驗證）。
