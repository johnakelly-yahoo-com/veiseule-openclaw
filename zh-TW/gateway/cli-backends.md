...
OpenClaw 可以在 API 提供者當機、被限流，或暫時行為異常時，執行 **本機 AI CLI** 作為 **純文字備援**。此設計刻意保守： This is intentionally conservative: This is intentionally conservative:
...
This is designed as a **safety net** rather than a primary path. 這是設計為一個**安全網**，而非主要使用路徑。請在以下情況使用它：
...
- 如果主要供應商失敗（驗證、速率限制、逾時），OpenClaw 將會
  接著嘗試使用 CLI 後端。
...
每個項目都以**provider id** 為鍵（例如 `claude-cli`、`my-cli`）。
...
5. **為每個後端持久化 session id**，讓後續請求重用相同的 CLI 工作階段。
...
- `sessionMode`：
  ...
  - `existing`：僅在先前已儲存 session id 的情況下才傳送。
  - `none`：永不送出工作階段 id。
...
OpenClaw 會將 base64 圖像寫入暫存檔。如果設定了 `imageArg`，這些
paths are passed as CLI args. If `imageArg` is set, those
...
- **沒有 OpenClaw 工具**（CLI 後端永遠不會接收工具呼叫）。部分 CLI
  仍可能執行其自身的代理程式工具。 **沒有 OpenClaw 工具**（CLI 後端永遠不會接收工具呼叫）。部分 CLI
...
- **Codex CLI 工作階段** 透過文字輸出復原（非 JSONL），其結構化程度
  ...
  normally. OpenClaw sessions still work
  normally.
