...
- 配對完成後，Gateway 會發行一個以連線角色 + 範圍為界定的**裝置權杖**。 34. 該權杖會在 `hello-ok.auth.deviceToken` 中回傳，客戶端應將其持久化以供日後連線使用。
...
- 所有 WS 客戶端在 `connect` 時都必須包含 `device` 身分（操作員 + 節點）。
  僅在啟用 `gateway.controlUi.allowInsecureAuth`（或作為破窗用途的 `gateway.controlUi.dangerouslyDisableDeviceAuth`）時，控制 UI 才可省略。
...
agent, sessions, nodes, approvals, etc.). 確切的介面由 `src/gateway/protocol/schema.ts` 中的 TypeBox 結構描述定義。
