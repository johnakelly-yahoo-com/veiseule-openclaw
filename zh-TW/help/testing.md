...
- 期望：
  ...
  - 應快速且穩定
- Pool 說明：
  - OpenClaw 在 Node 22/23 上使用 Vitest `vmForks` 以加快單元測試分片速度。
  - 在 Node 24 以上版本中，OpenClaw 會自動回退為一般 `forks`，以避免 Node VM 連結錯誤（`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`）。
  - 可使用 `OPENCLAW_TEST_VM_FORKS=0`（強制使用 `forks`）或 `OPENCLAW_TEST_VM_FORKS=1`（強制使用 `vmForks`）手動覆寫。
...
- 實用覆寫選項：
  - `OPENCLAW_E2E_WORKERS=<n>` 以強制指定 worker 數量（上限為 16）。
...
- Cerebras：`cerebras/`…（若你有存取權） (if you have access) (if you have access)
- LM Studio：`lmstudio/`…（本機；工具呼叫取決於 API 模式） LM Studio：`lmstudio/`…（本機；工具呼叫取決於 API 模式） (local; tool calling depends on API mode)

### 視覺：影像傳送（附件 → 多模態訊息）

在 `OPENCLAW_LIVE_GATEWAY_MODELS` 中至少包含一個支援影像的模型（Claude／Gemini／OpenAI 具備視覺能力的變體等），以驗證影像探測。 to exercise the image probe. to exercise the image probe.
...
Live 測試探索憑證的方式與 CLI 相同。實務上的含意： Practical implications: Practical implications:
