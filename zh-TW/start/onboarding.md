...
This doc describes the **current** first‑run onboarding flow. 目標是一個
順暢的「第 0 天」體驗：選擇 Gateway 的執行位置、連接驗證、執行精靈，並讓代理自行完成啟動。
如需了解入門流程的整體概覽，請參閱 [Onboarding Overview](/start/onboarding-overview)。
...
- **此 Mac（僅限本機）：** 導覽流程可以執行 OAuth 流程並在本機寫入憑證。
