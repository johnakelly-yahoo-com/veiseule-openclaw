...
- 私訊安全性：預設為配對。第一則私訊會傳送代碼；透過
  ...
  `openclaw pairing approve <channel><code>` 核准，或使用允許清單。 First DM sends a code; approve via
      `openclaw pairing approve <channel><code>` 核准，或使用允許清單。
  
</Step></Step><code>` or use allowlists.
  ...
  <Step title="Daemon install">
    ...
    - 執行環境選擇：Node（建議；WhatsApp 與 Telegram 需要）。 Bun is not recommended. Bun is not recommended.
...
More detail: [Moonshot AI (Kimi + Kimi Coding)](/providers/moonshot).
  ...
  <Accordion title="Custom provider">適用於 OpenAI-compatible 與 Anthropic-compatible 端點。
    ...
    - `--custom-base-url`
    - `--custom-model-id`
    - `--custom-api-key`（選用；若未提供則使用 `CUSTOM_API_KEY`）
