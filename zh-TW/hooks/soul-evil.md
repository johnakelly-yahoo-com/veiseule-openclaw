...
SOUL Evil 勾點會在 purge 視窗期間或以隨機機率，將**注入的** `SOUL.md` 內容替換為 `SOUL_EVIL.md`。它**不會**修改磁碟上的檔案。 It does **not** modify files on disk. It does **not** modify files on disk.
...
OpenClaw logs a warning and keeps the normal `SOUL.md`. If `SOUL_EVIL.md` is missing or empty,
