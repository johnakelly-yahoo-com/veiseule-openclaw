...
外掛程式只是**小型程式碼模組**，用來為 OpenClaw 擴充額外功能
...
Npm 規格僅限於 **registry-only**（套件名稱 + 可選版本/標籤）。 Git/URL/file
specs 會被拒絕。
...
- 45. 回傳 PCM 音訊緩衝區 + 取樣率。 回傳 PCM 音訊緩衝區 + 取樣率。 Plugins must resample/encode for providers.
- 電話語音不支援 Edge TTS。
...
但也可以用相同方式停用。 .enabled`或`openclaw plugins enable <id>\` 明確啟用。已安裝的外掛預設為啟用，
...
安全性注意：`openclaw plugins install` 會使用
`npm install --ignore-scripts`（不執行 lifecycle scripts）來安裝外掛相依套件。 請保持外掛相依樹為「純 JS/TS」，並避免需要 `postinstall` 建置的套件。

### 46. 頻道目錄中繼資料

頻道外掛可透過 `openclaw.channel` 宣告新手導覽中繼資料，並透過 `openclaw.install` 提供安裝提示。 This keeps the core catalog data-free. This keeps the core catalog data-free.
...
registry export). 在以下其中一個位置放置 JSON 檔案：
...
外掛可以隨附 hooks 並在執行時註冊它們。 外掛可以隨附 hooks 並在執行時註冊它們。 This lets a plugin bundle
...
當你需要的是**新的聊天介面**（「訊息通道」），而非模型提供者時，請使用此功能。
模型提供者文件位於 `/providers/*`。
...
- `acceptsArgs`：是否接受參數（預設：false）。若為 false 且提供了參數，指令將不會匹配，訊息會交由其他處理器 `acceptsArgs`：是否接受參數（預設：false）。若為 false 且提供了參數，指令將不會匹配，訊息會交由其他處理器 If false and arguments are provided, the command won't match and the message falls through to other handlers
...
- 保留指令名稱（如 `help`、`status`、`reset` 等）不可被外掛覆寫 且不能被外掛覆寫 且不能被外掛覆寫
