...
預設情況下，CLI 會將技能安裝到目前工作目錄下的 `./skills`。如果已設定 OpenClaw 工作區，`clawhub` 會回退使用該工作區，除非你覆寫 `--workdir`（或 `CLAWHUB_WORKDIR`）。OpenClaw 會從 `<workspace>/skills` 載入工作區技能，並在**下一個**工作階段中將其載入。如果你已使用 `~/.openclaw/skills` 或內建技能，則工作區技能會具有較高優先順序。
