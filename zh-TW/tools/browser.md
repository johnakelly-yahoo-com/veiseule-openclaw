...
- `color` 與各設定檔的 `color` 會為瀏覽器 UI 加上色彩標示，讓你辨識目前啟用的是哪個設定檔。
...
- 請選擇與你的 Browserless 帳戶相符的區域端點（請參閱其文件）。

## 安全性

核心概念：

- 瀏覽器控制僅限於本機回環；存取會透過 Gateway 的驗證或節點配對流程。
