...
- `…`：正在連線到本地中繼。
- `!`：relay 無法連線（最常見原因：此機器上的瀏覽器 relay 伺服器未在執行）。
...
22.
Gateway 會將瀏覽器動作代理到該節點；擴充功能與轉送保持在瀏覽器所在的本機。
...
25. 這很強大，也很有風險。 這很強大，也很有風險。 Treat it like giving the model “hands on your browser”.

- 擴充功能使用 Chrome 的 debugger API（`chrome.debugger`）。附加後，模型可以： When attached, the model can: When attached, the model can:
