...
在 Ubuntu（以及許多 Linux 發行版）上，預設的 Chromium 安裝方式是 **snap 套件**。Snap 的 AppArmor 限制機制會干擾 OpenClaw 啟動與監控瀏覽器程序的方式。
...
您正在使用 `chrome` 設定檔（extension relay）。它預期 OpenClaw
