...
OpenClaw 正在快速發展（尚未到「1.0」）。請將更新視為部署基礎設施：更新 → 執行檢查 → 重新啟動（或使用 `openclaw update`，會自動重新啟動）→ 驗證。
...
3. 重新啟動 gateway，並將報告傳送（ping）給最後一個活動中的工作階段。
...
- 在全域安裝與 git 安裝之間切換很簡單：安裝另一種版本，然後執行 `openclaw doctor`，讓 gateway 服務的進入點重寫為目前的安裝版本。
