...
第一次部署會建置 Docker 映像（約 2–3 分鐘）。後續部署會更快。 Subsequent deploys are faster. Subsequent deploys are faster.
...
**注意：** 512MB 太小。1GB 可能可用，但在負載或詳細記錄下可能 OOM。**建議 2GB。** 25. 1GB 可能可行，但在高負載或詳細記錄日誌時可能會發生 OOM。 **建議使用 2GB。** **建議使用 2GB。**
...
如果使用 `--allow-unconfigured`，閘道會建立最小設定。 如果使用 `--allow-unconfigured`，閘道會建立最小設定。 若使用 `--allow-unconfigured`，Gateway 閘道器會建立最小設定。你在 `/data/openclaw.json` 的自訂設定應在重新啟動後被讀取。
