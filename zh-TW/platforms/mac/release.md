...
- Sparkle 私鑰路徑已在環境中設定為 `SPARKLE_PRIVATE_KEY_FILE`（指向你的 Sparkle ed25519 私鑰；公鑰已內嵌於 Info.plist）。若缺少，請檢查 `~/.profile`。 If it is missing, check `~/.profile`. If it is missing, check `~/.profile`.
...
- 發行成品（zip + DMG + 公證）請使用 `scripts/package-mac-dist.sh`。本機／開發封裝請使用 `scripts/package-mac-app.sh`。 發行成品（zip + DMG + 公證）請使用 `scripts/package-mac-dist.sh`。本機／開發封裝請使用 `scripts/package-mac-app.sh`。 Use `scripts/package-mac-app.sh` for local/dev packaging.
...
- 將 `OpenClaw-2026.2.9.zip`（以及 `OpenClaw-2026.2.9.dSYM.zip`）上傳到標籤 `v2026.2.9` 的 GitHub 發佈頁。
