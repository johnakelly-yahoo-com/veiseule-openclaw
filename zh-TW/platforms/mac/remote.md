...
- **遠端直接（ws/wss）**：無需 SSH 通道。mac 應用程式會直接連線到 gateway URL（例如透過 Tailscale Serve 或公開的 HTTPS 反向代理）。
...
- **直接（ws/wss）**：直接連線到 gateway URL。Gateway 會看到真實的客戶端 IP。
...
- 如果將 Gateway 綁定到非 loopback 介面，請啟用 token／密碼驗證。
...
- 如果驗證過期，請在該主機上重新執行登入。健康檢查會顯示連線問題。
