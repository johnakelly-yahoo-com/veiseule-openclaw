...
本文件提出一種**離線優先（offline-first）**的記憶架構，以 Markdown 作為可審閱的正典事實來源，同時透過衍生索引加入**結構化檢索**（搜尋、實體摘要、信心度更新）。
...
- **對代理友善**：讓「在 token 預算內進行召回」變得容易（回傳小型事實組合）。
...
- 其他所有內容皆視為脫離上下文，並透過工具檢索
- 記憶寫入為明確的工具呼叫（追加／取代／插入），持久化後於下一回合重新注入

2. **Hindsight 風格的記憶基底**

- 區分哪些是已觀察、哪些是已相信、哪些是已摘要
- 支援保留／回憶／反思
- 可隨證據演進而調整信心度的觀點
- 具備實體感知的檢索 + 時序查詢（即使沒有完整的知識圖譜）
