...
- 傳入訊息透過 webhook 接收；傳出回覆、輸入中指示、已讀回條與 tapback 則透過 REST 呼叫完成。
...
- 務必設定 webhook 密碼。 如果你透過反向代理（Tailscale Serve/Funnel、nginx、Cloudflare Tunnel、ngrok）公開 gateway，代理可能會透過 loopback 連線至 gateway。 BlueBubbles webhook 處理器會將帶有轉發標頭的請求視為經代理轉發，且不會接受未設密碼的 webhook。

## 保持 Messages.app 存活（VM / 無頭設定）

某些 macOS VM／常駐設定可能會讓 Messages.app 進入「閒置」狀態（直到開啟/前景化前，來訊事件會停止）。一個簡單的因應方式是使用 AppleScript + LaunchAgent **每 5 分鐘戳一下 Messages**。 A simple workaround is to **poke Messages every 5 minutes** using an AppleScript + LaunchAgent. A simple workaround is to **poke Messages every 5 minutes** using an AppleScript + LaunchAgent.
...
- 第一次執行可能會觸發 macOS 的**自動化**提示（`osascript` → Messages）。請在執行該 LaunchAgent 的同一使用者工作階段中核准。 第一次執行可能會觸發 macOS 的**自動化**提示（`osascript` → Messages）。請在執行該 LaunchAgent 的同一使用者工作階段中核准。 Approve them in the same user session that runs the LaunchAgent.
...
- 配對是預設的權杖交換方式。 3. 詳細資訊：[配對](/channels/pairing)
...
- `channels.bluebubbles.mediaLocalRoots`：明確允許的本機絕對目錄清單，用於對外傳送本機媒體路徑。 除非已設定此項，否則預設會拒絕傳送本機路徑。 每個帳號的覆寫設定：`channels.bluebubbles.accounts.<accountId>`.mediaLocalRoots\`.
...
- 直接識別碼：`+15555550123`、`user@example.com`
  - 如果直接 handle 沒有既有的 DM 聊天，OpenClaw 會透過 `POST /api/v1/chat/new` 建立一個。 11. 這需要啟用 BlueBubbles Private API。
...
- 信任 localhost 表示同一主機上的反向代理可能會在無意間繞過密碼。 14. 若您為 gateway 設定代理，請在代理端要求驗證，並設定 `gateway.trustedProxies`。 15. 請參閱 [Gateway 安全性](/gateway/security#reverse-proxy-configuration)。
