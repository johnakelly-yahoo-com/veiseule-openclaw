...
Teams 無法連線至 `localhost`。本機開發需使用通道： Use a tunnel for local development: Use a tunnel for local development:
...
- 回退至 `messages.groupChat.historyLimit`。設定 `0` 可停用（預設 50）。 Set `0` to disable (default 50). Set `0` to disable (default 50).
- 私訊（DM）歷史可用 `channels.msteams.dmHistoryLimit`（使用者回合）限制；每位使用者可用 `channels.telegram.dms["<user_id>"].historyLimit` 覆寫。 私訊（DM）歷史可用 `channels.msteams.dmHistoryLimit`（使用者回合）限制；每位使用者可用 `channels.telegram.dms["<user_id>"].historyLimit` 覆寫。 私訊歷史可用 `channels.msteams.dmHistoryLimit`（使用者輪次）限制。每位使用者可用 `channels.msteams.dms["<user_id>"].historyLimit` 覆寫。
...
**結論：** RSC 用於即時監聽；Graph API 用於歷史存取。若要在離線時補齊遺漏訊息，需要 Graph API 與 `ChannelMessage.Read.All`（需管理員同意）。 **結論：** RSC 用於即時監聽；Graph API 用於歷史存取。若要在離線時補齊遺漏訊息，需要 Graph API 與 `ChannelMessage.Read.All`（需管理員同意）。 For catching up on missed messages while offline, you need Graph API with `ChannelMessage.Read.All` (requires admin consent).
...
**使用者提及的額外權限：** 對於對話中的使用者，@mentions 可直接使用。 不過，如果你想動態搜尋並提及**不在目前對話中**的使用者，請新增 `User.Read.All`（Application）權限並授予管理員同意。

## 已知限制

### Webhook 逾時

Teams 透過 HTTP webhook 傳送訊息。若處理時間過長（例如 LLM 回應過慢），可能會看到： Teams 透過 HTTP webhook 傳送訊息。若處理時間過長（例如 LLM 回應過慢），可能會看到： If processing takes too long (e.g., slow LLM responses), you may see:
...
- `channels.msteams.allowFrom` 接受 AAD 物件 ID、UPN 或顯示名稱。 39. 在認證允許的情況下，精靈會透過 Microsoft Graph 將名稱解析為 ID。
...
**問題：** Teams API 不會暴露頻道使用的 UI 樣式。若使用錯誤的 `replyStyle`： If you use the wrong `replyStyle`: If you use the wrong `replyStyle`:
...
- **頻道／群組：** 附件存放於 M365 儲存體（SharePoint／OneDrive）。Webhook 負載僅含 HTML stub，沒有實際位元組。**需要 Graph API 權限** 才能下載頻道附件。 The webhook payload only includes an HTML stub, not the actual file bytes. The webhook payload only includes an HTML stub, not the actual file bytes. **Graph API permissions are required** to download channel attachments.
...
- 只有在使用者互動**之後**才能進行主動式訊息，因為我們會在該時點儲存對話參考。
