...
output internal. Use `--no-deliver` to keep
output internal. `--deliver` remains as a deprecated alias for `--announce`.

注意：一次性（`--at`）工作在成功後預設會刪除。使用 `--keep-after-run` 以保留它們。 注意：一次性（`--at`）工作在成功後預設會刪除。使用 `--keep-after-run` 以保留它們。 Use `--keep-after-run` to keep them.
