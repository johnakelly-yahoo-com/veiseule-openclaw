...
診斷旗標可讓您在不啟用全面冗長記錄的情況下，開啟目標式的除錯記錄。旗標為選擇性啟用，且只有在子系統檢查它們時才會生效。 診斷旗標可讓您在不啟用全面冗長記錄的情況下，開啟目標式的除錯記錄。旗標為選擇性啟用，且只有在子系統檢查它們時才會生效。 Flags are opt-in and have no effect unless a subsystem checks them.
...
如果你設定了 `logging.file`，則改用該路徑。 5. 日誌為 JSONL 格式（每行一個 JSON 物件）。 6. 仍會依據 `logging.redactSensitive` 套用遮蔽。
