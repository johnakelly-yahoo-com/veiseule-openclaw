...
9. 它**不會**在每次請求時輪替。 固定的設定檔會被重複使用，直到：
...
- 使用 `/model …` 的每工作階段覆寫並指定設定檔覆寫（當你的 UI/聊天介面支援時）。

## 19. 冷卻時間

20. 當設定檔因驗證/速率限制錯誤（或看起來像速率限制的逾時）而失敗時，
    OpenClaw 會將其標記為冷卻中，並移至下一個設定檔。
    格式/無效請求錯誤（例如 Cloud Code Assist 工具呼叫 ID 驗證失敗）會被視為值得回退，並使用相同的冷卻機制。
...
計費/點數失敗（例如「點數不足」/「點數餘額過低」）被視為值得回退，但通常不是暫時性的。 24. 不採用短冷卻時間，OpenClaw 會將設定檔標記為 **停用**（使用較長的退避），並輪替到下一個設定檔/供應商。
