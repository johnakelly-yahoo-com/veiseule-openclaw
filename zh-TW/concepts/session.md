...
OpenClaw 預設會在呼叫 LLM 之前，從記憶體中的上下文修剪**舊的工具結果**。
This does **not** rewrite JSONL history. See [/concepts/session-pruning](/concepts/session-pruning).

## 壓縮前記憶體清理

當工作階段接近自動壓縮時，OpenClaw 可以執行**靜默記憶體清理**。
...
- 重設策略：在過期前會重用工作階段，並於下一則傳入訊息時判定是否過期。
- 每日重設：預設為**Gateway 主機當地時間凌晨 4:00**。若工作階段的最後更新時間早於最近一次每日重設時間，則視為過期。
- 閒置重設（選用）：`idleMinutes` 會新增一個滑動式閒置時間視窗。當同時設定每日與閒置重設時，**以先到期者為準**，強制建立新的工作階段。
- 舊版僅閒置：若你只設定 `session.idleMinutes`，而未設定任何 `session.reset`/`resetByType`，OpenClaw 會為了相容性而維持僅閒置模式。
- 依類型覆寫（選用）：`resetByType` 允許你為 `direct`、`group` 與 `thread` 工作階段覆寫重設策略（thread = Slack/Discord 討論串、Telegram 主題、由連接器提供的 Matrix 討論串）。
- 依頻道覆寫（選用）：`resetByChannel` 會覆寫某頻道的重設政策（套用於該頻道的所有工作階段類型，且優先於 `reset`/`resetByType`）。
- 重設觸發條件：精確輸入 `/new` 或 `/reset`（以及 `resetTriggers` 中的任何其他觸發詞）會啟動新的工作階段 ID，並將訊息的其餘內容繼續傳遞。`/new <model>` 可接受模型別名、`provider/model`，或提供者名稱（模糊比對）以設定新工作階段的模型。若單獨送出 `/new` 或 `/reset`，OpenClaw 會執行簡短的「hello」問候回合以確認重設。
