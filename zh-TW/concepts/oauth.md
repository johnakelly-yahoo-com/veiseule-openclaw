...
OpenClaw 也支援隨附其自有 OAuth 或 API 金鑰流程的 **provider plugins**。可透過以下方式執行： 透過以下方式執行： 透過以下方式執行：
...
以上所有項目也都遵循 `$OPENCLAW_STATE_DIR`（狀態目錄覆寫）。 以上所有項目也都遵循 `$OPENCLAW_STATE_DIR`（狀態目錄覆寫）。 以上所有項目也都遵循 `$OPENCLAW_STATE_DIR`（狀態目錄覆寫）。完整參考：[/gateway/configuration](/gateway/configuration#auth-storage-oauth--api-keys)
