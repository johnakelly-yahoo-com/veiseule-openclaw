...
Qianfan 是百度的 MaaS 平台，提供**統一 API**，可透過單一端點與 API 金鑰，將請求路由至多個模型。它相容於 OpenAI，因此只要切換基底 URL，多數 OpenAI SDK 即可運作。 It is OpenAI-compatible, so most OpenAI SDKs work by switching the base URL. It is OpenAI-compatible, so most OpenAI SDKs work by switching the base URL.
