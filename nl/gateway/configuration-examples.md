...
De onderstaande voorbeelden zijn afgestemd op het huidige config-schema. Voor de volledige referentie en toelichting per veld, zie [Configuratie](/gateway/configuration).
...
> JSON5 laat je opmerkingen en afsluitende komma’s gebruiken. Gewoon JSON werkt ook.
...
### Beveiligde DM-modus (gedeelde inbox / multi-user DM’s)

Als meer dan één persoon je bot een DM kan sturen (meerdere vermeldingen in `allowFrom`, koppelingsgoedkeuringen voor meerdere personen, of `dmPolicy: "open"`), schakel **beveiligde DM-modus** in zodat DM’s van verschillende afzenders standaard niet één context delen:
...
### OAuth met API-sleutel failover
...
### Anthropic-abonnement + API-sleutel, MiniMax fallback
