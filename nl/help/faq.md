...
`channels.telegram.allowFrom` is **de Telegram-gebruikers-ID van de menselijke afzender** (numeriek). Het is niet de bot gebruikersnaam.

De onboarding-wizard accepteert invoer van `@username` en zet dit om naar een numerieke ID, maar OpenClaw-autorisatie gebruikt uitsluitend numerieke ID's.
