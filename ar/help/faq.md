...
7. إذا كانت المصادقة قيد التشغيل، قم بإدراج الرمز المميز/كلمة المرور في إطار "الاتصال".
...
استخدم وكيل الذكاء الاصطناعي المحلي الذي يمكنه **مشاهدة جهازك**. هذا أكثر فاعلية بكثير من السؤال في Discord، لأن معظم حالات "أنا عالق" هي **مشكلات إعداد محلية أو بيئية** لا يستطيع المساعدون عن بُعد فحصها.
...
```powershell
# install.ps1 لا يحتوي بعد على خيار -Verbose مخصص.
Set-PSDebug -Trace 1
...
### إذا كنت تريد هامشًا إضافيًا (سجلات، وسائط، خدمات أخرى)، **يوصى بـ 2GB**، لكنها ليست حدًا أدنى صارمًا.
...
1. إذا قمت بتعيين الاسم المستعار الخاص بك بنفس الاسم، فإن قيمتك تفوز.
2. الحفاظ على السياق الهام في مساحة العمل واطلب من البوت أن يقرأه مرة أخرى.
...
عادةً لا عادةً لا يحتاج OpenClaw إلى سياق كبير + أمان قوي؛ بطاقات صغيرة و تسرب. إذا كنت يجب عليك تشغيل **أكبر** بناء MiniMax M2.1 يمكنك محليا (LM Studio) وانظر [/gateway/local-models](/gateway/local-models). النماذج المصغرة/المحددة كمياً تزيد من خطر الحقن الفوري - انظر [Security](/gateway/security).
...
تحتاج إلى **بعض أجهزة macOS** تسجيل الدخول إلى الرسائل. ليس من الضروري **على الإطلاق** أن يكون جهاز Mac mini — أي جهاز Mac يعمل. **استخدم [BlueBubbles](/channels/bluebubbles)** (مستحسن) لiMessage - خادم BlueBbles يعمل على macOS، في حين يمكن تشغيل البوابة على Linux أو أي مكان آخر.
...
يقبل معالج الإعداد الإدخال بصيغة `@username` ويحوّله إلى معرّف رقمي، لكن تفويض OpenClaw يستخدم المعرّفات الرقمية فقط.
...
OpenClaw ليس مجرد غلاف كلود." إنه **مستوى تحكم محلي أولًا** يتيح لك تشغيل مساعد قوي على **أجهزتك الخاصة**، ويمكن الوصول إليه من تطبيقات الدردشة التي تستخدمها بالفعل، مع جلسات حالية وذاكرة وأدوات — دون تسليم التحكم في سير عملك إلى SaaS مستضاف.
...
OpenClaw هو **مساعد شخصي** وطبقة تنسيق، وليس بديل لـ IDE. استخدم
...
**الخيار A - تشغيل الـ Gateway على جهاز Mac (الأبسط).**
...
### الترقية إلى **2026.1.12** (أو تشغيل من المصدر \\`main')، ثم إعادة تشغيل البوابة.
...
الصورة الافتراضية تركز على الأمان وتعمل كمستخدم `node`، لذا فهي لا
...
- # الإعدادات الافتراضية إلى الوكيل الافتراضي (حذف --وكيل)نماذج openclaw Auth order الحصول على --مقدم الخدمات الاصطناعي# قفل تدوير لملف شخصي واحد (فقط حاول هذا)نماذج openclaw Forth Order مجموعة --مورد الانتاج البشري: الإفتراضي# أو تعيين طلب صريح (رد الفعل داخل المزود)نماذج openclaw Auth الطلب--مجموعة موفر الاصطناعي: العمل الانساني الاصطناعي:# Clear override (العودة إلى ضبط المصادقة. rder / round-robin)openclaw ordth order clear--مزود الإنسان
- ما الذي يساعد:
...
| إغلاق وإعادة فتح PowerShell ، ثم إعادة تشغيل المثبت.                                         | تحديث openclaw&#xA;حالة تحديث openclaw&#xA;تحديث openclaw --القناة المستقرة<unk> beta<unk> dev&#xA;تحديث openclaw --tag <dist-tag|version>&#xA;تحديث openclaw - عدم إعادة تشغيل |
| ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `$OPENCLAW_STATE_DIR/openclaw.json`                                                                          | يمكن أن يساعد OpenClaw في إعلانات ومدوّنات اتصال بالمحفوظات الرائدة لساي                                                                                                                                  |
| ما هي استراتيجية النسخ الاحتياطي الموصى بها                                                                  | ملاحظات:                                                                                                                                                                                  |
| إذا كنت لا تزال ترغب في تجربة Bun، قم بذلك على بوابة غير إنتاجية&#xA;بدون WhatsApp/Telegram. | إذا كنت تريد فقط **أنت** لتتمكن من تشغيل ردود المجموعة:                                                                                                                                   |
| الخطوات الدنيا:                                                                              | تذكير ومتابعة:\* \* كرون أو ضربة قلب مدفعية وقوائم مرجعية.                                                                                                                |
| `$OPENCLAW_STATE_DIR/credentials/`                                                                           | البوابات والعقد البعيدة                                                                                                                                                                                   |
| `$OPENCLAW_STATE_DIR/agents/`                                                                                | لماذا أرى النموذج غير مسموح به ومن ثم لا يوجد رد                                                                                                                                                          |
| لديك ثلاثة أنماط مدعومة:                                                                     | 'gateway.remote.token' فقط لـ **مكالمات CLI البعيدة**؛ إنها لا تمكن مصادقة البوابة المحلية.                                                               |
| عادة ما تبدو الفوز اليومي:                                                                   | يتم وضع حد أقصى للطلبات المعلقة في **3 في كل قناة**؛ تحقق من `قائمة الاقتران openclaw <channel>` إذا لم تصل التعليمات البرمجية.                                                           |
...
هل البوابة صحية؟ `openclaw status`
...
- تاريخ المحادثة والحالة (لكل وكيل)
...
**تدوير ملف التعريف المصادقة** داخل نفس المورد.
...
1. **طلب LM رفض رسالة تفكر في التوقيع يتطلب جوجل مضاد للجاذبية**
2. لم يتم العثور على بيانات اعتماد للملف الشخصي "انساني:الافتراضي"
3. كيف يمكنني استخدام نماذج مختلفة لمهام مختلفة
4. ما هو وقت التشغيل الذي أحتاجه
5. **المزيد من أدوات الجهاز.** العقدة تعرض 'canvas' و 'camera' و 'الشاشة' بالإضافة إلى 'system.run'.
...
### البيانات الوصفية للجلسة (لكل وكيل)
...
- تأكد من تكوين MiniMax (معالج أو JSON)، أو أن مفتاح API صغير ماكس
...
1. المرسلون غير المعروفين يتلقون رمز الاقتران؛ البوت لا يعالج رسالتهم.
...
2. إذا كنت تستخدم CLI أو TUI ، يجب أن يبدو عنوان URL كالتالي:
   - الإصلاحات السريعة:

3. اطلب من الروبوت أن يلخص الحالة الراهنة ويكتبها إلى ملف.
   - الموافقة على: `فقرات openclaw فقرات <channel> <code>`

4. الخيار 1 (الأسرع): سجلات الذيل وإرسال رسالة اختبار في المجموعة:
...
1. استخدام أمر إعادة الضبط:
2. **استخدم تطبيق macOS في الوضع البعيد** (يمكن أن يكون هدف SH اسم المضيف الذيل ).
   سوف ينفق التطبيق منفذ البوابة ويتصل كعقدة.
3. إذا لم يكن لديك نسخة احتياطية، قم بإعادة تشغيل `openclaw doctor` وإعادة تكوين القنوات/النماذج.
...
استخدم معرف النموذج الدقيق (حساس لحالة الحالة): `minimax/MiniMax-M2.1` أو
...
1. أما اليوم فالأنماط المدعومة هي:
2. أو نسخ 'Auth-profiles.json' من 'agentDir' للوكيل الرئيسي إلى 'agentDir' للوكيل الجديد.
...
1. واجهة مستخدم التحكم تقول غير مصرح بها أو تواصل إعادة الاتصال الآن

   ```
   COPILOT_GITHUB_TOKEN=...
   ```

2. هي اختصارات مبنية على شبكة opus sonnet

3. المشاريع الأولى الجيدة:
...
## **الانتقال الاحتياطي للنموذج** إلى النموذج التالي في `agents.defaults.model.fallbacks`.
...
قم بفتح تطبيق macOS محلياً ثم قم بتوصيل وضع **عن بعد عبر SSH** (أو تخصيص مباشرة)
...
```bash
إذا كان جهاز التحكم، هل الرابط بين النفق/نطاق الخط؟
```
...
أو أضفها إلى بلوكة التكوين "env" (ينطبق فقط إذا كان مفقوداً).
...
- استخدم عنوان WS URL: `ws://<host>:18789` (أو `wss://...` if HTTPS).
...
1. `البشرية:<email>` لهويات OAuth
2. لماذا حالة بوابة openclaw تقول تشغيل Runtime ولكن مسبار RPC فشل
3. 'توجيه' - رسائل جديدة تعيد توجيه المهمة الحالية
4. Run:
...
```json5
ذاكرة التخزين المؤقت للمصادقة وقت التشغيل (تتم إدارتها تلقائياً)
```
...
1. ما هو الحد الأدنى من التكوين العقلاني لتثبيت أول
2. إذا كنت تبعد ، قم بتأكيد اتصال النفق/النطاق المستقيم و أنه يمكن الوصول إلى
   بوابة WebSocket
...
**تأكد من أن VPS + Mac على نفس الخيل**.
...
ضع المفاتيح المفقودة في '~/.openclaw/.env' لذلك يتم التقاطها حتى عندما لا ترث الخدمة قذيفتك.
...
استيراد OAuth القديم (تم نسخه إلى ملفات تعريف المصادقة عند الاستخدام الأول)

- المصادقة الشخصية (OAuth + API مفاتيح)
...
- مازالت عالقة؟ مازالت عالقة؟ تشغيل 'حالة openclaw - all' واتبع [Troubleshooting](/gateway/troubleshooting). انظر [Dashboard](/web/dashboard) للحصول على تفاصيل المصادقة.
...
- حالة المزود (على سبيل المثال `Whatsapp/<accountId>/creds.json`)
...
1. حالة openclaw
   وضع نماذج openclaw
   سجلات openclaw --تابع
2. الوثائق: [Cron jobs](/automation/cron-jobs), [Cron vs Heartbeat](/automation/cron-vs-heartbeat),
   [Heartbeat](/gateway/heartbeat).
3. **2) ويندوز الأصلي (غير مستحسن):** تشغيل البوابة مباشرة في ويندوز.
...
1. هل تعمل البوابة؟ يتم تشغيل البوابة: \\`openclaw status'
2. هل البوابة صحية؟ `openclaw status`
3. هل لدى واجهة المستخدم الرمز الصحيح؟ `openclaw dashboard`
4. معالجة الإجراءات:kill sessionId:XXX
...
لا تفتح منفذ WS في علامة تبويب المتصفح العادية.
...
- npm تثبيت -g openclaw@latest
  openclaw doctor
  إعادة تشغيل بوابة openclaw
- هل لدى واجهة المستخدم الرمز الصحيح؟ `openclaw dashboard`
...
```
هل تعمل البوابة؟ `openclaw gateway status`
```
