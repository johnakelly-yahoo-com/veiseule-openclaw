...
- `MEMORY.md` و/أو `memory.md` (عند وجودهما في مساحة العمل؛ يمكن إدراج أحدهما أو كليهما)

يتم **إدراج جميع هذه الملفات في نافذة السياق** في كل دورة، مما
يعني أنها تستهلك الرموز. حافظ على اختصارها — خاصةً `MEMORY.md`، الذي يمكن أن
ينمو بمرور الوقت ويؤدي إلى استخدام مرتفع غير متوقع للسياق وزيادة تكرار
الضغط (compaction).

> **ملاحظة:** ملفات `memory/*.md` اليومية **لا** يتم إدراجها تلقائيًا. يتم
> الوصول إليها عند الطلب عبر أداتي `memory_search` و`memory_get`، لذلك
> لا تُحتسب ضمن نافذة السياق ما لم يقم النموذج بقراءتها صراحةً.

تُختصر الملفات الكبيرة مع وسم دال. يُتحكَّم في الحد الأقصى لحجم كل ملف بواسطة
`agents.defaults.bootstrapMaxChars` (الافتراضي: 20000). إجمالي محتوى التهيئة المدرج
عبر الملفات محدود بواسطة `agents.defaults.bootstrapTotalMaxChars`
(القيمة الافتراضية: 24000). تؤدي الملفات المفقودة إلى حقن
وسم قصير لملف مفقود.

تقوم جلسات الوكيل الفرعي بإدراج `AGENTS.md` و`TOOLS.md` فقط (يتم
استبعاد ملفات التهيئة الأخرى للحفاظ على صِغر سياق الوكيل الفرعي).
