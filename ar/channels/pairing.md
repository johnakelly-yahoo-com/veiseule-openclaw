...
إذا كنت تستخدم إضافة `device-pair`، يمكنك تنفيذ اقتران الجهاز لأول مرة بالكامل من خلال Telegram:

1. في Telegram، أرسل رسالة إلى البوت الخاص بك: `/pair`
2. يرد البوت برسالتين: رسالة تعليمات ورسالة منفصلة تحتوي على **رمز الإعداد** (سهل النسخ/اللصق في Telegram).
3. على هاتفك، افتح تطبيق OpenClaw على iOS → الإعدادات → Gateway.
4. الصق رمز الإعداد واتصل.
5. العودة إلى Telegram: `/pair approve`

رمز الإعداد هو حمولة JSON مُشفّرة بصيغة base64 وتحتوي على:

- `url`: عنوان WebSocket الخاص بالبوابة (`ws://...` أو `wss://...`)
- `token`: رمز اقتران قصير العمر

تعامل مع رمز الإعداد ككلمة مرور طوال فترة صلاحيته.
