...
تكامل Ollama في OpenClaw يستخدم **واجهة Ollama API الأصلية** (`/api/chat`) بشكل افتراضي، والتي تدعم بالكامل البث واستدعاء الأدوات في الوقت نفسه. لا يلزم أي إعداد خاص.

#### وضع OpenAI-Compatible القديم

إذا كنت بحاجة إلى استخدام نقطة النهاية المتوافقة مع OpenAI بدلاً من ذلك (على سبيل المثال، خلف وكيل يدعم فقط تنسيق OpenAI)، فقم بتعيين `api: "openai-completions"` بشكل صريح:
...
ملاحظة: قد لا تدعم نقطة النهاية المتوافقة مع OpenAI البث + استدعاء الأدوات في الوقت نفسه. قد تحتاج إلى تعطيل البث باستخدام `params: { streaming: false }` في إعدادات النموذج.
