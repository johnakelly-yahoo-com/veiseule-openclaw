...
سجلات Bonjour/mDNS من نوع TXT هي **غير موثّقة**.

- يجب على العملاء التعامل مع قيم TXT كإرشادات لواجهة المستخدم فقط. يجب أن يفضّل التوجيه (المضيف/المنفذ) **نقطة نهاية الخدمة التي تم حلّها** (SRV + A/AAAA) بدلاً من القيم المقدمة عبر TXT مثل `lanHost` أو `tailnetDns` أو `gatewayPort`.
- يجب ألا يسمح تثبيت TLS مطلقًا لقيمة `gatewayTlsSha256` المُعلَن عنها بتجاوز بصمة مثبتة مسبقًا.
- يجب على عقد iOS/Android اعتبار الاتصالات المباشرة المعتمدة على الاكتشاف على أنها **TLS فقط** واشتراط تأكيد صريح «الوثوق بهذه البصمة» قبل تخزين بصمة لأول مرة (تحقق خارج النطاق).
