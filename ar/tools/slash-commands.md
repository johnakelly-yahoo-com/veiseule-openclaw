...
- **التوجيهات**: `/think`، `/verbose`، `/reasoning`، `/elevated`، `/exec`، `/model`، `/queue`.
  ...
  - يتم تطبيق التوجيهات فقط على **المرسلين المصرح لهم**. لا تُطبَّق التوجيهات إلا على **المرسلين المصرّح لهم** (قوائم السماح/الاقتران للقنوات إضافةً إلى `commands.useAccessGroups`).
...
- `commands.allowFrom` (اختياري) يحدد قائمة سماح لكل مزود لتفويض الأوامر. عند تهيئته، يكون هو
  مصدر التفويض الوحيد للأوامر والتوجيهات (يتم تجاهل قوائم سماح القنوات/الاقتران و `commands.useAccessGroups`). استخدم `"*"` كقيمة افتراضية عامة؛ وتتجاوز المفاتيح الخاصة بكل مزود هذه القيمة.
...
- `/kill <id|#|all>` (إيقاف فوري لوكيل فرعي واحد أو لجميع الوكلاء الفرعيين قيد التشغيل لهذه الجلسة؛ بدون رسالة تأكيد)
- `/steer <id|#> <message>` (توجيه وكيل فرعي قيد التشغيل فورًا: أثناء التشغيل إن أمكن، وإلا يتم إيقاف العمل الحالي وإعادة التشغيل باستخدام رسالة التوجيه)
