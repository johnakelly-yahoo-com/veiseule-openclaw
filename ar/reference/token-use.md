...
- مساحة العمل + ملفات الإقلاع (`AGENTS.md`، `SOUL.md`، `TOOLS.md`، `IDENTITY.md`، `USER.md`، `HEARTBEAT.md`، `BOOTSTRAP.md` عند الإضافة الجديدة). تُقتطع الملفات الكبيرة بواسطة `agents.defaults.bootstrapMaxChars` (الافتراضي: 20000). ملفات `memory/*.md` تُحمَّل عند الطلب عبر أدوات الذاكرة ولا يتم حقنها تلقائيًا.
