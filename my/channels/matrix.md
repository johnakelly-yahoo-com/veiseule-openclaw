...
Multi-account အထောက်အပံ့: account တစ်ခုချင်းစီအလိုက် credentials နှင့် ရွေးချယ်နိုင်သော `name` ပါဝင်သည့် `channels.matrix.accounts` ကို အသုံးပြုပါ။ မျှဝေထားသော pattern အတွက် [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) ကို ကြည့်ပါ။

Account တစ်ခုချင်းစီသည် မည်သည့် homeserver မဆိုပေါ်တွင် သီးခြား Matrix user အဖြစ် လည်ပတ်ပါသည်။ Account တစ်ခုချင်းစီ၏ config သည်
အပေါ်ဆုံးအဆင့် `channels.matrix` settings မှ ဆက်ခံပြီး option မည်သည့်တစ်ခုမဆို override လုပ်နိုင်ပါသည်
(DM policy, groups, encryption စသည်)။
...
- Env variables (`MATRIX_HOMESERVER`, `MATRIX_ACCESS_TOKEN`, စသည်) **default** account အတွက်သာ သက်ဆိုင်ပါသည်။
- Base channel settings (DM policy, group policy, mention gating စသည်) Account တစ်ခုချင်းစီအလိုက် override မလုပ်ထားပါက account အားလုံးအပေါ် သက်ရောက်ပါသည်။
- `bindings[].match.accountId` ကို အသုံးပြု၍ account တစ်ခုချင်းစီကို မတူညီသော agent သို့ လမ်းကြောင်းခွဲနိုင်ပါသည်။
- Crypto state ကို account တစ်ခုချင်းစီ + access token အလိုက် သိမ်းဆည်းထားသည် (account တစ်ခုချင်းစီအတွက် သီးခြား key stores များ)။
...
- Display names သို့မဟုတ် bare localparts (ဥပမာ: `"Alice"` သို့မဟုတ် `"alice"`) ကို မသုံးပါနှင့်။ ၎င်းတို့သည် မရှင်းလင်းသဖြင့် allowlist matching အတွက် လျစ်လျူရှုခံရပါသည်။ Full `@user:server` IDs ကို အသုံးပြုပါ။
