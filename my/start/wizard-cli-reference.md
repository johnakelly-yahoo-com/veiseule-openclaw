...
<Steps>
  ...
  <Step title="Channels">
    - [WhatsApp](/channels/whatsapp): ရွေးချယ်နိုင်သော QR လော့ဂ်အင်
    - [Telegram](/channels/telegram): bot token
    - [Discord](/channels/discord): bot token
    - [Google Chat](/channels/googlechat): service account JSON + webhook audience
    - [Mattermost](/channels/mattermost) plugin: bot token + base URL
    - [Signal](/channels/signal): ရွေးချယ်နိုင်သော `signal-cli` ထည့်သွင်းမှု + အကောင့် ပြင်ဆင်သတ်မှတ်ခြင်း
    - [BlueBubbles](/channels/bluebubbles): iMessage အတွက် အကြံပြုသည်; server URL + password + webhook
    - [iMessage](/channels/imessage): legacy `imsg` CLI လမ်းကြောင်း + DB အသုံးပြုခွင့်
    - DM security: default သည် pairing ဖြစ်သည်။ ပထမဆုံး DM တွင် code တစ်ခု ပို့မည်ဖြစ်ပြီး၊ အောက်ပါအတိုင်း approve လုပ်ပါ  
      `openclaw pairing approve <channel> <code>` သို့မဟုတ် allowlist များကို အသုံးပြုပါ။
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider">
    OpenAI-compatible နှင့် Anthropic-compatible endpoint များနှင့် အလုပ်လုပ်နိုင်သည်။

    Non-interactive flags:
    - `--auth-choice custom-api-key`
    - `--custom-base-url`
    - `--custom-model-id`
    - `--custom-api-key` (optional; `CUSTOM_API_KEY` ကို fallback အဖြစ် အသုံးပြုသည်)
    - `--custom-provider-id` (optional)
    - `--custom-compatibility <openai|anthropic>` (optional; default သည် `openai` ဖြစ်သည်)
