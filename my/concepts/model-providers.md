...
## ပါဝင်ပြီးသား ပံ့ပိုးသူများ (pi-ai catalog)
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- ဥပမာ မော်ဒယ်: `openai/gpt-5.1-codex`
...
- Hugging Face Inference: `huggingface` (`HUGGINGFACE_HUB_TOKEN` သို့မဟုတ် `HF_TOKEN`) — OpenAI-compatible router; ဥပမာ model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `openclaw onboard --auth-choice huggingface-api-key`။ [Hugging Face (Inference)](/providers/huggingface) ကို ကြည့်ပါ။
...
- Provider: `vllm`
- Auth: မဖြစ်မနေမလိုအပ်ပါ (သင့် server ပေါ် မူတည်သည်)
- ပုံမှန် base URL: `http://127.0.0.1:8000/v1`

Local တွင် auto-discovery ကို အသုံးပြုလိုပါက (သင့် server သည် auth မစစ်ဆေးပါက မည်သည့်တန်ဖိုးမဆို အသုံးပြုနိုင်သည်):

```bash
export VLLM_API_KEY="vllm-local"
```

ထို့နောက် model တစ်ခု သတ်မှတ်ပါ (`/v1/models` မှ ပြန်လည်ရရှိသော ID များထဲမှ တစ်ခုဖြင့် အစားထိုးပါ):
