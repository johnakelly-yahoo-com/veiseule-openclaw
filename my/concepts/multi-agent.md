...
7. ချန်နယ်အဆင့် ကိုက်ညီမှု (`accountId: "*"`)
8. default agent သို့ fallback (`agents.list[].default`၊ မရှိပါက စာရင်းထဲ ပထမဆုံး entry၊ default: `main`)

binding တစ်ခုတွင် match field များစွာ (ဥပမာ `peer` + `guildId`) သတ်မှတ်ထားပါက၊ သတ်မှတ်ထားသော field အားလုံးကို လိုအပ်ပါသည် (`AND` semantics)။
