...
ယနေ့တွင် channel messages များသို့ **အမှန်တကယ် token-delta streaming** မရှိသေးပါ။ Telegram preview streaming သည် တစ်စိတ်တစ်ပိုင်း-stream surface တစ်ခုသာ ဖြစ်ပါသည်။
...
**Channel မှတ်စု:** Telegram မဟုတ်သော ချန်နယ်များအတွက် `*.blockStreaming` ကို `true` ဟု တိတိကျကျ မသတ်မှတ်ထားပါက block streaming ကို **ပိတ်ထားသည်**။ Telegram သည် block replies မလိုအပ်ဘဲ live preview ကို stream ပြုလုပ်နိုင်သည် (`channels.telegram.streamMode`)။
...
- စာသားမဟုတ်သော/ရှုပ်ထွေးသော final များအတွက် ပုံမှန် final message ပေးပို့မှုသို့ fallback ပြုလုပ်ပါသည်။
- `/reasoning stream` သည် reasoning ကို live preview ထဲသို့ ရေးသွင်းပေးသည် (Telegram သာ)။

```
Telegram
  └─ sendMessage (ယာယီ preview message)
       ├─ streamMode=partial → နောက်ဆုံး text ကို edit ပြုလုပ်ခြင်း
       └─ streamMode=block   → chunker + edit updates
  └─ final text-only reply → တူညီသော message ပေါ်တွင် final edit ပြုလုပ်ခြင်း
  └─ fallback: preview ကို cleanup ပြုလုပ်ပြီး ပုံမှန် final delivery (media/complex)
```

Legend:

- `preview message`: ထုတ်လုပ်နေစဉ်အတွင်း update ပြုလုပ်ထားသော ယာယီ Telegram message။
- `final edit`: တူညီသော preview message ပေါ်တွင် တိုက်ရိုက် ပြင်ဆင်ခြင်း (text-only)။
