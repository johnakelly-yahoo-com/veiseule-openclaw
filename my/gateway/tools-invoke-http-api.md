...
- `gateway.auth.rateLimit` ကို configure လုပ်ထားပြီး auth မအောင်မြင်မှုများ အလွန်များလာပါက endpoint သည် `Retry-After` နှင့်အတူ `429` ကို ပြန်လည်ပေးပို့မည်ဖြစ်သည်။
...
`gateway.tools` မှတစ်ဆင့် ဤ deny list ကို စိတ်ကြိုက်ပြင်ဆင်နိုင်သည်:
...
- `200` → `{ ok: true, result }`
- `400` → `{ ok: false, error: { type, message } }` (မမှန်ကန်သော request သို့မဟုတ် tool input အမှား)
- `401` → အထောက်အထား မရှိ
- `429` → auth rate-limited (`Retry-After` သတ်မှတ်ထားသည်)
- `404` → ကိရိယာ မရရှိနိုင် (မတွေ့ရှိခြင်း သို့မဟုတ် allowlist တွင် မပါဝင်ခြင်း)
- `405` → ခွင့်မပြုသော method
- `500` → `{ ok: false, error: { type, message } }` (မျှော်လင့်မထားသော tool execution အမှား; message ကို sanitized လုပ်ထားသည်)
