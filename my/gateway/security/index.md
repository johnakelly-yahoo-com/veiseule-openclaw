...
- Canvas host: `/__openclaw__/canvas/` နှင့် `/__openclaw__/a2ui/` (မည်သည့် HTML/JS မဆို ဖြစ်နိုင်သည်; ယုံကြည်စိတ်ချရမှုမရှိသော content အဖြစ် ဆက်ဆံပါ)
...
- secret အသစ် ထုတ်လုပ်/သတ်မှတ်ပါ (`gateway.auth.token` သို့မဟုတ် `OPENCLAW_GATEWAY_PASSWORD`)။
- Gateway ကို restart လုပ်ပါ (သို့မဟုတ် macOS app က Gateway ကို ကြီးကြပ်နေပါက app ကို restart လုပ်ပါ)။
...
- `gateway.auth.mode: "trusted-proxy"`: အသုံးပြုသူများကို authenticate ပြုလုပ်ပြီး identity ကို header များမှတဆင့် ပို့ပေးသည့် identity-aware reverse proxy ကို ယုံကြည်အသုံးပြုပါ ( [Trusted Proxy Auth](/gateway/trusted-proxy-auth) ကို ကြည့်ပါ )။
...
ထပ်ဆောင်း hardening ရွေးချယ်စရာများ:

- `tools.exec.applyPatch.workspaceOnly: true` (မူလသတ်မှတ်ချက်): sandboxing ကို ပိတ်ထားသော်လည်း `apply_patch` သည် workspace directory အပြင်ဘက်တွင် write/delete မလုပ်နိုင်ကြောင်း သေချာစေသည်။ သင်သည် `apply_patch` ကို workspace အပြင်ဘက်ရှိ ဖိုင်များကို ထိတွေ့စေလိုသည့် အထူးရည်ရွယ်ချက်ရှိသောအခါမှသာ `false` သို့ သတ်မှတ်ပါ။
- `tools.fs.workspaceOnly: true` (ရွေးချယ်နိုင်သည်): `read`/`write`/`edit`/`apply_patch` လမ်းကြောင်းများကို workspace directory အတွင်းသာ ကန့်သတ်ထားသည် (ယခု absolute paths များကို ခွင့်ပြုထားပြီး guardrail တစ်ခုတည်းဖြင့် ထိန်းချုပ်လိုပါက အသုံးဝင်သည်)။
...
1. အချိန်အမှတ်အသား၊ gateway host OS + OpenClaw version
2. session transcript(s) + log tail အတို (redacting ပြုလုပ်ပြီးနောက်)
3. တိုက်ခိုက်သူ ပို့ခဲ့သည့် အရာ + agent လုပ်ခဲ့သည့် အရာ
