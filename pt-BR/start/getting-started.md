...
Se você executa o OpenClaw como uma conta de serviço ou deseja locais personalizados para config/estado:

- `OPENCLAW_HOME` define o diretório inicial usado para a resolução interna de caminhos.
- `OPENCLAW_STATE_DIR` substitui o diretório de estado.
- `OPENCLAW_CONFIG_PATH` substitui o caminho do arquivo de configuração.

Referência completa das variáveis de ambiente: [Variáveis de ambiente](/help/environment).
