...
- **ควรใช้ตัวแปรสภาพแวดล้อม (env vars) แทนไฟล์คอนฟิก** สำหรับ API keys และโทเค็นทั้งหมด วิธีนี้จะช่วยป้องกันไม่ให้ข้อมูลลับอยู่ใน `openclaw.json` ซึ่งอาจถูกเปิดเผยหรือบันทึกล็อกโดยไม่ตั้งใจ
...
หากใช้ตัวแปรสภาพแวดล้อม (env var) ไม่จำเป็นต้องเพิ่มโทเค็นลงในคอนฟิก Gateway จะอ่านค่า `DISCORD_BOT_TOKEN` โดยอัตโนมัติ
...
หากคุณต้องการ webhook callbacks (Twilio, Telnyx เป็นต้น) โดยไม่ต้องเปิดเผยสู่สาธารณะ:
