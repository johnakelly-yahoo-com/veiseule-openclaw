...
- ข้อมูลรับรองของช่องทาง: โทเค็นบอท Discord, โทเค็น Telegram เป็นต้น
...
**เคล็ดลับ:** เลือกภูมิภาคที่ใกล้คุณ ตัวเลือกที่พบบ่อย: `lhr`(ลอนดอน), `iad`(เวอร์จิเนีย), `sjc`(ซานโฮเซ) **เคล็ดลับ:** เลือกภูมิภาคที่ใกล้คุณ ตัวเลือกที่พบบ่อย: `lhr`(ลอนดอน), `iad`(เวอร์จิเนีย), `sjc`(ซานโฮเซ) Common options: `lhr` (London), `iad` (Virginia), `sjc` (San Jose).
...
การดีพลอยครั้งแรกจะสร้างอิมเมจDocker(~2-3นาที) การดีพลอยครั้งถัดไปจะเร็วขึ้น Subsequent deploys are faster. Subsequent deploys are faster.
...
If using env var, no need to add token to config. หากใช้ตัวแปรสภาพแวดล้อม (env var) ไม่จำเป็นต้องเพิ่มโทเค็นลงในคอนฟิก Gateway จะอ่านค่า `DISCORD_BOT_TOKEN` โดยอัตโนมัติ
...
**หมายเหตุ:** 512MBเล็กเกินไป 1GBอาจใช้ได้แต่มีโอกาสOOMเมื่อมีโหลดหรือบันทึกแบบละเอียด **แนะนำ2GB** **หมายเหตุ:** 512MBเล็กเกินไป 1GBอาจใช้ได้แต่มีโอกาสOOMเมื่อมีโหลดหรือบันทึกแบบละเอียด **แนะนำ2GB** 1GB may work but can OOM under load or with verbose logging. **2GB is recommended.**
...
คำสั่ง`fly ssh console -C`ไม่รองรับการเปลี่ยนเส้นทางเชลล์ เพื่อเขียนไฟล์คอนฟิก: To write a config file: To write a config file:
...
**หมายเหตุ:** `fly sftp`อาจล้มเหลวหากไฟล์มีอยู่แล้ว ให้ลบก่อน: ลบก่อน: ลบก่อน:
...
**หมายเหตุ:** หลังจาก`fly deploy`คำสั่งของเครื่องอาจรีเซ็ตกลับเป็นค่าที่อยู่ใน`fly.toml` หากคุณแก้ไขด้วยตนเอง ให้ปรับใช้ซ้ำหลังดีพลอย If you made manual changes, re-apply them after deploy. If you made manual changes, re-apply them after deploy.

## การดีพลอยแบบส่วนตัว(Hardened)

โดยค่าเริ่มต้น FlyจะจัดสรรIPสาธารณะ ทำให้Gatewayของคุณเข้าถึงได้ที่`https://your-app.fly.dev` ซึ่งสะดวกแต่ทำให้การดีพลอยของคุณถูกค้นพบโดยสแกนเนอร์อินเทอร์เน็ต(Shodan, Censys ฯลฯ) โดยค่าเริ่มต้น FlyจะจัดสรรIPสาธารณะ ทำให้Gatewayของคุณเข้าถึงได้ที่`https://your-app.fly.dev` ซึ่งสะดวกแต่ทำให้การดีพลอยของคุณถูกค้นพบโดยสแกนเนอร์อินเทอร์เน็ต(Shodan, Censys ฯลฯ) This is convenient but means your deployment is discoverable by internet scanners (Shodan, Censys, etc.).
...
หากคุณต้องการ webhook callbacks (Twilio, Telnyx เป็นต้น) โดยไม่ต้องเปิดเผยสู่สาธารณะ: without public exposure:
