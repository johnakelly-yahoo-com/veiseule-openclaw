...
- **หมายเหตุเรื่อง sandboxing**: agent sandboxing ก็ใช้ Docker เช่นกัน แต่**ไม่จำเป็น**ต้องรัน Gateway ทั้งหมดใน Docker ดู [Sandboxing](/gateway/sandboxing) See [Sandboxing](/gateway/sandboxing). See [Sandboxing](/gateway/sandboxing).
...
**เพิ่มไปยังการตั้งค่า shell ของคุณ (zsh):**

```bash
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

จากนั้นใช้ `clawdock-start`, `clawdock-stop`, `clawdock-dashboard` เป็นต้น รัน `clawdock-help` เพื่อดูคำสั่งทั้งหมด

ดูรายละเอียดเพิ่มเติมได้ที่ [`ClawDock` Helper README](https://github.com/openclaw/openclaw/blob/main/scripts/shell-helpers/README.md)
...
`openclaw-gateway` and `openclaw-cli` by generating `docker-compose.extra.yml`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
...
- `docker-compose.extra.yml` ถูกสร้างอัตโนมัติ อย่าแก้ไขด้วยมือ Don’t hand-edit it. Don’t hand-edit it.
...
`node` ซึ่งช่วยลดพื้นที่โจมตี แต่หมายความว่า: This keeps the attack surface small, but it means: This keeps the attack surface small, but it means:
...
- Dockerfile CMD ใช้ `--allow-unconfigured`; คอนฟิกที่เมานต์ด้วย `gateway.mode` ไม่ใช่ `local` จะยังเริ่มทำงานได้ ให้ override CMD หากต้องการบังคับใช้ guard Override CMD to enforce the guard. Override CMD to enforce the guard.
...
คอนเทนเนอร์ Docker Gateway ยังคงอยู่บนโฮสต์ แต่การรันเครื่องมือจะถูกแยกออก: The gateway stays on your host, but the tool execution is isolated: The gateway stays on your host, but the tool execution is isolated:
...
คำเตือน: `scope: "shared"` จะปิดการแยกระหว่างเซสชัน ทุกเซสชันใช้คอนเทนเนอร์และเวิร์กสเปซเดียวกัน คำเตือน: `scope: "shared"` จะปิดการแยกระหว่างเซสชัน ทุกเซสชันใช้คอนเทนเนอร์และเวิร์กสเปซเดียวกัน All sessions share
...
จะสร้าง `openclaw-sandbox-common:bookworm-slim` วิธีใช้งาน: To use it: To use it:
