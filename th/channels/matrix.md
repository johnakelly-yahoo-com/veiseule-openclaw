...
โพล (ส่ง + poll-start เป็นข้อความ) ตำแหน่ง และ E2EE (พร้อมการรองรับคริปโต) สถานะ: รองรับผ่านปลั๊กอิน (@vector-im/matrix-bot-sdk) รองรับข้อความส่วนตัว ห้อง เธรด สื่อ รีแอ็กชัน
...
6. เริ่ม DM กับบอตหรือเชิญเข้าห้องจากไคลเอนต์ Matrix ใดก็ได้
   ...
   ดังนั้นให้ตั้งค่า `channels.matrix.encryption: true` และยืนยันอุปกรณ์ เริ่ม DM กับบอตหรือเชิญเข้าห้องจากไคลเอนต์ Matrix ใดก็ได้
   (Element, Beeper ฯลฯ; ดู [https://matrix.org/ecosystem/clients/](https://matrix.org/ecosystem/clients/)) Beeper ต้องใช้ E2EE
...
- ยืนยันอุปกรณ์ในไคลเอนต์ Matrix อื่น (Element ฯลฯ) เพื่อเปิดใช้การแชร์กุญแจ เพื่อเปิดใช้การแชร์คีย์ เพื่อเปิดใช้การแชร์คีย์
...
ได้รับการยืนยันใหม่สำหรับห้องที่เข้ารหัส สถานะการซิงก์จะอยู่ควบคู่กันใน `bot-storage.json`
หากโทเคนการเข้าถึง (อุปกรณ์) เปลี่ยน จะมีการสร้างสโตร์ใหม่ และบอทต้อง
...
รองรับหลายบัญชี: ใช้ `channels.matrix.accounts` พร้อมข้อมูลรับรองแยกต่อบัญชี และ `name` (ไม่บังคับ) ดูรูปแบบที่ใช้ร่วมกันได้ที่ [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)

แต่ละบัญชีทำงานเป็นผู้ใช้ Matrix แยกกันบน homeserver ใดก็ได้ การกำหนดค่ารายบัญชี
สืบทอดจากการตั้งค่าระดับบนของ `channels.matrix` และสามารถแทนที่ตัวเลือกใด ๆ ได้
(นโยบาย DM, กลุ่ม, การเข้ารหัส เป็นต้น)
...
- การเริ่มต้นบัญชีจะทำแบบลำดับต่อเนื่องเพื่อหลีกเลี่ยง race condition จากการนำเข้าโมดูลพร้อมกัน
- ตัวแปรสภาพแวดล้อม (`MATRIX_HOMESERVER`, `MATRIX_ACCESS_TOKEN` เป็นต้น) ใช้กับบัญชี **default** เท่านั้น
- การตั้งค่าพื้นฐานของช่อง (นโยบาย DM, นโยบายกลุ่ม, mention gating เป็นต้น) มีผลกับทุกบัญชี เว้นแต่จะถูกแทนที่ในการตั้งค่ารายบัญชี
- ใช้ `bindings[].match.accountId` เพื่อกำหนดเส้นทางแต่ละบัญชีไปยังเอเจนต์ที่แตกต่างกัน
- สถานะ Crypto ถูกจัดเก็บแยกตามบัญชี + access token (key store แยกต่อบัญชี)
...
- ค่าเริ่มต้น: `channels.matrix.dm.policy = "pairing"` ผู้ส่งที่ไม่รู้จักจะได้รับรหัสจับคู่ ค่าเริ่มต้น: `channels.matrix.dm.policy = "pairing"` ผู้ส่งที่ไม่รู้จักจะได้รับรหัสจับคู่ ผู้ส่งที่ไม่รู้จักจะได้รับโค้ดจับคู่
- อนุมัติผ่าน:
...
- ห้ามใช้ชื่อที่แสดงผล (display name) หรือ localpart เดี่ยว ๆ (ตัวอย่าง: `"Alice"` หรือ `"alice"`) ค่าเหล่านี้กำกวมและจะถูกละเว้นสำหรับการจับคู่ allowlist ให้ใช้ ID แบบเต็มรูปแบบ `@user:server`

## ห้อง (กลุ่ม)

- ค่าเริ่มต้น: `channels.matrix.groupPolicy = "allowlist"` (ต้องกล่าวถึงจึงตอบ) ใช้ `channels.defaults.groupPolicy` เพื่อแทนค่าค่าเริ่มต้นเมื่อยังไม่ตั้งค่า ค่าเริ่มต้น: `channels.matrix.groupPolicy = "allowlist"` (ต้องกล่าวถึงจึงตอบ) ใช้ `channels.defaults.groupPolicy` เพื่อแทนค่าค่าเริ่มต้นเมื่อยังไม่ตั้งค่า ใช้ `channels.defaults.groupPolicy` เพื่อแทนที่ค่าเริ่มต้นเมื่อยังไม่ตั้งค่า
...
- `channels.matrix.accounts`: การกำหนดค่าหลายบัญชีโดยอ้างอิงตาม account ID (แต่ละบัญชีสืบทอดการตั้งค่าระดับบน)
