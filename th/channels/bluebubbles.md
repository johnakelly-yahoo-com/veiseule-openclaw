...
- 17. แนะนำ/ทดสอบแล้ว: macOS Sequoia (15) แนะนำ/ทดสอบแล้ว: macOS Sequoia (15) แนะนำ/ทดสอบแล้ว: macOS Sequoia (15) ใช้งานได้กับ macOS Tahoe (26) แต่การแก้ไขข้อความขณะนี้ไม่ทำงานบน Tahoe และการอัปเดตไอคอนกลุ่มอาจรายงานว่าสำเร็จแต่ไม่ซิงก์
...
- ตั้งรหัสผ่าน webhook เสมอ หากคุณเปิดเผย gateway ผ่าน reverse proxy (Tailscale Serve/Funnel, nginx, Cloudflare Tunnel, ngrok) proxy อาจเชื่อมต่อกับ gateway ผ่าน loopback ตัวจัดการ webhook ของ BlueBubbles จะมองว่าคำขอที่มี forwarding headers มาจาก proxy และจะไม่ยอมรับ webhook ที่ไม่มีรหัสผ่าน
...
- `channels.bluebubbles.mediaLocalRoots`: รายการ allowlist แบบระบุชัดเจนของไดเรกทอรีภายในเครื่อง (absolute path) ที่อนุญาตสำหรับเส้นทางสื่อภายในเครื่องขาออก การส่งไฟล์จากเส้นทางภายในเครื่องจะถูกปฏิเสธโดยค่าเริ่มต้น เว้นแต่จะมีการตั้งค่านี้ การตั้งค่าแทนที่รายบัญชี: `channels.bluebubbles.accounts.<accountId> .mediaLocalRoots`.
...
- OpenClaw จะซ่อนแอ็กชันที่ทราบว่าใช้งานไม่ได้โดยอัตโนมัติตามเวอร์ชัน macOS ของเซิร์ฟเวอร์ BlueBubbles OpenClaw จะซ่อนแอ็กชันที่ทราบว่ามีปัญหาโดยอัตโนมัติตามเวอร์ชัน macOS ของเซิร์ฟเวอร์ BlueBubbles หากยังเห็นการแก้ไขบน macOS 26 (Tahoe) ให้ปิดด้วยตนเองด้วย `channels.bluebubbles.actions.edit=false` OpenClaw จะซ่อนแอ็กชันที่ทราบว่ามีปัญหาโดยอัตโนมัติตามเวอร์ชัน macOS ของเซิร์ฟเวอร์ BlueBubbles หากยังเห็นการแก้ไขบน macOS 26 (Tahoe) ให้ปิดด้วยตนเองด้วย `channels.bluebubbles.actions.edit=false`
