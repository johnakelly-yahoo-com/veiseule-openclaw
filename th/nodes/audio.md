...
- ค่าเริ่มต้น `maxChars` สำหรับเสียงคือ **ไม่ตั้งค่า** (ถอดความเต็ม) ค่าเริ่มต้นของ `maxChars` สำหรับเสียงคือ **unset** (ทรานสคริปต์เต็ม) ตั้งค่า `tools.media.audio.maxChars` หรือ `maxChars` รายการต่อรายการเพื่อตัดเอาต์พุต ค่าเริ่มต้นของ `maxChars` สำหรับเสียงคือ **unset** (ทรานสคริปต์เต็ม) ตั้งค่า `tools.media.audio.maxChars` หรือ `maxChars` รายการต่อรายการเพื่อตัดเอาต์พุต
...
เมื่อกำหนด `requireMention: true` สำหรับแชทกลุ่ม OpenClaw จะถอดเสียงไฟล์เสียง **ก่อน** ตรวจสอบการกล่าวถึง ทำให้สามารถประมวลผลข้อความเสียงได้แม้ว่าจะมีการกล่าวถึงอยู่ภายใน

**วิธีการทำงาน:**

1. หากข้อความเสียงไม่มีข้อความตัวอักษรและกลุ่มต้องการการกล่าวถึง OpenClaw จะทำการถอดเสียงแบบ "preflight"
2. ข้อความถอดเสียงจะถูกตรวจสอบหารูปแบบการกล่าวถึง (เช่น `@BotName`, อีโมจิทริกเกอร์)
3. หากพบการกล่าวถึง ข้อความจะเข้าสู่กระบวนการตอบกลับเต็มรูปแบบ
4. ใช้ข้อความถอดเสียงเพื่อตรวจจับการกล่าวถึง เพื่อให้ข้อความเสียงผ่านเงื่อนไขการกล่าวถึงได้

**พฤติกรรมสำรอง:**

- หากการถอดเสียงล้มเหลวในขั้นตอน preflight (หมดเวลา, API error ฯลฯ) ข้อความจะถูกประมวลผลตามการตรวจจับการกล่าวถึงจากข้อความตัวอักษรเท่านั้น
- เพื่อให้แน่ใจว่าข้อความแบบผสม (ข้อความ + เสียง) จะไม่ถูกละทิ้งโดยไม่ตั้งใจ

**ตัวอย่าง:** ผู้ใช้ส่งข้อความเสียงว่า "Hey @Claude, what's the weather?" ในกลุ่ม Telegram ที่ตั้งค่า `requireMention: true` ข้อความเสียงถูกถอดเสียง ตรวจพบการกล่าวถึง และเอเจนต์ตอบกลับ
...
- การถอดเสียงแบบ preflight จะประมวลผลไฟล์เสียง **ไฟล์แรกเท่านั้น** เพื่อตรวจจับการกล่าวถึง ไฟล์เสียงเพิ่มเติมจะถูกประมวลผลในขั้นตอนทำความเข้าใจสื่อหลัก
