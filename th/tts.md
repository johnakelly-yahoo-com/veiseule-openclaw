...
Edge TTS **ไม่** จำเป็นต้องใช้ API key Edge TTS **ไม่** จำเป็นต้องใช้ API key Edge TTS **ไม่**ต้องใช้API key หากไม่พบAPI keyใดๆ OpenClawจะใช้Edge TTSเป็นค่าเริ่มต้น
...
ไม่ Auto‑TTS ถูกตั้งค่าเป็น **ปิด** โดยค่าเริ่มต้น ไม่ Auto‑TTS ถูกตั้งค่าเป็น **ปิด** โดยค่าเริ่มต้น ไม่ การแปลงเป็นเสียงอัตโนมัติ(Auto‑TTS) **ปิด**เป็นค่าเริ่มต้น เปิดใช้งานได้ในคอนฟิกด้วย
...
- `maxTextLength`: เพดานอินพุตTTSแบบตายตัว(จำนวนอักขระ) หากเกิน `/tts audio` จะล้มเหลว `maxTextLength`: เพดานอินพุตTTSแบบตายตัว(จำนวนอักขระ) หากเกิน `/tts audio` จะล้มเหลว `/tts audio` จะล้มเหลวหากเกินกำหนด
...
- **Edge TTS**: ใช้ `edge.outputFormat` (ค่าเริ่มต้น `audio-24khz-48kbitrate-mono-mp3`)
  - `node-edge-tts` รองรับ `outputFormat` แต่ไม่ใช่ทุกฟอร์แมตที่มีจากบริการEdge citeturn2search0 citeturn2search0 citeturn2search0
  - ค่ารูปแบบเอาต์พุตเป็นไปตามMicrosoft Speech output formats(รวมถึงOgg/WebM Opus) citeturn1search0 citeturn1search0 citeturn1search0
  - Telegram `sendVoice` รองรับOGG/MP3/M4A; หากต้องการโน้ตเสียงOpusที่รับประกัน
    ให้ใช้OpenAI/ElevenLabs citeturn1search1 citeturn1search1 citeturn1search1
...
`/voice` เป็นคำสั่งเนทีฟแทน ข้อความ `/tts ...` ยังใช้งานได้ ข้อความ `/tts ...` ยังใช้งานได้ ข้อความ `/tts ...` ยังใช้งานได้
...
เครื่องมือ `tts` แปลงข้อความเป็นเสียงและส่งกลับเป็นพาธ `MEDIA:` เครื่องมือ `tts` แปลงข้อความเป็นเสียงและส่งคืนพาธ `MEDIA:` เมื่อผลลัพธ์เข้ากันได้กับTelegram เครื่องมือจะใส่ `[[audio_as_voice]]` เพื่อให้Telegramส่งบับเบิลเสียง เครื่องมือ `tts` แปลงข้อความเป็นเสียงและส่งคืนพาธ `MEDIA:` เมื่อผลลัพธ์เข้ากันได้กับTelegram เครื่องมือจะใส่ `[[audio_as_voice]]` เพื่อให้Telegramส่งบับเบิลเสียง
