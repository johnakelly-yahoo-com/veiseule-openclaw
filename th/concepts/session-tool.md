...
37. `global` และ `unknown` เป็นค่าที่สงวนไว้และจะไม่ถูกแสดงรายการ `global` และ `unknown` เป็นค่าที่สงวนไว้และจะไม่ถูกแสดงรายการ `global` และ `unknown` เป็นค่าที่สงวนไว้และจะไม่ถูกแสดงรายการ หากเป็น `session.scope = "global"` เราจะทำการ alias เป็น `main` สำหรับทุกเครื่องมือ เพื่อให้ผู้เรียกไม่เห็น `global`
...
- ข้อความระหว่างเซสชันจะถูกบันทึกด้วย `message.provenance.kind = "inter_session"` เพื่อให้ผู้อ่านทรานสคริปต์สามารถแยกความแตกต่างระหว่างคำสั่งเอเจนต์ที่ถูกส่งต่อกับอินพุตจากผู้ใช้ภายนอกได้
