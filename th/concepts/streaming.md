...
- **Block streaming (ช่องทาง):** ส่ง **บล็อก** ที่เสร็จสมบูรณ์ออกมาเมื่อผู้ช่วยเขียนเสร็จ เป็นข้อความปกติของช่องทาง (ไม่ใช่เดลตาโทเคน) การสตรีมร่างของ Telegram เป็นพื้นผิวการสตรีมแบบบางส่วนเพียงอย่างเดียว นี่คือข้อความของช่องทางปกติ (ไม่ใช่ token deltas)
...
ปัจจุบัน **ยังไม่มีการสตรีมแบบ token-delta ที่แท้จริง** ไปยังข้อความในแชนเนล การสตรีมตัวอย่าง (preview) บน Telegram เป็นพื้นผิวที่รองรับการสตรีมบางส่วนเพียงจุดเดียว
...
ก่อนส่งออก เพื่อลด “สแปมบรรทัดเดี่ยว” ในขณะที่ยังให้เอาต์พุตแบบคืบหน้า สิ่งนี้ทำให้การตอบแบบหลายบับเบิลดูเป็นธรรมชาติมากขึ้น สิ่งนี้ทำให้การตอบแบบหลายบับเบิลดูเป็นธรรมชาติมากขึ้น
...
more natural. This makes multi-bubble responses feel
more natural.
...
- **สตรีมเป็นชิ้น:** `blockStreamingDefault: "on"` + `blockStreamingBreak: "text_end"` (ปล่อยไปตามที่สร้าง) ช่องทางที่ไม่ใช่ Telegram ต้องตั้ง `*.blockStreaming: true` เพิ่ม ช่องทางที่ไม่ใช่ Telegram ก็ต้องตั้งค่า `*.blockStreaming: true` ด้วย
...
(`channels.telegram.streamMode`) โดยไม่ต้องใช้การตอบแบบบล็อก Telegram สามารถสตรีมตัวอย่างแบบสดได้
(`channels.telegram.streamMode`) โดยไม่ต้องตอบกลับเป็นบล็อก
...
- ใช้ Bot API `sendMessage` (การอัปเดตครั้งแรก) + `editMessageText` (การอัปเดตครั้งถัดไป)
...
- การสตรีมตัวอย่างแยกจากการสตรีมแบบบล็อก
- เมื่อการสตรีมแบบร่างทำงาน OpenClaw จะปิด block streaming สำหรับการตอบนั้นเพื่อหลีกเลี่ยงการสตรีมซ้ำซ้อน
- ผลลัพธ์สุดท้ายที่เป็นข้อความล้วนจะถูกนำไปใช้โดยแก้ไขข้อความตัวอย่างเดิม
...
- `preview message`: ข้อความ Telegram ชั่วคราวที่ถูกอัปเดตระหว่างการสร้าง
- `final edit`: การแก้ไขแบบแทนที่ในข้อความตัวอย่างเดิม (เฉพาะข้อความล้วน)
