...
OpenClaw จะส่งกลับ: หากตั้งค่า `agents.defaults.models` จะกลายเป็น **allowlist** สำหรับ `/model` และสำหรับ
session overrides เมื่อผู้ใช้เลือกโมเดลที่ไม่อยู่ใน allowlist นั้น
...
“ไม่ตอบกลับ” วิธีแก้คืออย่างใดอย่างหนึ่ง: The fix is to either: The fix is to either:
...
- การอ้างอิงโมเดลจะถูกพาร์สโดยแยกที่ `/` **ครั้งแรก** ใช้ `provider/model` เมื่อพิมพ์ `/model <ref>` การอ้างอิงโมเดลจะถูกพาร์สโดยแยกที่ `/` **ครั้งแรก** ใช้ `provider/model` เมื่อพิมพ์ `/model <ref>` Use `provider/model` when typing `/model <ref>`.
...
`OPENROUTER_API_KEY`) หากไม่มีคีย์ ให้ใช้ `--no-probe` เพื่อแสดงเฉพาะผู้สมัคร Without a key, use `--no-probe` to list candidates only. Without a key, use `--no-probe` to list candidates only.
...
ให้ส่ง `--yes` เพื่อยอมรับค่าเริ่มต้น เมื่อรันใน TTY คุณสามารถเลือกฟอลแบ็กแบบโต้ตอบได้ ในโหมดไม่โต้ตอบ
