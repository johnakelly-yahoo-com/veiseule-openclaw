...
หากคุณมีทั้งโปรไฟล์OAuthและโปรไฟล์คีย์APIสำหรับผู้ให้บริการเดียวกัน รอบ‑โรบินอาจสลับไปมาระหว่างกันข้ามข้อความได้หากไม่ได้ยึดไว้ เพื่อบังคับใช้โปรไฟล์เดียว: To force a single profile: To force a single profile:
...
timeouts that exhausted profile rotation (other errors do not advance fallback). This applies to auth failures, rate limits, and
