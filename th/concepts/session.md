...
40. OpenClaw ถือว่า **หนึ่งเซสชันแชตโดยตรงต่อเอเจนต์** เป็นหลัก OpenClaw ถือว่า **หนึ่งเซสชันแชตตรงต่อหนึ่งเอเจนต์** เป็นหลัก แชตตรงจะถูกรวมเป็น `agent:<agentId>:<mainKey>` (ค่าเริ่มต้น `main`) ในขณะที่แชตกลุ่ม/ช่องทางจะมีคีย์ของตัวเอง และจะเคารพ `session.mainKey`. 41. `session.mainKey` จะถูกนำไปใช้
...
45. สถานะของเซสชันทั้งหมดเป็น **กรรมสิทธิ์ของ gateway** (OpenClaw “ตัวหลัก”) 46. ไคลเอนต์ UI (แอป macOS, WebChat ฯลฯ) 47. ต้องสอบถาม gateway เพื่อดูรายการเซสชันและจำนวนโทเค็น แทนการอ่านไฟล์ภายในเครื่อง
...
- สโตร์เป็นแมป `sessionKey -> { sessionId, updatedAt, ... 49. }`. 50. การลบรายการปลอดภัย; ระบบจะสร้างใหม่เมื่อมีการร้องขอ
