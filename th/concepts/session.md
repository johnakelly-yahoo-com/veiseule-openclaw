...
OpenClaw ถือว่า **หนึ่งเซสชันแชตโดยตรงต่อเอเจนต์** เป็นหลัก OpenClaw ถือว่า **หนึ่งเซสชันแชตตรงต่อหนึ่งเอเจนต์** เป็นหลัก แชตตรงจะถูกรวมเป็น `agent:<agentId>:<mainKey>` (ค่าเริ่มต้น `main`) ในขณะที่แชตกลุ่ม/ช่องทางจะมีคีย์ของตัวเอง และจะเคารพ `session.mainKey`. OpenClaw ถือว่า **หนึ่งเซสชันแชตตรงต่อหนึ่งเอเจนต์** เป็นหลัก แชตตรงจะถูกรวมเป็น `agent:<agentId>:<mainKey>` (ค่าเริ่มต้น `main`) ในขณะที่แชตกลุ่ม/ช่องทางจะมีคีย์ของตัวเอง และจะเคารพ `session.mainKey`. `session.mainKey` จะถูกนำไปใช้
...
- 42. `per-account-channel-peer`: แยกตามบัญชี + ช่องทาง + ผู้ส่ง (แนะนำสำหรับอินบ็อกซ์หลายบัญชี)
      `per-account-channel-peer`: แยกตามบัญชี + ช่องทาง + ผู้ส่ง (แนะนำสำหรับอินบ็อกซ์หลายบัญชี)
...
- ค่าเริ่มต้นคือ `dmScope: "main"` เพื่อความต่อเนื่อง (DM ทั้งหมดใช้เซสชันหลักร่วมกัน) เหมาะสำหรับการตั้งค่าผู้ใช้คนเดียว 44. สิ่งนี้เหมาะสำหรับการตั้งค่าแบบผู้ใช้เดียว
...
- จำนวนโทเคนที่แสดงใน UI มาจากฟิลด์ในสโตร์ของ Gateway (`inputTokens`, `outputTokens`, `totalTokens`, `contextTokens`) ไคลเอนต์จะไม่พาร์สทรานสคริปต์ JSONL เพื่อ “แก้ไข” ยอดรวม 48. ไคลเอนต์จะไม่แยกวิเคราะห์ทรานสคริปต์ JSONL เพื่อ “แก้ไข” ยอดรวม
...
- สโตร์เป็นแมป `sessionKey -> { sessionId, updatedAt, ... }`. การลบรายการปลอดภัย; ระบบจะสร้างใหม่เมื่อมีการร้องขอ
...
สิ่งนี้จะทำงานเฉพาะเมื่อ
workspace สามารถเขียนได้ การคอมแพคชันเป็นแบบ **คงอยู่ถาวร** (ต่างจากการตัดแต่งเซสชัน) ดู [/concepts/session-pruning](/concepts/session-pruning).
...
[Compaction](/concepts/compaction) ดู [Memory](/concepts/memory) และ
[Compaction](/concepts/compaction) รีเซ็ตเมื่อว่าง (ไม่บังคับ): `idleMinutes` จะเพิ่มหน้าต่างเวลาว่างแบบเลื่อน ดู [Memory](/concepts/memory) และ
[Compaction](/concepts/compaction) รีเซ็ตเมื่อว่าง (ไม่บังคับ): `idleMinutes` จะเพิ่มหน้าต่างเวลาว่างแบบเลื่อน
...
- ส่ง `/compact` (คำสั่งเสริมไม่บังคับ) เป็นข้อความเดี่ยวเพื่อสรุปบริบทเก่าและคืนพื้นที่หน้าต่าง ดู [/concepts/compaction](/concepts/compaction) หากตัวเชื่อมต่ออัปเดตเพียงการกำหนดเส้นทางการส่ง (เช่น เพื่อให้เซสชันหลักของ DM สดใหม่) ก็ควรส่งบริบทขาเข้าเพื่อให้เซสชันคงเมทาดาทาเชิงอธิบายไว้
...
- `threadId`: thread/topic id เมื่อช่องทางรองรับ
  ฟิลด์ต้นกำเนิดจะถูกเติมสำหรับข้อความตรง ช่องทาง และกลุ่ม หากคอนเน็กเตอร์อัปเดตเฉพาะการกำหนดเส้นทางการส่งมอบ (เช่น เพื่อทำให้เซสชันหลักของ DM สดใหม่) ก็ควรส่งบริบทขาเข้าเพื่อให้เซสชันคงเมทาดาทาอธิบายที่มาไว้ ส่วนขยายสามารถทำได้โดยส่ง `ConversationLabel`, `GroupSubject`, `GroupChannel`, `GroupSpace`, และ `SenderName` ในบริบทขาเข้า และเรียก `recordSessionMetaFromInbound` (หรือส่งบริบทเดียวกันไปยัง `updateLastRoute`). ส่ง `/compact` (คำสั่งเสริมไม่บังคับ) เป็นข้อความเดี่ยวเพื่อสรุปบริบทเก่าและคืนพื้นที่หน้าต่าง ดู [/concepts/compaction](/concepts/compaction) หากตัวเชื่อมต่ออัปเดตเพียงการกำหนดเส้นทางการส่ง (เช่น เพื่อให้เซสชันหลักของ DM สดใหม่) ก็ควรส่งบริบทขาเข้าเพื่อให้เซสชันคงเมทาดาทาเชิงอธิบายไว้ ส่วนขยายสามารถทำได้โดยส่ง `ConversationLabel`,
  `GroupSubject`, `GroupChannel`, `GroupSpace` และ `SenderName` ในบริบทขาเข้า และเรียก `recordSessionMetaFromInbound` (หรือส่งบริบทเดียวกันไปยัง `updateLastRoute`) Extensions can do this by sending `ConversationLabel`,
