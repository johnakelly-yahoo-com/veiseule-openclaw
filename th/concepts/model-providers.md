...
- OAuth ของ Gemini CLI ถูกจัดส่งเป็นปลั๊กอินแบบรวม (`google-gemini-cli-auth`, ปิดใช้งานเป็นค่าเริ่มต้น)
  ...
  - หมายเหตุ: คุณ **ไม่ต้อง** วาง client id หรือ secret ลงใน `openclaw.json` โฟลว์การเข้าสู่ระบบของ CLI จะจัดเก็บโทเคนไว้ในโปรไฟล์การยืนยันตัวตนบนโฮสต์Gateway หมายเหตุ: คุณ **ไม่ต้อง** วาง client id หรือ secret ลงใน `openclaw.json` โฟลว์การเข้าสู่ระบบของ CLI จะจัดเก็บโทเคนไว้ในโปรไฟล์การยืนยันตัวตนบนโฮสต์Gateway The CLI login flow stores
    โทเค็นในโปรไฟล์การยืนยันตัวตนบนโฮสต์ของ Gateway
...
- Hugging Face Inference: `huggingface` (`HUGGINGFACE_HUB_TOKEN` หรือ `HF_TOKEN`) — เราเตอร์ที่เข้ากันได้กับ OpenAI; ตัวอย่างโมเดล: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `openclaw onboard --auth-choice huggingface-api-key` ดูที่ [Hugging Face (Inference)](/providers/huggingface)
...
Ollama จะถูกตรวจพบโดยอัตโนมัติเมื่อรันภายในเครื่องที่ `http://127.0.0.1:11434/v1` ดู [/providers/ollama](/providers/ollama) สำหรับคำแนะนำโมเดลและการกำหนดค่าแบบกำหนดเอง Ollama จะถูกตรวจพบโดยอัตโนมัติเมื่อรันภายในเครื่องที่ `http://127.0.0.1:11434/v1` ดู [/providers/ollama](/providers/ollama) สำหรับคำแนะนำโมเดลและการกำหนดค่าแบบกำหนดเอง See [/providers/ollama](/providers/ollama) for model recommendations and custom configuration.

### vLLM

vLLM เป็นเซิร์ฟเวอร์ที่เข้ากันได้กับ OpenAI แบบโลคัล (หรือโฮสต์เอง):

- Provider: `vllm`
- การยืนยันตัวตน (Auth): ไม่บังคับ (ขึ้นอยู่กับเซิร์ฟเวอร์ของคุณ)
- Base URL ค่าเริ่มต้น: `http://127.0.0.1:8000/v1`

หากต้องการเปิดใช้การค้นหาอัตโนมัติแบบโลคัล (ใช้ค่าใดก็ได้หากเซิร์ฟเวอร์ของคุณไม่ได้บังคับ auth):
...
ดูรายละเอียดที่ [/providers/vllm](/providers/vllm)
