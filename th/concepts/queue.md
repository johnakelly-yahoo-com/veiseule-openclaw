...
ส่ง `/queue collect` เป็นคำสั่งเดี่ยว (ต่อเซสชัน) หรือกำหนด `messages.queue.byChannel.discord: "collect"`. แนะนำให้ใช้ `collect`/`steer` หากคุณต้องการ
หนึ่งคำตอบต่อหนึ่งข้อความขาเข้า
28.
ส่ง `/queue collect` เป็นคำสั่งเดี่ยว (ต่อเซสชัน) หรือกำหนด `messages.queue.byChannel.discord: "collect"`
...
Summarize จะเก็บรายการสั้น ๆ แบบหัวข้อย่อยของข้อความที่ถูกทิ้ง และแทรกเป็นพรอมป์ followup แบบสังเคราะห์
