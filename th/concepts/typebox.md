...
ผู้ดูแลเซสชัน ผู้ดูแลเซสชัน เฟรมแรก **ต้อง** เป็นคำขอ `connect` หลังจากนั้นไคลเอนต์สามารถเรียกเมธอด (เช่น `health`, `send`, `chat.send`) และสมัครรับอีเวนต์ (เช่น
`presence`, `tick`, `agent`)
...
| การส่งข้อความ | `send`, `poll`, `agent`, `agent.wait`                     | side-effects ต้องใช้ `idempotencyKey` |
| แชต           | `chat.history`, `chat.send`, `chat.abort`, `chat.inject`  | WebChat ใช้สิ่งเหล่านี้               |
...
- **ฝั่งเซิร์ฟเวอร์**: ทุกเฟรมขาเข้าถูกตรวจสอบด้วย AJV แฮนด์เชครับเฉพาะคำขอ `connect` ที่พารามิเตอร์ตรงกับ `ConnectParams` ไฟล์ดิบที่เผยแพร่มักจะพร้อมใช้งานที่: การจับมือจะยอมรับเฉพาะคำขอ `connect` ที่พารามิเตอร์ตรงกับ `ConnectParams`
...
JSON Schema ที่สร้างแล้วอยู่ในรีโปที่ `dist/protocol.schema.json` โดยไฟล์ดิบที่เผยแพร่มักจะอยู่ที่: ตัวบ่งชี้การพิมพ์จะถูกส่งไปยังช่องแชตขณะมีการรันอยู่ ตัวบ่งชี้การพิมพ์จะถูกส่งไปยังช่องแชตขณะมีการรันอยู่
