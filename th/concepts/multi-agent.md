...
โปรไฟล์การยืนยันตัวตนเป็นแบบ**ต่อเอเจนต์** เอเจนต์แต่ละตัวจะอ่านจากของตัวเอง: โปรไฟล์การยืนยันตัวตนเป็นแบบ**ต่อเอเจนต์** เอเจนต์แต่ละตัวจะอ่านจากของตัวเอง: Each agent reads from its own:
...
2. การจับคู่ `parentPeer` (การสืบทอดเธรด)
3. เป้าหมาย: มีเอเจนต์ที่ _แยกจากกัน_ หลายตัว(เวิร์กสเปซ+`agentDir`+เซสชันแยก)พร้อมบัญชีช่องทางหลายบัญชี(เช่น WhatsApp สองบัญชี)ภายในGatewayที่กำลังรันหนึ่งตัว ขาเข้าจะถูกกำหนดเส้นทางไปยังเอเจนต์ผ่านการผูก(binding) Inbound is routed to an agent via bindings.
4. `guildId`(Discord)
5. `teamId`(Slack)
6. จับคู่`accountId`สำหรับช่องทาง
7. การจับคู่ระดับช่องทาง(`accountId: "*"`)
8. ย้อนกลับไปยังเอเจนต์เริ่มต้น(`agents.list[].default`หรือรายการแรก ค่าเริ่มต้น:`main`)

หาก binding กำหนดหลายฟิลด์สำหรับการจับคู่ (เช่น `peer` + `guildId`) จำเป็นต้องตรงกับทุกฟิลด์ที่ระบุ (ลักษณะการทำงานแบบ `AND`)

## หลายบัญชี/หลายหมายเลขโทรศัพท์

7. ช่องทางที่รองรับ **หลายบัญชี** (เช่น WhatsApp) ใช้ `accountId` เพื่อระบุ
   การล็อกอินแต่ละครั้ง ช่องทางที่รองรับ **หลายบัญชี** (เช่น WhatsApp) ใช้ `accountId` เพื่อระบุ
...
- 8. รายการอนุญาต/ปฏิเสธของเครื่องมือเป็น **tools** ไม่ใช่ skills รายการอนุญาต/ปฏิเสธของเครื่องมือเป็น **tools** ไม่ใช่ skills รายการอนุญาต/ปฏิเสธเครื่องมือเป็น**เครื่องมือ**ไม่ใช่Skills หากSkillต้องรันไบนารี ให้แน่ใจว่าอนุญาต`exec`และมีไบนารีอยู่ในsandbox
...
หมายเหตุ: `setupCommand` อยู่ภายใต้ `sandbox.docker` และรันเพียงครั้งเดียวเมื่อสร้างคอนเทนเนอร์
...
หมายเหตุ: `tools.elevated` เป็น **แบบโกลบอล** และอิงตามผู้ส่ง; ไม่สามารถกำหนดค่าแยกตามเอเจนต์ได้
หากต้องการขอบเขตแยกตามเอเจนต์ ให้ใช้ `agents.list[].tools` เพื่อปฏิเสธ `exec`
