...
แนวป้องกันด้านความปลอดภัยใน system prompt เป็นเชิงแนะนำ แนวป้องกันด้านความปลอดภัยใน system prompt เป็นเชิงแนะนำ OpenClaw สร้าง system prompt แบบกำหนดเองสำหรับการรันเอเจนต์ทุกครั้ง โดย prompt นี้เป็น **ของ OpenClaw** และไม่ใช้ default prompt ของ p-coding-agent
...
แนวทางเหล่านี้ชี้นำพฤติกรรมของโมเดล แต่ไม่ได้บังคับใช้นโยบาย OpenClaw สามารถเรนเดอร์ system prompt ที่เล็กลงสำหรับซับเอเจนต์ได้ แนวทางเหล่านี้ชี้นำพฤติกรรมของโมเดล แต่ไม่ได้บังคับใช้นโยบาย OpenClaw สามารถเรนเดอร์ system prompt ที่เล็กลงสำหรับซับเอเจนต์ได้ การ์ดเรลด้านความปลอดภัยใน system prompt เป็นเชิงแนะนำ ใช้เพื่อชี้นำพฤติกรรมของโมเดลแต่ไม่บังคับใช้นโยบาย การบังคับใช้แบบเข้มควรใช้ tool policy, การอนุมัติการรันคำสั่ง, sandboxing และ channel allowlists; ผู้ดูแลสามารถปิดสิ่งเหล่านี้ได้ตามการออกแบบ
...
- `minimal`: ใช้สำหรับซับเอเจนต์; ตัด **Skills**, **Memory Recall**, **OpenClaw
  ...
  **Messaging**, **Silent Replies** และ **Heartbeats** ออก โดยยังคงมี Tooling, **Safety**,
  Workspace, Sandbox, Current Date & Time (เมื่อทราบ), Runtime และบริบทที่แทรกไว้ ไฟล์ขนาดใหญ่จะถูกตัดทอนพร้อมตัวบ่งชี้ ไฟล์ขนาดใหญ่จะถูกตัดทอนพร้อมตัวบ่งชี้
...
- `MEMORY.md` และ/หรือ `memory.md` (เมื่อมีอยู่ใน workspace; อาจถูกฉีดเข้าไปไฟล์ใดไฟล์หนึ่งหรือทั้งสองไฟล์)

ไฟล์ทั้งหมดเหล่านี้จะถูก **ฉีดเข้าสู่ context window** ในทุกเทิร์น ซึ่งหมายความว่าจะใช้โทเค็น ควรทำให้กระชับ — โดยเฉพาะ `MEMORY.md` ซึ่งอาจขยายใหญ่ขึ้นเมื่อเวลาผ่านไป และทำให้การใช้ context สูงเกินคาด รวมถึงเกิดการบีบอัด (compaction) บ่อยขึ้น

> **หมายเหตุ:** ไฟล์รายวัน `memory/*.md` จะ **ไม่** ถูกฉีดโดยอัตโนมัติ ไฟล์เหล่านี้จะถูกเข้าถึงตามความต้องการผ่านเครื่องมือ `memory_search` และ `memory_get` ดังนั้นจะไม่นับรวมใน context window เว้นแต่โมเดลจะเรียกอ่านอย่างชัดเจน
...
ตัวบ่งชี้ไฟล์หายแบบสั้น ดู [Context](/concepts/context) เนื้อหา bootstrap ที่ถูกฉีดรวมทั้งหมดจากทุกไฟล์ถูกจำกัดด้วย `agents.defaults.bootstrapTotalMaxChars`
(ค่าเริ่มต้น: 24000) หากไฟล์หายไป จะฉีดตัวบ่งชี้ว่าไฟล์หายไปแบบสั้น ๆ

เซสชันของซับเอเจนต์จะฉีดเฉพาะ `AGENTS.md` และ `TOOLS.md` (ไฟล์ bootstrap อื่น ๆ จะถูกกรองออกเพื่อให้ context ของซับเอเจนต์มีขนาดเล็ก)
...
เพื่อดูว่าไฟล์ที่แทรกแต่ละไฟล์มีส่วนต่อบริบทเท่าใด (ดิบ vs ที่แทรก, การตัดทอน และโอเวอร์เฮดของสคีมาเครื่องมือ) ให้ใช้ `/context list` หรือ `/context detail` ดู [Context](/concepts/context) system prompt จะมีส่วน **Current Date & Time** โดยเฉพาะเมื่อทราบเขตเวลาของผู้ใช้ system prompt จะมีส่วน **Current Date & Time** โดยเฉพาะเมื่อทราบเขตเวลาของผู้ใช้

## การจัดการเวลา

prompt สั่งให้โมเดลใช้ `read` เพื่อโหลด SKILL.md จากตำแหน่งที่ระบุ (workspace, managed หรือ bundled) prompt สั่งให้โมเดลใช้ `read` เพื่อโหลด SKILL.md จากตำแหน่งที่ระบุ (workspace, managed หรือ bundled) system prompt จะมีส่วน **Current Date & Time** โดยเฉพาะเมื่อทราบโซนเวลาของผู้ใช้ เพื่อให้แคชของ prompt มีความเสถียร ขณะนี้จะรวมเฉพาะ
...
(ถามผู้ใช้เฉพาะเมื่อไม่สามารถเข้าถึงได้) ส่งคืน **raw provider timestamps** ส่งคืน **raw provider timestamps**
