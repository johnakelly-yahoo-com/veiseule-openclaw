...
แต่ละบรรทัดในไฟล์ล็อกคือออบเจ็กต์ JSON หนึ่งรายการ แต่ละบรรทัดในไฟล์ล็อกเป็นออบเจ็กต์ JSON CLI และ Control UI จะพาร์สรายการเหล่านี้เพื่อแสดงผลลัพธ์แบบมีโครงสร้าง (เวลา ระดับ ระบบย่อย ข้อความ) แต่ละบรรทัดในไฟล์ล็อกเป็นออบเจ็กต์ JSON CLI และ Control UI จะพาร์สรายการเหล่านี้เพื่อแสดงผลลัพธ์แบบมีโครงสร้าง (เวลา ระดับ ระบบย่อย ข้อความ)
...
มาแทนที่ล็อก; มันมีไว้เพื่อป้อนข้อมูลให้กับเมตริก เทรซ และเอ็กซ์พอร์ตเตอร์อื่น ๆ มัน **ไม่ได้**
...
2.
แฟล็กไม่แยกตัวพิมพ์เล็ก‑ใหญ่และรองรับไวลด์การ์ด (เช่น `telegram.*` หรือ `*`).
...
- สามารถเปิด/ปิด traces/metrics ได้ด้วย `traces` / `metrics` (ค่าเริ่มต้น: เปิด) Traces/metrics สามารถเปิดปิดได้ด้วย `traces` / `metrics` (ค่าเริ่มต้น: เปิด) Traces รวมสแปนการใช้โมเดลและสแปนการประมวลผล webhook/ข้อความเมื่อเปิดใช้งาน Traces/metrics สามารถเปิดปิดได้ด้วย `traces` / `metrics` (ค่าเริ่มต้น: เปิด) Traces รวมสแปนการใช้โมเดลและสแปนการประมวลผล webhook/ข้อความเมื่อเปิดใช้งาน
...
- 7. เคารพ `logging.level` (ระดับบันทึกไฟล์) เคารพ `logging.level` (ระดับบันทึกไฟล์) เคารพ `logging.level` (ระดับไฟล์ล็อก) การปกปิดของคอนโซล **ไม่มีผล** กับล็อก OTLP
