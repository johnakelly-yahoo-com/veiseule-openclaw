...
- **ยังเขียนโปรแกรมได้**: แต่ละขั้นสามารถเรียก CLI หรือสคริปต์ใดก็ได้ หากต้องการ JS/TS ให้สร้างไฟล์ `.lobster` จากโค้ด **ยังเขียนโปรแกรมได้**: แต่ละขั้นสามารถเรียก CLI หรือสคริปต์ใดก็ได้ หากต้องการ JS/TS ให้สร้างไฟล์ `.lobster` จากโค้ด If you want JS/TS, generate `.lobster` files from code.
...
สร้างคำสั่งเล็กๆ ที่สื่อสารด้วย JSON แล้วเชื่อมต่อเป็นการเรียก Lobster ครั้งเดียว (ชื่อตัวอย่างด้านล่าง — เปลี่ยนเป็นของคุณได้) (Example command names below — swap in your own.) (Example command names below — swap in your own.)
...
deterministic while still letting you classify/summarize/draft with a model. This keeps the workflow
...
Lobster สามารถรันไฟล์เวิร์กโฟลว์ YAML/JSON ที่มีฟิลด์ `name`, `args`, `steps`, `env`, `condition`, และ `approval` ในการเรียกเครื่องมือของ OpenClaw ให้ตั้งค่า `pipeline` เป็นพาธของไฟล์ In OpenClaw tool calls, set `pipeline` to the file path. In OpenClaw tool calls, set `pipeline` to the file path.
...
ให้ใส่เครื่องมือหรือกลุ่มแกนหลักที่ต้องการลงใน allowlist ด้วย หมายเหตุ: allowlist เป็นแบบ opt-in สำหรับปลั๊กอินเสริม หาก allowlist ของคุณระบุเฉพาะ
เครื่องมือปลั๊กอิน (เช่น `lobster`) OpenClaw จะยังคงเปิดใช้งานเครื่องมือแกนหลักไว้ หากต้องการจำกัดเครื่องมือแกนหลัก
...
OpenProse ทำงานคู่กับ Lobster ได้ดี: ใช้ `/prose` เพื่อจัดการเตรียมงานหลายเอเจนต์ จากนั้นรันไปป์ไลน์ Lobster เพื่อการอนุมัติที่กำหนดผลลัพธ์ได้แน่นอน หากโปรแกรม Prose ต้องใช้ Lobster ให้อนุญาตเครื่องมือ `lobster` สำหรับซับเอเจนต์ผ่าน `tools.subagents.tools` ดู [OpenProse](/prose) If a Prose program needs Lobster, allow the `lobster` tool for sub-agents via `tools.subagents.tools`. If a Prose program needs Lobster, allow the `lobster` tool for sub-agents via `tools.subagents.tools`. See [OpenProse](/prose).
