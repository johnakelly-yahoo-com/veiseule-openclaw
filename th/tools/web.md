...
- `web_fetch` ทำ HTTP GET แบบธรรมดาและสกัดเนื้อหาที่อ่านง่าย
  (HTML → markdown/text) โดย **ไม่** รัน JavaScript It does **not** execute JavaScript. It does **not** execute JavaScript.
...
สำหรับการติดตั้งแบบ gateway ให้ใส่ไว้ใน `~/.openclaw/.env` For a gateway install, put it in `~/.openclaw/.env`. **ทางเลือกผ่านสภาพแวดล้อม:** ตั้งค่า `BRAVE_API_KEY` ในสภาพแวดล้อมของกระบวนการ Gateway
...
โมเดล Perplexity Sonar มีความสามารถค้นหาเว็บในตัวและส่งคืนคำตอบที่สังเคราะห์โดย AI พร้อมการอ้างอิง คุณสามารถใช้ผ่าน OpenRouter (ไม่ต้องใช้บัตรเครดิต รองรับคริปโต/พรีเพด) โมเดล Perplexity Sonar มีความสามารถค้นหาเว็บในตัวและส่งคืนคำตอบที่สังเคราะห์โดย AI พร้อมการอ้างอิง คุณสามารถใช้ผ่าน OpenRouter (ไม่ต้องใช้บัตรเครดิต รองรับคริปโต/พรีเพด) You can use them via OpenRouter (no credit card required - supports
crypto/prepaid).
...
- `country` (ไม่บังคับ): รหัสประเทศ 2 ตัวอักษรสำหรับผลลัพธ์ตามภูมิภาค (เช่น "DE", "US", "ALL") หากไม่ระบุ Brave จะเลือกภูมิภาคเริ่มต้นของตน If omitted, Brave chooses its default region. If omitted, Brave chooses its default region.
...
- `freshness` (ไม่บังคับ เฉพาะ Brave): กรองตามเวลาการค้นพบ (`pd`, `pw`, `pm`, `py`, หรือ `YYYY-MM-DDtoYYYY-MM-DD`)
  - Brave: `pd`, `pw`, `pm`, `py` หรือ `YYYY-MM-DDtoYYYY-MM-DD`
...
- `web_fetch` ใช้ Readability (สกัดเนื้อหาหลัก) ก่อน จากนั้นใช้ Firecrawl (หากตั้งค่าไว้) หากทั้งสองล้มเหลว เครื่องมือจะส่งคืนข้อผิดพลาด If both fail, the tool returns an error. If both fail, the tool returns an error.
