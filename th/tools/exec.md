...
- `host=node`: จะส่งเฉพาะการแทนที่ env ที่ไม่ถูกบล็อกซึ่งคุณระบุไปยังโหนด. การแทนที่ `env.PATH`
  จะถูกปฏิเสธสำหรับการรันบนโฮสต์. หากคุณต้องการเพิ่มรายการ PATH บนโหนด
  ให้กำหนดค่า environment ของบริการโฮสต์ของโหนด (systemd/launchd) หรือติดตั้งเครื่องมือไว้ในตำแหน่งมาตรฐาน
...
`security=allowlist` คำสั่งเชลล์จะถูกอนุญาตอัตโนมัติเฉพาะเมื่อทุกส่วนของ pipeline อยู่ใน allowlist หรือเป็น safe bin. การเชื่อมคำสั่ง (`;`, `&&`, `||`) และการทำ redirection จะถูกปฏิเสธใน
allowlist mode เว้นแต่ทุกเซกเมนต์ระดับบนสุดจะเป็นไปตาม allowlist (รวมถึง safe bins)
ยังไม่รองรับการทำ redirection
...
- `tools.exec.applyPatch.workspaceOnly` มีค่าเริ่มต้นเป็น `true` (จำกัดอยู่ภายใน workspace) ตั้งค่าเป็น `false` เฉพาะเมื่อคุณต้องการให้ `apply_patch` เขียน/ลบไฟล์นอกไดเรกทอรี workspace โดยตั้งใจเท่านั้น
