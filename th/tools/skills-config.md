...
- `allowBundled`: allowlist แบบไม่บังคับสำหรับสกิลแบบ **bundled** เท่านั้น `allowBundled`: รายการอนุญาตแบบไม่บังคับสำหรับSkillsที่ **รวมมา** เท่านั้น เมื่อกำหนดแล้ว จะมีสิทธิ์เฉพาะSkillsที่รวมมาและอยู่ในรายการ (ไม่กระทบSkillsแบบ managed/workspace) `allowBundled`: รายการอนุญาตแบบไม่บังคับสำหรับSkillsที่ **รวมมา** เท่านั้น เมื่อกำหนดแล้ว จะมีสิทธิ์เฉพาะSkillsที่รวมมาและอยู่ในรายการ (ไม่กระทบSkillsแบบ managed/workspace)
...
- คีย์ภายใต้ `entries` จะถูกแมปกับชื่อสกิลโดยค่าเริ่มต้น คีย์ภายใต้ `entries` จะถูกแมปกับชื่อสกิลโดยค่าเริ่มต้น คีย์ภายใต้ `entries` จะแมปกับชื่อSkillโดยค่าเริ่มต้น หากSkillกำหนด
...
จะ **ไม่** สืบทอด `process.env` ของโฮสต์ sandbox **ไม่** สืบทอดค่า `process.env` ของโฮสต์ sandbox **ไม่** สืบทอดค่า `process.env` ของโฮสต์
