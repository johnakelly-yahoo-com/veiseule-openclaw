...
### การขออนุมัติ (`exec.ask`)

- **off**: ไม่ถามเลย
- **on-miss**: ถามเฉพาะเมื่อไม่ตรงกับรายการอนุญาต
- **always**: ถามทุกคำสั่ง

### การขออนุมัติสำรอง (`askFallback`)
...
## รายการที่อนุญาต (ต่อเอเจนต์)

Allowlists are **per agent**. รายการที่อนุญาตเป็นแบบ **ต่อเอเจนต์** หากมีหลายเอเจนต์ ให้สลับไปยังเอเจนต์ที่คุณ
...
`skills.bins` over the Gateway RPC to fetch the skill bin list. This uses
...
positional file args and path-like tokens, so they can only operate on the incoming stream. Safe bins reject
positional file args and path-like tokens, so they can only operate on the incoming stream.
Safe bins จะบังคับให้ตีความโทเค็น argv เป็น **ข้อความล้วน (literal text)** ในขณะรัน (ไม่มี globbing
และไม่มีการขยาย `$VARS`) สำหรับเซกเมนต์ที่รับผ่าน stdin เท่านั้น ดังนั้นรูปแบบอย่าง `*` หรือ `$HOME/...` จะไม่สามารถ
ถูกใช้เพื่อแอบอ่านไฟล์ได้
...
add/remove allowlist patterns, then **Save**. Pick a scope (Defaults or an agent), tweak the policy,
...
หากไม่มีการตัดสินใจก่อนหมดเวลา คำขอจะถูกถือว่า timeout ของการอนุมัติและแสดงเป็นเหตุผลการปฏิเสธ เมื่อจำเป็นต้องมีการอนุมัติ เครื่องมือ exec จะส่งคืนทันทีพร้อม id การอนุมัติ ใช้ id นี้เพื่อเชื่อมโยงกับอีเวนต์ระบบภายหลัง (`Exec finished` / `Exec denied`)
...
กระบวนการนี้ใช้ไปป์ไลน์การส่งออกปกติเหมือนเดิม This uses the normal outbound delivery pipeline. This uses the normal outbound delivery pipeline.
...
- การอนุมัติใช้กับคำขอรันคำสั่งบนโฮสต์จาก **ผู้ส่งที่ได้รับอนุญาต** เท่านั้น ผู้ส่งที่ไม่ได้รับอนุญาตไม่สามารถออก `/exec` ได้ Unauthorized senders cannot issue `/exec`. Unauthorized senders cannot issue `/exec`.
