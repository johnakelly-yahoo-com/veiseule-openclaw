...
Npm specs เป็นแบบ **registry-only** (ชื่อแพ็กเกจ + เวอร์ชัน/แท็กแบบระบุหรือไม่ระบุก็ได้) Git/URL/file
specs จะถูกปฏิเสธ
...
ปลั๊กอินของOpenClawเป็น **โมดูลTypeScript** ที่โหลดขณะรันผ่าน jiti **การตรวจสอบคอนฟิกจะไม่รันโค้ดปลั๊กอิน**; จะใช้ไฟล์แมนิฟेस्टของปลั๊กอินและJSON Schemaแทน ดู [Plugin manifest](/plugins/manifest) **การตรวจสอบความถูกต้องของคอนฟิกจะไม่รันโค้ดของปลั๊กอิน**; แต่จะใช้ plugin manifest และ JSON Schema แทน ดูที่ [Plugin manifest](/plugins/manifest) **การตรวจสอบความถูกต้องของคอนฟิกจะไม่รันโค้ดของปลั๊กอิน**; แต่จะใช้ plugin manifest และ JSON Schema แทน ดูที่ [Plugin manifest](/plugins/manifest)
...
- คืนค่าเป็นบัฟเฟอร์เสียงPCM + อัตราการสุ่มตัวอย่าง ปลั๊กอินต้องรีแซมเปิล/เข้ารหัสให้ผู้ให้บริการ คืนค่าเป็นบัฟเฟอร์เสียงPCM + อัตราการสุ่มตัวอย่าง ปลั๊กอินต้องรีแซมเปิล/เข้ารหัสให้ผู้ให้บริการ ปลั๊กอินต้องทำการ resample/encode สำหรับผู้ให้บริการ
...
รากของปลั๊กอินคือไดเรกทอรีของไฟล์นั้นและต้องมีแมนิฟেস্ট หากพาธชี้ไปที่ไฟล์ รากของปลั๊กอินคือไดเรกทอรีของไฟล์นั้น และต้องมี manifest อยู่ภายใน หากพาธชี้ไปที่ไฟล์ รากของปลั๊กอินคือไดเรกทอรีของไฟล์นั้น และต้องมี manifest อยู่ภายใน
...
แต่ละรายการจะกลายเป็นปลั๊กอินหนึ่งตัว แต่ละรายการจะกลายเป็นปลั๊กอินหนึ่งตัว แต่ละรายการจะกลายเป็นปลั๊กอิน หากแพ็กระบุส่วนขยายหลายรายการ idของปลั๊กอิน
...
หมายเหตุด้านความปลอดภัย: `openclaw plugins install` จะติดตั้ง dependencies ของปลั๊กอินด้วย
`npm install --ignore-scripts` (ไม่มี lifecycle scripts) คงโครงสร้าง dependency ของปลั๊กอินให้เป็น "pure JS/TS" และหลีกเลี่ยงแพ็กเกจที่ต้องใช้การ build ผ่าน `postinstall`
...
วางไฟล์JSONไว้ที่หนึ่งในตำแหน่ง: วางไฟล์ JSON ไว้ที่หนึ่งในตำแหน่งต่อไปนี้: วางไฟล์ JSON ไว้ที่หนึ่งในตำแหน่งต่อไปนี้:
...
`plugins.slots` เพื่อเลือกว่าปลั๊กอินใดเป็นเจ้าของสล็อต: หมวดหมู่ปลั๊กอินบางประเภทเป็นแบบ **เอกสิทธิ์**(เปิดใช้งานได้ครั้งละหนึ่ง) ใช้
...
พร้อมการวินิจฉัย ตัวอื่น ๆ จะถูกปิดใช้งานพร้อมการวินิจฉัย ตัวอื่น ๆ จะถูกปิดใช้งานพร้อมการวินิจฉัย
...
ปลั๊กอินสามารถส่ง hook มาและลงทะเบียนได้ในขณะรันไทม์ ปลั๊กอินสามารถส่ง hook มาและลงทะเบียนได้ในขณะรันไทม์ ปลั๊กอินสามารถบรรจุฮุคและลงทะเบียนขณะรันไทม์ได้ วิธีนี้ทำให้ปลั๊กอินรวมระบบอัตโนมัติแบบขับเคลื่อนด้วยอีเวนต์โดยไม่ต้องติดตั้งแพ็กฮุคแยกต่างหาก
...
ลงทะเบียนผู้ให้บริการผ่าน `api.registerProvider(...)`. ผู้ให้บริการแต่ละรายเปิดเผยวิธีการยืนยันตัวตนหนึ่งอย่างหรือมากกว่า(OAuth, API key, device code ฯลฯ) วิธีเหล่านี้ขับเคลื่อน: เมธอดเหล่านี้ขับเคลื่อน: เมธอดเหล่านี้ขับเคลื่อน:
...
ปลั๊กอินสามารถลงทะเบียนคำสั่งสแลชแบบกำหนดเองที่รัน **โดยไม่เรียกเอเจนต์AI** เหมาะสำหรับคำสั่งสลับสถานะ การตรวจสอบสถานะ หรือการกระทำด่วนที่ไม่ต้องประมวลผลด้วยLLM ปลั๊กอินสามารถลงทะเบียนคำสั่งสแลชแบบกำหนดเองที่รัน **โดยไม่เรียกเอเจนต์AI** เหมาะสำหรับคำสั่งสลับสถานะ การตรวจสอบสถานะ หรือการกระทำด่วนที่ไม่ต้องประมวลผลด้วยLLM สิ่งนี้มีประโยชน์สำหรับคำสั่งสลับ สถานะเช็ก หรือการกระทำด่วนที่ไม่ต้องการการประมวลผลจาก LLM
...
- `acceptsArgs`: คำสั่งรับอาร์กิวเมนต์หรือไม่(ค่าเริ่มต้น: false) หากเป็น false และมีการส่งอาร์กิวเมนต์ คำสั่งจะไม่ตรงและข้อความจะถูกส่งต่อไปยังตัวจัดการอื่น `acceptsArgs`: คำสั่งรับอาร์กิวเมนต์หรือไม่(ค่าเริ่มต้น: false) หากเป็น false และมีการส่งอาร์กิวเมนต์ คำสั่งจะไม่ตรงและข้อความจะถูกส่งต่อไปยังตัวจัดการอื่น หากเป็น false และมีการส่งอาร์กิวเมนต์ คำสั่งจะไม่ตรงและข้อความจะถูกส่งต่อไปยังตัวจัดการอื่น
...
- ชื่อคำสั่งที่สงวนไว้(เช่น `help`, `status`, `reset` เป็นต้น) ไม่สามารถถูกแทนที่โดยปลั๊กอินได้ ไม่สามารถถูก override โดยปลั๊กอินได้ ไม่สามารถถูก override โดยปลั๊กอินได้
