...
- บริการควบคุมเบราว์เซอร์ผูกกับ loopback บนพอร์ตที่ได้มาจาก `gateway.port`
  (ค่าเริ่มต้น: `18791` ซึ่งคือ gateway + 2) ส่วน relay ใช้พอร์ตถัดไป (`18792`) รีเลย์จะใช้พอร์ตถัดไป (`18792`) รีเลย์จะใช้พอร์ตถัดไป (`18792`)
...
การตรวจจับอัตโนมัติ: ตั้งค่า `browser.executablePath` เพื่อแทนที่
การตรวจจับอัตโนมัติ:
...
- **Remote CDP:** ตั้งค่า `browser.profiles.<name>`.cdpUrl`(หรือ`browser.cdpUrl`) เพื่อ
  ...
  เพื่อแนบกับเบราว์เซอร์ที่อิง Chromium ระยะไกล ในกรณีนี้ OpenClaw จะไม่เปิดเบราว์เซอร์ภายในเครื่อง .cdpUrl`(หรือ`browser.cdpUrl\`)
...
โทเคน แทนการบันทึกไว้ในไฟล์คอนฟิก ควรใช้ตัวแปรสภาพแวดล้อมหรือระบบจัดการความลับสำหรับ
...
- หากเปิดใช้งานการควบคุมผ่านเบราว์เซอร์และไม่ได้ตั้งค่า auth, OpenClaw จะสร้าง `gateway.auth.token` อัตโนมัติเมื่อเริ่มต้นและบันทึกลงใน config
...
หากมีการตั้งค่า gateway auth, เส้นทาง HTTP ของเบราว์เซอร์จะต้องใช้ auth ด้วย:
...
`OPENCLAW_HOME_VOLUME` หรือ bind mount ดู [Docker](/install/docker) ดู [Docker](/install/docker) ดู [Docker](/install/docker)
...
- พาธสำหรับดาวน์โหลดและ trace output จะถูกจำกัดให้อยู่ภายใต้ temp root ของ OpenClaw:
  - OpenClaw รองรับหลายโปรไฟล์ที่ตั้งชื่อได้ (routing configs) โปรไฟล์สามารถเป็น: โปรไฟล์สามารถเป็น:
  - downloads: `/tmp/openclaw/downloads` (fallback: `${os.tmpdir()}/openclaw/downloads`)
- พาธสำหรับอัปโหลดจะถูกจำกัดให้อยู่ภายใต้ temp uploads root ของ OpenClaw:
  - โปรไฟล์เริ่มต้นคือ `chrome` (extension relay) โปรไฟล์เริ่มต้นคือ `chrome` (extension relay) ใช้ `defaultProfile: "openclaw"` สำหรับเบราว์เซอร์แบบจัดการ
...
- `browser act kind=evaluate` / `openclaw browser evaluate` และ `wait --fn`
  ...
  สิ่งนี้ได้ ปิดใช้งานด้วย `browser.evaluateEnabled=false` หากคุณไม่ต้องการมัน การฉีดพรอมต์สามารถชี้นำ
