...
- คำสั่ง: `pnpm test`
- การตั้งค่า: `scripts/test-parallel.mjs` (รัน `vitest.unit.config.ts`, `vitest.extensions.config.ts`, `vitest.gateway.config.ts`)
- ไฟล์: `src/**/*.test.ts`
- ขอบเขต:
  - การทดสอบ unit ล้วน
...
- ความคาดหวัง:
  ...
  - ไม่ต้องใช้คีย์จริง
  - ควรเร็วและเสถียร
- หมายเหตุเกี่ยวกับ Pool:
  - OpenClaw ใช้ Vitest `vmForks` บน Node 22/23 เพื่อให้ unit shards ทำงานได้เร็วขึ้น
  - บน Node 24 ขึ้นไป OpenClaw จะสลับกลับไปใช้ `forks` ปกติโดยอัตโนมัติ เพื่อหลีกเลี่ยงข้อผิดพลาดการลิงก์ VM ของ Node (`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`)
  - กำหนดค่าเองได้ด้วย `OPENCLAW_TEST_VM_FORKS=0` (บังคับใช้ `forks`) หรือ `OPENCLAW_TEST_VM_FORKS=1` (บังคับใช้ `vmForks`)
...
- ค่าเริ่มต้นขณะรัน:
  - ใช้ Vitest `vmForks` เพื่อให้การเริ่มต้นไฟล์เร็วขึ้น
  - ใช้จำนวน workers แบบปรับตามสภาพแวดล้อม (CI: 2-4, local: 4-8)
  - รันในโหมดเงียบ (silent mode) เป็นค่าเริ่มต้น เพื่อลด overhead จาก console I/O
- ตัวเลือก override ที่มีประโยชน์:
  - `OPENCLAW_E2E_WORKERS=<n>` เพื่อบังคับจำนวน worker (สูงสุดไม่เกิน 16)
  - `OPENCLAW_E2E_VERBOSE=1` เพื่อเปิดใช้งานการแสดงผล console แบบ verbose อีกครั้ง
- ขอบเขต:
...
รวมอย่างน้อยหนึ่งโมเดลที่รองรับรูปภาพใน `OPENCLAW_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI รุ่นที่รองรับ vision ฯลฯ) เพื่อทดสอบ image probe เพื่อทดสอบ image probe เพื่อทดสอบ image probe
...
เคล็ดลับ: อย่าพยายามฮาร์ดโค้ด “โมเดลทั้งหมด” ไว้ในเอกสาร เคล็ดลับ: อย่าพยายามฮาร์ดโค้ด “ทุกโมเดล” ในเอกสาร รายการที่เป็นทางการคือสิ่งที่ `discoverModels(...)` ส่งกลับบนเครื่องของคุณ + คีย์ที่มีอยู่ เคล็ดลับ: อย่าพยายามฮาร์ดโค้ด “ทุกโมเดล” ในเอกสาร รายการที่เป็นทางการคือสิ่งที่ `discoverModels(...)` ส่งกลับบนเครื่องของคุณ + คีย์ที่มีอยู่

## ข้อมูลรับรอง (ห้ามคอมมิต)

การทดสอบ live ค้นหาข้อมูลรับรองแบบเดียวกับ CLI นัยเชิงปฏิบัติ: ผลกระทบเชิงปฏิบัติ: ผลกระทบเชิงปฏิบัติ:
