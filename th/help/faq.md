...
Windows (PowerShell) เวอร์ชันเทียบเท่า:
...
`channels.telegram.allowFrom` คือ **Telegram user ID ของผู้ส่งที่เป็นมนุษย์** (เป็นตัวเลข) It is not the bot username.

ตัวช่วยตั้งค่าเริ่มต้น (onboarding wizard) รับค่าเป็น `@username` และแปลงเป็น ID แบบตัวเลขให้ แต่การกำหนดสิทธิ์ของ OpenClaw ใช้เฉพาะ ID แบบตัวเลขเท่านั้น
