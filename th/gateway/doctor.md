...
28.
การย้ายสถานะเดิม (legacy) จะรันอัตโนมัติเมื่อถูกตรวจพบ
...
- การตรวจจับไดเรกทอรีเวิร์กสเปซเพิ่มเติม (`~/openclaw`).
...
32. การย้ายเหล่านี้เป็นแบบ best-effort และ idempotent; doctor จะส่งคำเตือนเมื่อยังมีโฟลเดอร์ legacy เหลือไว้เป็นแบ็กอัป การย้ายเหล่านี้เป็นแบบ best-effort และ idempotent; doctor จะส่งคำเตือนเมื่อ
    ...
    ตั้งใจให้ย้ายผ่าน `openclaw doctor` เท่านั้น การยืนยันตัวตน WhatsApp ถูกออกแบบให้ย้ายผ่าน `openclaw doctor` เท่านั้น
...
36.
การรีเฟรชพรอมป์จะปรากฏเฉพาะเมื่อรันแบบโต้ตอบ (TTY); `--non-interactive` จะข้ามความพยายามรีเฟรช
...
38.
บริการ OpenClaw gateway ที่ตั้งชื่อตามโปรไฟล์ถือเป็น first-class และจะไม่ถูกทำเครื่องหมายว่าเป็น "extra"
...
Doctor จะเตือนเมื่อไม่มี `gateway.auth` บน gateway ภายในเครื่อง และเสนอที่จะสร้างโทเค็น Doctor เตือนเมื่อไม่มี `gateway.auth` บน gateway ภายในเครื่อง และเสนอให้
สร้างโทเคน ใช้ `openclaw doctor --generate-gateway-token` เพื่อบังคับการสร้างโทเคนในงานอัตโนมัติ Doctor เตือนเมื่อไม่มี `gateway.auth` บน gateway ภายในเครื่อง และเสนอให้
...
41. Doctor ตรวจสอบรันไทม์ของบริการ (PID, สถานะการออกล่าสุด) และเตือนเมื่อบริการถูกติดตั้งแต่ไม่ได้กำลังทำงาน Doctor ตรวจสอบรันไทม์ของบริการ (PID, สถานะการออกล่าสุด) และเตือนเมื่อบริการถูกติดตั้งแต่ไม่ได้กำลังทำงาน Doctor ตรวจรันไทม์ของบริการ (PID, สถานะการออกล่าสุด) และเตือนเมื่อ
