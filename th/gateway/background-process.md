...
OpenClaw รันคำสั่งเชลล์ผ่านเครื่องมือ `exec` และเก็บงานที่รันเป็นเวลานานไว้ในหน่วยความจำ เครื่องมือ `process` ใช้จัดการเซสชันเบื้องหลังเหล่านั้น The `process` tool manages those background sessions. The `process` tool manages those background sessions.
...
- `tools.exec.notifyOnExitEmptySuccess` (ค่าเริ่มต้น false): เมื่อเป็น true จะจัดคิวเหตุการณ์เสร็จสิ้นสำหรับการรันแบบ background ที่สำเร็จแต่ไม่มีเอาต์พุตด้วย
...
- เมื่อไม่ระบุทั้ง `offset` และ `limit` ระบบจะส่งคืน 200 บรรทัดล่าสุดและรวมคำแนะนำการแบ่งหน้า
- เมื่อระบุ `offset` แต่ไม่ระบุ `limit` ระบบจะส่งคืนข้อมูลตั้งแต่ `offset` จนถึงจุดสิ้นสุด (ไม่จำกัดไว้ที่ 200 บรรทัด)
