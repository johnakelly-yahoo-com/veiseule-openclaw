...
- [โปรโตคอล Gateway](/gateway/protocol)
- [โปรโตคอล Bridge (legacy)](/gateway/bridge-protocol)
...
Bonjour เป็นแบบ best-effort และไม่ข้ามเครือข่าย Bonjourเป็นแบบbest-effortและไม่ข้ามเครือข่าย ใช้เพื่อความสะดวกในกรณี “LANเดียวกัน” เท่านั้น Bonjourเป็นแบบbest-effortและไม่ข้ามเครือข่าย ใช้เพื่อความสะดวกในกรณี “LANเดียวกัน” เท่านั้น
...
- ระเบียน TXT ของ Bonjour/mDNS **ไม่มีการยืนยันตัวตน** ไคลเอนต์ต้องถือว่าค่า TXT เป็นเพียงคำแนะนำด้าน UX เท่านั้น
- การกำหนดเส้นทาง (host/port) ควรให้ความสำคัญกับ **resolved service endpoint** (SRV + A/AAAA) มากกว่า `lanHost`, `tailnetDns` หรือ `gatewayPort` ที่มาจาก TXT
- TLS pinning ต้องไม่อนุญาตให้ `gatewayTlsSha256` ที่โฆษณาไว้ แทนที่ค่าที่ถูก pin เก็บไว้ก่อนหน้า
- โหนด iOS/Android ควรถือว่าการเชื่อมต่อโดยตรงที่ได้จาก discovery เป็นแบบ **TLS เท่านั้น** และต้องให้ผู้ใช้ยืนยัน “เชื่อถือ fingerprint นี้” อย่างชัดเจนก่อนบันทึก pin ครั้งแรก (การยืนยันนอกช่องทาง)
