...
title: "API การเรียกใช้เครื่องมือ"
---
...
- หากมีการกำหนดค่า `gateway.auth.rateLimit` และเกิดความล้มเหลวในการยืนยันตัวตนมากเกินไป endpoint จะส่งคืน `429` พร้อม `Retry-After`
...
- `sessionKey` (สตริง, ไม่บังคับ): คีย์เซสชันเป้าหมาย `sessionKey` (สตริง, ไม่บังคับ): คีย์เซสชันเป้าหมาย `sessionKey` (string, ไม่บังคับ): คีย์เซสชันเป้าหมาย หากไม่ระบุหรือเป็น `"main"` Gateway จะใช้คีย์เซสชันหลักที่ตั้งค่าไว้ (คำนึงถึง `session.mainKey` และเอเจนต์เริ่มต้น หรือ `global` ในสโคปส่วนกลาง)
...
Gateway HTTP ยังใช้รายการปฏิเสธแบบฮาร์ดโค้ดเป็นค่าเริ่มต้น (แม้ว่า session policy จะอนุญาตเครื่องมือนั้นก็ตาม):
...
```json5
{
  gateway: {
    tools: {
      // เครื่องมือเพิ่มเติมที่ต้องการบล็อกผ่าน HTTP /tools/invoke
      deny: ["browser"],
      // นำเครื่องมือออกจากรายการปฏิเสธเริ่มต้น
      allow: ["gateway"],
    },
  },
}
