...
สำหรับการเข้าถึงแบบ LAN/tailnet ให้ผูกแบบชัดเจนและคงการยืนยันตัวตนไว้ พอร์ต WS ของGateway (ค่าเริ่มต้น `18789`) จะผูกกับ loopback โดยค่าเริ่มต้น
...
- ระเบียน TXT ของ Bonjour/mDNS เป็นแบบ **ไม่ยืนยันตัวตน (unauthenticated)** ไคลเอนต์ต้องไม่ถือว่า TXT เป็นเส้นทางที่เชื่อถือได้อย่างเป็นทางการ
- ไคลเอนต์ควรกำหนดเส้นทางโดยใช้ service endpoint ที่ถูก resolve แล้ว (SRV + A/AAAA) ให้ถือว่า `lanHost`, `tailnetDns`, `gatewayPort` และ `gatewayTlsSha256` เป็นเพียงคำแนะนำเท่านั้น
- TLS pinning ต้องไม่อนุญาตให้ `gatewayTlsSha256` ที่โฆษณาไว้แทนที่ pin ที่บันทึกไว้ก่อนหน้า
- โหนด iOS/Android ควรถือว่าการเชื่อมต่อโดยตรงผ่านการค้นพบเป็นแบบ **TLS-only** และต้องให้ผู้ใช้ยืนยันอย่างชัดเจนก่อนเชื่อถือ fingerprint ครั้งแรก
