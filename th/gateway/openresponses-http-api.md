...
เอ็นด์พอยต์นี้ **ปิดใช้งานเป็นค่าเริ่มต้น** ต้องเปิดใช้งานในคอนฟิกก่อน เปิดใช้งานใน config ก่อน เปิดใช้งานใน config ก่อน
...
ใช้คอนฟิกการยืนยันตัวตนของ Gateway ส่ง bearer token: ส่ง bearer token: ส่ง bearer token:
...
คำขอเป็นไปตาม OpenResponses API พร้อมอินพุตแบบอิงไอเท็ม การรองรับปัจจุบัน: การรองรับปัจจุบัน: การรองรับปัจจุบัน:
...
- ไฟล์ PDF จะถูกแยกข้อความ ไฟล์ PDF จะถูกแยกข้อความ PDF จะถูกแยกข้อความ หากพบข้อความน้อย หน้าต้น ๆ จะถูกแรสเตอร์เป็นภาพ
  แล้วส่งให้โมเดล

การแยก PDF ใช้บิลด์ legacy ของ `pdfjs-dist` ที่เป็นมิตรกับ Node (ไม่มี worker) การแยก PDF ใช้บิลด์ legacy ของ `pdfjs-dist` ที่เป็นมิตรกับ Node (ไม่มี worker) การแยก PDF ใช้บิลด์ legacy ของ `pdfjs-dist` ที่เป็นมิตรกับ Node (ไม่ใช้ worker) บิลด์สมัยใหม่ของ
...
- `maxUrlParts`: `8` (จำนวนรวมของส่วน `input_file` + `input_image` ที่อ้างอิงด้วย URL ต่อหนึ่งคำขอ)
- คำขอถูกป้องกัน (การแก้ DNS, การบล็อก IP ส่วนตัว, จำกัดการรีไดเร็กต์, ไทม์เอาต์)
- รองรับ allowlist ของ hostname แบบกำหนดเองตามประเภทอินพุต (`files.urlAllowlist`, `images.urlAllowlist`).
  - โฮสต์แบบระบุชัดเจน: `"cdn.example.com"`
  - ซับโดเมนแบบ wildcard: `"*.assets.example.com"` (ไม่ครอบคลุม apex domain)
...
- URL allowlist จะถูกบังคับใช้ก่อนการ fetch และในทุกขั้นตอนของ redirect.
- การเพิ่ม hostname ลงใน allowlist ไม่ได้เป็นการข้ามการบล็อก IP ภายใน/ส่วนตัว.
- สำหรับ gateway ที่เปิดให้เข้าถึงจากอินเทอร์เน็ต ควรใช้มาตรการควบคุม network egress เพิ่มเติมนอกเหนือจากการป้องกันในระดับแอป.
  ดู [Security](/gateway/security).
