...
- **หมดอายุจากความไม่ใช้งาน** (`session.reset.idleMinutes` หรือแบบเดิม `session.idleMinutes`) จะสร้าง `sessionId` ใหม่เมื่อมีข้อความเข้ามาหลังพ้นช่วงไม่ใช้งาน เมื่อกำหนดทั้งรายวัน+ไม่ใช้งาน กรณีที่หมดอายุก่อนจะถูกใช้ ประเภทเอนทรีที่น่าสนใจ: ประเภทเอนทรีที่น่าสนใจ:
...
Compaction เป็นแบบ **ถาวร** (ไม่เหมือนกับ session pruning) ธรรมเนียมปฏิบัติ: การคอมแพคชันเป็นแบบ **คงอยู่ถาวร** (ต่างจากการตัดแต่งเซสชัน) ดู [/concepts/session-pruning](/concepts/session-pruning).
