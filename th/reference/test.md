...
- `pnpm test:coverage`: รัน Vitest พร้อมการครอบคลุมด้วย V8 22. 22. เกณฑ์ส่วนกลางคือ 70% สำหรับ lines/branches/functions/statements `pnpm test:coverage`: รัน Vitest พร้อมความครอบคลุม V8 เกณฑ์รวมคือ70%สำหรับบรรทัด/แขนง/ฟังก์ชัน/สเตตเมนต์ การคำนวณความครอบคลุมจะยกเว้นจุดเริ่มต้นที่เน้นอินทิเกรชันหนัก(การเชื่อมสายCLI,บริดจ์gateway/telegram,เซิร์ฟเวอร์สแตติกเว็บแชต)เพื่อโฟกัสที่ลอจิกที่ทดสอบด้วยยูนิตได้

- `pnpm test` บน Node 24+: OpenClaw จะปิดใช้งาน Vitest `vmForks` อัตโนมัติ และใช้ `forks` เพื่อหลีกเลี่ยงข้อผิดพลาด `ERR_VM_MODULE_LINK_FAILURE` / `module is already linked` คุณสามารถบังคับพฤติกรรมได้ด้วย `OPENCLAW_TEST_VM_FORKS=0|1`

- `pnpm test:e2e`: รันการทดสอบสโมคแบบเอนด์ทูเอนด์ของGateway(การจับคู่WS/HTTP/โหนดหลายอินสแตนซ์) ค่าเริ่มต้นคือ `vmForks` + workers แบบปรับอัตโนมัติใน `vitest.e2e.config.ts`; ปรับแต่งได้ด้วย `OPENCLAW_E2E_WORKERS=<n>` และตั้งค่า `OPENCLAW_E2E_VERBOSE=1` เพื่อดูบันทึกแบบละเอียด

- `pnpm test:live`: รันการทดสอบผู้ให้บริการแบบ live (minimax/zai) `pnpm test:live`: รันการทดสอบไลฟ์ของผู้ให้บริการ(minimax/zai)ต้องใช้คีย์APIและ`LIVE=1`(หรือ`*_LIVE_TEST=1`เฉพาะผู้ให้บริการ)เพื่อยกเลิกการข้าม `pnpm test:live`: รันการทดสอบไลฟ์ของผู้ให้บริการ(minimax/zai)ต้องใช้คีย์APIและ`LIVE=1`(หรือ`*_LIVE_TEST=1`เฉพาะผู้ให้บริการ)เพื่อยกเลิกการข้าม
