...
- Query-string tokens จะถูกปฏิเสธ (`?token=...` จะส่งกลับ `400`)
...
- `agentId` ไม่บังคับ (string): ส่ง hook นี้ไปยัง agent ที่ระบุ หากเป็น ID ที่ไม่รู้จัก จะย้อนกลับไปใช้ agent เริ่มต้น เมื่อกำหนดค่าแล้ว hook จะทำงานโดยใช้ workspace และการกำหนดค่าของ agent ที่ resolve ได้
- `sessionKey` ไม่บังคับ (string): คีย์ที่ใช้ระบุเซสชันของเอเจนต์ ค่าเริ่มต้นเป็น `hook:<uuid>` แบบสุ่ม การใช้คีย์ที่สม่ำเสมอช่วยให้สนทนาได้หลายเทิร์นภายในบริบทของ hook โดยค่าเริ่มต้น ฟิลด์นี้จะถูกปฏิเสธ เว้นแต่จะตั้งค่า `hooks.allowRequestSessionKey=true`
...
- `model` ไม่บังคับ (string): การแทนที่โมเดล (เช่น `anthropic/claude-3-5-sonnet` หรือชื่อแฝง) ต้องอยู่ในรายการโมเดลที่อนุญาตหากมีการจำกัด Must be in the allowed model list if restricted. Must be in the allowed model list if restricted.
...
## นโยบาย session key (มีการเปลี่ยนแปลงที่ไม่เข้ากันย้อนหลัง)

ค่า override ของ `sessionKey` ใน payload `/hooks/agent` ถูกปิดใช้งานโดยค่าเริ่มต้น

- คำแนะนำ: ตั้งค่า `hooks.defaultSessionKey` แบบคงที่ และปิดการ override จาก request
- ทางเลือก: อนุญาตให้แทนที่คำขอ (request overrides) เฉพาะเมื่อจำเป็น และจำกัดคำนำหน้า (prefixes)
...
การตั้งค่าความเข้ากันได้ (พฤติกรรมแบบเดิม):
...
- `hooks.transformsDir` + `transform.module` โหลดโมดูล JS/TS สำหรับลอจิกแบบกำหนดเอง
  - `hooks.transformsDir` (หากตั้งค่า) ต้องอยู่ภายใน transforms root ภายใต้ไดเรกทอรีการตั้งค่า OpenClaw ของคุณ (โดยทั่วไปคือ `~/.openclaw/hooks/transforms`)
  - `transform.module` ต้องถูก resolve ภายในไดเรกทอรี transforms ที่มีผลใช้งาน (เส้นทาง traversal/escape จะถูกปฏิเสธ)
...
- `agentId` ใช้กำหนดเส้นทาง hook ไปยังเอเจนต์ที่ระบุ; หากไม่พบ ID จะย้อนกลับไปใช้เอเจนต์เริ่มต้น
- `hooks.allowedAgentIds` ใช้จำกัดการกำหนดเส้นทาง `agentId` แบบระบุชัดเจน ละเว้นค่านี้ (หรือใส่ `*`) เพื่ออนุญาตทุกเอเจนต์ ตั้งค่า `[]` เพื่อปฏิเสธการกำหนดเส้นทาง `agentId` แบบระบุชัดเจน
- `sessionKey` ไม่บังคับ (string): คีย์ที่ใช้ระบุเซสชันของเอเจนต์ ค่าเริ่มต้นเป็น `hook:<uuid>` แบบสุ่ม การใช้คีย์ที่สม่ำเสมอช่วยให้สนทนาได้หลายเทิร์นภายในบริบทของ hook 5.
- `hooks.allowRequestSessionKey` ควบคุมว่า payload ของ `/hooks/agent` สามารถตั้งค่า `sessionKey` ได้หรือไม่ (ค่าเริ่มต้น: `false`)
- `hooks.allowedSessionKeyPrefixes` ใช้จำกัดค่า `sessionKey` แบบระบุชัดเจนจาก request payload และ mappings (ถ้ากำหนด)
...
- `openclaw webhooks gmail setup` เขียนคอนฟิก `hooks.gmail` สำหรับ `openclaw webhooks gmail run`
  ...
  14.
  ดู [Gmail Pub/Sub](/automation/gmail-pubsub) สำหรับขั้นตอนการ watch ของ Gmail แบบครบถ้วน
...
- `429` หลังจากเกิดความล้มเหลวในการยืนยันตัวตนซ้ำ ๆ จากไคลเอนต์เดียวกัน (ตรวจสอบ `Retry-After`)
...
- ความล้มเหลวในการยืนยันตัวตนที่เกิดซ้ำจะถูกจำกัดอัตราต่อที่อยู่ไคลเอนต์ เพื่อชะลอความพยายาม brute-force
- หากคุณใช้การกำหนดเส้นทางหลายเอเจนต์ ให้ตั้งค่า `hooks.allowedAgentIds` เพื่อจำกัดการเลือก `agentId` แบบระบุชัดเจน
- คงค่า `hooks.allowRequestSessionKey=false` เว้นแต่คุณจำเป็นต้องให้ผู้เรียกเลือก session เอง
- หากคุณเปิดใช้งาน `sessionKey` จาก request ให้จำกัด `hooks.allowedSessionKeyPrefixes` (เช่น `["hook:"]`)
