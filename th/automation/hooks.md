...
Hooks คือสคริปต์ขนาดเล็กที่ทำงานเมื่อมีบางสิ่งเกิดขึ้น โดยมีอยู่สองประเภท: There are two kinds: There are two kinds:
...
หากคุณเขียนฟังก์ชัน TypeScript เล็กๆได้ คุณก็สามารถเขียน hook ได้ Hooks จะถูกค้นพบโดยอัตโนมัติ และคุณสามารถเปิดหรือปิดการใช้งานผ่าน CLI หากคุณเขียนฟังก์ชัน TypeScript เล็กๆได้ คุณก็สามารถเขียน hook ได้ Hooks จะถูกค้นพบโดยอัตโนมัติ และคุณสามารถเปิดหรือปิดการใช้งานผ่าน CLI Hooks are discovered automatically, and you enable or disable them via the CLI.
...
`package.json` ติดตั้งด้วยคำสั่ง: Install them with: Install them with:

```bash
openclaw hooks install <path-or-spec>
```

Npm specs เป็นแบบ registry-only (ชื่อแพ็กเกจ + เวอร์ชัน/แท็กที่ไม่บังคับ) Git/URL/file specs จะถูกปฏิเสธ
...
หมายเหตุด้านความปลอดภัย: `openclaw hooks install` จะติดตั้ง dependencies ด้วย `npm install --ignore-scripts`
(ไม่มีการรัน lifecycle scripts) รักษา dependency tree ของ hook pack ให้เป็น "pure JS/TS" และหลีกเลี่ยงแพ็กเกจที่ต้องพึ่งพา
การ build ในขั้นตอน `postinstall`
...
หมายเหตุ: `module` ต้องเป็นพาธแบบสัมพันธ์กับ workspace พาธแบบสัมบูรณ์และการอ้างอิงออกนอก workspace จะถูกปฏิเสธ
...
- พาธจะถูก resolve โดยอ้างอิงจาก workspace
- ไฟล์ต้องอยู่ภายใน workspace เท่านั้น (ตรวจสอบด้วย realpath)
- จะโหลดเฉพาะ basename ของ bootstrap ที่รู้จักเท่านั้น
- คงรายการ allowlist ของ Subagent ไว้ (`AGENTS.md` และ `TOOLS.md` เท่านั้น)
...
### จาก Legacy Config สู่ Discovery
