...
หน้านี้อธิบายพฤติกรรม CLI ปัจจุบัน หน้านี้อธิบายพฤติกรรมCLIปัจจุบัน หากคำสั่งมีการเปลี่ยนแปลง ให้ปรับปรุงเอกสารนี้ หน้านี้อธิบายพฤติกรรมCLIปัจจุบัน หากคำสั่งมีการเปลี่ยนแปลง ให้ปรับปรุงเอกสารนี้
...
การเปลี่ยนแปลงปลั๊กอินส่วนใหญ่ต้องรีสตาร์ทGateway ดู [/plugin](/tools/plugin) ดู [/plugin](/tools/plugin). ดู [/plugin](/tools/plugin).
...
ข้อความแชตรองรับคำสั่ง `/...` (ข้อความและเนทีฟ) ดู [/tools/slash-commands](/tools/slash-commands) ข้อความแชตรองรับคำสั่ง `/...` (ข้อความและเนทีฟ) ดู [/tools/slash-commands](/tools/slash-commands) ดู [/tools/slash-commands](/tools/slash-commands).
...
- `--custom-base-url <url>` (โหมดไม่โต้ตอบ; ใช้ร่วมกับ `--auth-choice custom-api-key`)
- `--custom-model-id <id>` (โหมดไม่โต้ตอบ; ใช้ร่วมกับ `--auth-choice custom-api-key`)
- `--custom-api-key <key>` (โหมดไม่โต้ตอบ; ไม่บังคับ; ใช้ร่วมกับ `--auth-choice custom-api-key`; หากไม่ระบุจะใช้ค่า `CUSTOM_API_KEY` แทน)
- `--custom-provider-id <id>` (โหมดไม่โต้ตอบ; ไม่บังคับ; รหัสผู้ให้บริการแบบกำหนดเอง)
- `--custom-compatibility <openai|anthropic>` (โหมดไม่โต้ตอบ; ไม่บังคับ; ค่าเริ่มต้นคือ `openai`)
- `--gateway-port <port>`
...
ตัวช่วยDNSสำหรับการค้นหาในวงกว้าง (CoreDNS+Tailscale) ดู [/gateway/discovery](/gateway/discovery) ดู [/gateway/discovery](/gateway/discovery). ดู [/gateway/discovery](/gateway/discovery).
...
เพิ่มเอเจนต์แบบแยกใหม่ เพิ่มเอเจนต์แบบแยกใหม่ เพิ่มเอเจนต์แบบแยกใหม่ จะรันวิซาร์ดแบบแนะนำเว้นแต่ส่งแฟล็ก (หรือ `--non-interactive`); โหมดไม่โต้ตอบต้องใช้ `--workspace`
...
- `gateway status` ยังแสดงบริการGatewayเดิมหรือเพิ่มเติมเมื่อสามารถตรวจพบได้ (`--deep` เพิ่มการสแกนระดับระบบ) บริการOpenClawที่ตั้งชื่อตามโปรไฟล์ถือเป็นชั้นหนึ่งและไม่ถูกทำเครื่องหมายว่า “เพิ่มเติม” `gateway status` ยังแสดงบริการGatewayเดิมหรือเพิ่มเติมเมื่อสามารถตรวจพบได้ (`--deep` เพิ่มการสแกนระดับระบบ) บริการOpenClawที่ตั้งชื่อตามโปรไฟล์ถือเป็นชั้นหนึ่งและไม่ถูกทำเครื่องหมายว่า “เพิ่มเติม” บริการ OpenClaw ที่ตั้งชื่อตามโปรไฟล์จะถูกปฏิบัติเป็นระดับแรกและไม่ถูกทำเครื่องหมายว่าเป็น "extra".
...
`nodes` สื่อสารกับGatewayและกำหนดเป้าหมายโหนดที่จับคู่ ดู [/nodes](/nodes) See [/nodes](/nodes). See [/nodes](/nodes).
...
CLIควบคุมเบราว์เซอร์ (Chrome/Brave/Edge/Chromiumแบบเฉพาะ) ดู [`openclaw browser`](/cli/browser) และ [เครื่องมือBrowser](/tools/browser) CLIควบคุมเบราว์เซอร์ (Chrome/Brave/Edge/Chromiumแบบเฉพาะ) ดู [`openclaw browser`](/cli/browser) และ [เครื่องมือBrowser](/tools/browser) ดู [`openclaw browser`](/cli/browser) และ [เครื่องมือ Browser](/tools/browser).
