...
ใช้ `--agent <id>` เพื่อตรวจสอบสถานะโมเดล/การยืนยันตัวตนของเอเจนต์ที่ตั้งค่าไว้ ใช้ `--agent <id>` เพื่อตรวจสอบสถานะโมเดล/การยืนยันตัวตนของเอเจนต์ที่ตั้งค่าไว้ เมื่อเว้นว่างไว้,
...
- การอ้างอิงโมเดลจะถูกพาร์สโดยแยกที่ `/` **ครั้งแรก** ใช้ `provider/model` เมื่อพิมพ์ `/model <ref>` การอ้างอิงโมเดลจะถูกพาร์สโดยแยกที่ `/` **ครั้งแรก** ใช้ `provider/model` เมื่อพิมพ์ `/model <ref>` การอ้างอิงโมเดลจะถูกแยกโดยแบ่งที่ `/` ตัวแรก หากรหัสโมเดลมี `/` (สไตล์ OpenRouter) ให้ใส่คำนำหน้าผู้ให้บริการ (ตัวอย่าง: `openrouter/moonshotai/kimi-k2`)
...
`openclaw plugins list` เพื่อดูว่ามีผู้ให้บริการใดติดตั้งอยู่บ้าง `models auth login` รันโฟลว์การยืนยันตัวตนของปลั๊กอินผู้ให้บริการ (OAuth/คีย์API) ใช้
