...
- エージェントは固有のナメクジです（例：`main`、`research`）。 ゲートウェイはリストを公開します。 ゲートウェイはリストを公開します。
...
その他の Gateway（ゲートウェイ）のスラッシュコマンド（例：`/context`）は Gateway（ゲートウェイ）に転送され、システム出力として表示されます。詳細は [Slash commands](/tools/slash-commands) を参照してください。 [Slash commands](/tools/slash-commands)を参照してください。 [Slash commands](/tools/slash-commands)を参照してください。
