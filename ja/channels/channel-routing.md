...
モデルがチャンネルを選択することはありません。ルーティングは決定論的で、ホストの設定によって制御されます。 OpenClaw は、**メッセージが届いた元のチャンネルへ返信**します。  
...
2. **親ピア一致**（スレッド継承）。
3. WebChat は **選択されたエージェント** に接続し、既定ではエージェントのメインセッションを使用します。  
...
6. **チャンネル一致**（そのチャンネル上の任意のアカウント）。
7. **アカウント一致**（チャンネル上の `accountId`）。
8. **既定のエージェント**（`agents.list[].default`、それ以外はリストの先頭、最終的に `main` へフォールバック）。

バインディングに複数の一致フィールド（`peer`、`guildId`、`teamId`、`roles`）が含まれる場合、**指定されたすべてのフィールドが一致**しなければ、そのバインディングは適用されません。
