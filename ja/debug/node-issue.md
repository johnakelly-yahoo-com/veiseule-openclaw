...
開発者スクリプトを Bun から `tsx` に切り替えた後に始まりました(`2871657e`, 2026-01-06) 。 同じランタイムパスが Bun で動作しました。 同じランタイムパスが Bun で動作しました。
...
- `tsx` は esbuild を使用して TS/ESM を変換します。 `tsx` は esbuild を使用して TS/ESM を変換します。 `tsx` は TS/ESM を変換するために esbuild を使用します。esbuild の `keepNames` は `__name` ヘルパーを出力し、関数定義を `__name(...)` でラップします。
