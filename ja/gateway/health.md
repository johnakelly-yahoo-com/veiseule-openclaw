...
- セッションストア：`ls -l ~/.openclaw/agents/<agentId>/sessions/sessions.json`（パスは設定で上書き可能）。件数と最近の受信者は `status` で表示されます。 カウントと最近の受信者は `status` で表示されます。 カウントと最近の受信者は `status` で表示されます。
