...
- `funnel`: `tailscale funnel` による公開 HTTPS。OpenClaw では共有パスワードが必要です。 `funnel`: `tailscale funnel` による公開 HTTPS。OpenClaw では共有パスワードが必要です。 OpenClawは共有パスワードが必要です。
...
で `gateway.auth.mode: "password"` を強制します。 OpenClaw は、ローカルの Tailscale
デーモン(`tailscale whois`)を介して `x-forwarded-for` アドレスを解決し、それを受け入れる前にヘッダにマッチさせることで、
身元を検証します。
OpenClawは、ループバックから
Tailscaleの`x-forwarded-for`、`x-forward-proto`、および`x-forwarded-host`
...
- サーブ/ファンネルは**ゲートウェイコントロール UI + WS** のみを公開します。 サーブ/ファンネルは**ゲートウェイコントロール UI + WS** のみを公開します。 Serve/Funnel が公開するのは **Gateway コントロール UI + WS** のみです。
