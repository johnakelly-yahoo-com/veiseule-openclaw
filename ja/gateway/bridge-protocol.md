...
は、代わりに統合されたGateway WebSocketプロトコルを使用する必要があります。 新しいノードクライアント
...
`bridgeTlsSha256` が含まれ、ノードは証明書をピン留めできます。 Bonjour/mDNS の TXT レコードは
認証されていない点に注意してください。クライアントは、明示的なユーザーの意思または他のアウトオブバンド検証なしに、広告されたフィンガープリントを
信頼できるピンとして扱ってはなりません。
