...
Gateway は `_openclaw-gw._tcp` を `local.` でアドバタイズします。iOS アプリはこれらを自動的に一覧表示します。 iOSアプリはこれらを自動的に一覧表示します。 iOSアプリはこれらを自動的に一覧表示します。
...
- これは Gateway HTTP サーバー（`gateway.port` と同じポート、デフォルトは `18789`）から配信されます。
