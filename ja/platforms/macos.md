...
macOS アプリはノードとして振る舞います。一般的なコマンドは次のとおりです。 一般的なコマンド: 一般的なコマンド:
...
- **IP レポート:** SSH トンネルはループバックを使用するため、Gateway から見えるノード IP は `127.0.0.1` になります。実際のクライアント IP を表示したい場合は、**Direct (ws/wss)** トランスポートを使用してください（[macOS remote access](/platforms/mac/remote) を参照）。 実際のクライアント
  IP を表示させたい場合は、**Direct (ws/wss)** を使用してください ([macOS リモート アクセス](/platforms/mac/remote)を参照してください)。 実際のクライアント
...
の詳細については、[Gateway protocol](/gateway/protocol)を参照してください。 プロトコル
