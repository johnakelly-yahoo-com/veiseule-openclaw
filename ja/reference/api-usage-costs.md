...
返信が実行される前に、インバウンドメディアを要約/転写することができます。 これは model/provider API を使用します。 これは model/provider API を使用します。
...
- `memorySearch.provider = "openai"` → OpenAIの埋め込み
- `memorySearch.provider = "gemini"` → Geminiの埋め込み
- `memorySearch.provider = "voyage"` → Voyageの埋め込み
