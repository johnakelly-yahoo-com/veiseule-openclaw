...
<Steps>
  ...
  <Step title="Install OpenClaw under prefix">
    `--prefix <prefix>` を使用して npm でインストールし、その後ラッパーを `<prefix>/bin/openclaw` に書き込みます。
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Why does npm hit EACCES on Linux?">
    一部の Linux 環境では、npm のグローバル プレフィックスが root 所有のパスを指しています。`install.sh` はプレフィックスを `~/.npm-global` に切り替え、（それらのファイルが存在する場合）シェルの rc ファイルに PATH の export を追記できます。
  </Accordion>
  ...
  <Accordion title="Windows: how to get verbose installer output">
    ...
    スクリプト レベルの診断には PowerShell のトレースを使用してください。
