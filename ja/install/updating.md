...
OpenClaw は高速に進化しています（1.0 未満）。更新は本番インフラの出荷のように扱ってください：更新 → チェックを実行 → 再起動（または `openclaw update` を使用して再起動）→ 確認。

## 推奨：Web サイトのインストーラーを再実行（インプレースアップグレード）

**推奨**される更新方法は、Web サイトのインストーラーを再実行することです。既存のインストールを検出し、その場でアップグレードし、必要に応じて `openclaw doctor` を実行します。
...
- **ソースインストール** の場合は、次を使用します。
  ...
  リポジトリがクリーンな場合に **のみ** `git pull --rebase` が実行されます。
...
- Gateway の実行方法を確認してください：**フォアグラウンドのターミナル** か **監視サービス**（launchd/systemd）か。
...
3. Gateway を再起動し、最後にアクティブだったセッションへレポートを送信します。
...
注記：**ソースインストール**（git checkout）の場合、`openclaw doctor` は最初に `openclaw update` を実行することを提案します。
