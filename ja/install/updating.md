...
注記：npm インストールでは、Gateway は起動時に更新ヒントをログに出力します（現在のチャンネルタグを確認）。`update.checkOnStart: false` で無効化できます。 `update.checkOnStart: false` で無効にします。 `update.checkOnStart: false` で無効にします。
...
**npm/pnpm** でインストールしている場合（git メタデータなし）、`openclaw update` はパッケージマネージャー経由での更新を試みます。インストールを検出できない場合は、「更新（グローバルインストール）」を使用してください。 インストールが検出できない場合は、代わりに "Update (global install)" を使用してください。 インストールが検出できない場合は、代わりに "Update (global install)" を使用してください。

## 更新（Control UI / RPC）

Control UI には **Update & Restart**（RPC：`update.run`）があります。これは次を行います： それ: それ:
...
Doctor は「安全な更新」コマンドです。意図的に地味で、修復＋移行＋警告を行います。 それは意図的に退屈です:修理+移行+警告。 それは意図的に退屈です:修理+移行+警告。
