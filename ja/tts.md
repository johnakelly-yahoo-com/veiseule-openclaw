...
いいえ. 自動TTSはデフォルトで**オフ**です。 自動TTSはデフォルトで**オフ**です。 いいえ。自動 TTS はデフォルトで **オフ** です。設定で `messages.tts.auto` を使用するか、
...
- `maxTextLength`: TTS 入力のハード上限（文字数）。超過すると `/tts audio` が失敗します。 `/tts audio` を超えると失敗します。 `/tts audio` を超えると失敗します。
...
- **Edge TTS**: `edge.outputFormat` を使用（デフォルト `audio-24khz-48kbitrate-mono-mp3`）。
  - `node-edge-tts` は `outputFormat` を受け付けますが、すべてのフォーマットが Edge サービスから利用できるわけではありません。citeturn2search0 <unk> cite<unk> turn2search0<unk> <unk> cite<unk> turn2search0<unk>
  - 出力フォーマットの値は Microsoft Speech の出力フォーマットに従います（Ogg/WebM Opus を含む）。citeturn1search0 <unk> cite<unk> turn1search0<unk> <unk> cite<unk> turn1search0<unk>
  - Telegram の `sendVoice` は OGG/MP3/M4A を受け付けます。保証された Opus ボイスノートが必要な場合は OpenAI/ElevenLabs を使用してください。citeturn1search1 <unk> cite<unk> turn1search1<unk> <unk> cite<unk> turn1search1<unk>
...
そこで `/voice` をネイティブ コマンドとして登録します。テキスト `/tts ...` も引き続き使用できます。 テキスト`/tts ...`はまだ動作します。 テキスト`/tts ...`はまだ動作します。
