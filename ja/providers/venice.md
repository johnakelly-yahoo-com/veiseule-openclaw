...
Venice AI は、検閲なしモデルのサポートと、匿名化プロキシを通じた主要なプロプライエタリモデルへのアクセスを提供する、プライバシー重視の AI 推論を実現します。すべての推論はデフォルトでプライベートです。データを用いた学習は行われず、ログも保存されません。 すべての推論はデフォルトでプライベートです。データに関するトレーニングやロギングはありません。 すべての推論はデフォルトでプライベートです。データに関するトレーニングやロギングはありません。
...
| モード       | 説明                                                                                                 | モデル                                      |
| --------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **非公開**   | 完全にプライベート。 プロンプト/応答は**保存されたりログに記録されることはありません**。 エフェメラル。                                            | Llama、Qwen、DeepSeek、Venice Uncensored など |
| **匿名化済み** | メタデータを剥奪してヴェネツィアを介してプロキシされました。 基礎となるプロバイダー (OpenAI、Anthropic) は匿名化されたリクエストを見ます。 | Claude、GPT、Gemini、Grok、Kimi、MiniMax      |
...
セットアップ後、OpenClaw は利用可能なすべての Venice モデルを表示します。用途に応じて選択してください。 あなたのニーズに基づいて選択: あなたのニーズに基づいて選択:
...
`VENICE_API_KEY` が設定されている場合、OpenClaw は Venice API からモデルを自動的に検出します。API に到達できない場合は、静的カタログにフォールバックします。 API に到達できない場合は、静的カタログに戻ります。 API に到達できない場合は、静的カタログに戻ります。
...
Venice はクレジットベースのシステムを使用しています。最新の料金は [venice.ai/pricing](https://venice.ai/pricing) を確認してください。 Venice はクレジットベースのシステムを使用しています。最新の料金は [venice.ai/pricing](https://venice.ai/pricing) を確認してください。 現在の料金については[venice.ai/pricing](https://venice.ai/pricing)をチェックしてください:
...
Venice API は `https://api.venice.ai/api/v1` にあります。ネットワークが HTTPS 接続を許可していることを確認してください。 ネットワークで HTTPS 接続が許可されていることを確認します。 ネットワークで HTTPS 接続が許可されていることを確認します。
