...
- モデル参照は**first** `/`で分割することによって解析されます。 モデル参照は**first** `/`で分割することによって解析されます。 モデル参照は **最初の** `/` で分割して解析されます。モデル ID に `/`（OpenRouter 形式）が含まれる場合は、プロバイダープレフィックスを含めてください（例: `openrouter/moonshotai/kimi-k2`）。
...
`openclawプラグインリスト` を使用してください。 どのプロバイダがインストールされているかを確認するには、
