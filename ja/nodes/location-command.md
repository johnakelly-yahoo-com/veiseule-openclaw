...
- iOS/macOS：ユーザーはシステムのプロンプトや設定で **While Using** または **Always** を選択できます。アプリは昇格を要求できますが、OS により設定画面が必要になる場合があります。 アプリはアップグレードをリクエストできますが、OSは設定が必要な場合があります。 アプリはアップグレードをリクエストできますが、OSは設定が必要な場合があります。
...
任意。 任意。 任意です。macOS ノードは権限マップ経由で `location` を報告します。iOS/Android では省略される場合があります。
