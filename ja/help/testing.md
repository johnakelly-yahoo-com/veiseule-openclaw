...
- 期待値：
  ...
  - 高速かつ安定していること
- プールに関する注意：
  - OpenClaw は、より高速なユニットシャードのために Node 22/23 で Vitest の `vmForks` を使用します。
  - Node 24 以降では、Node VM のリンクエラー（`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`）を回避するため、OpenClaw は自動的に通常の `forks` にフォールバックします。
  - `OPENCLAW_TEST_VM_FORKS=0`（`forks` を強制）または `OPENCLAW_TEST_VM_FORKS=1`（`vmForks` を強制）で手動上書きできます。
...
- ランタイムのデフォルト：
  - Vitestの`vmForks`を使用して、ファイル起動を高速化します。
  - 適応型ワーカーを使用します（CI: 2〜4、ローカル: 4〜8）。
  - コンソールI/Oのオーバーヘッドを削減するため、デフォルトでサイレントモードで実行されます。
- 便利な上書き設定：
  - ワーカー数を強制指定するには `OPENCLAW_E2E_WORKERS=<n>` を使用します（上限16）。
  - 詳細なコンソール出力を再有効化するには `OPENCLAW_E2E_VERBOSE=1` を使用します。
- スコープ：
...
image プローブを実行するため、`OPENCLAW_LIVE_GATEWAY_MODELS` には少なくとも 1 つの image 対応モデル（Claude／Gemini／OpenAI の vision 対応バリアントなど）を含めてください。 画像プローブを行使することです 画像プローブを行使することです
...
ヒント：「すべてのモデル」をドキュメントでハードコードしようとしないでください。 ヒント：ドキュメントに「すべてのモデル」をハードコードしないでください。権威あるリストは、あなたのマシンで `discoverModels(...)` が返す内容と、利用可能なキーです。 ヒント：ドキュメントに「すべてのモデル」をハードコードしないでください。権威あるリストは、あなたのマシンで `discoverModels(...)` が返す内容と、利用可能なキーです。
