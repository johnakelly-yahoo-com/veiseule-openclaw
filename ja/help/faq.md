...
`channels.telegram.allowFrom` は **人間の送信者の Telegram ユーザー ID**（数値）です。 Botのユーザー名ではありません。

オンボーディングウィザードでは `@username` の入力を受け付けて数値 ID に解決しますが、OpenClaw の認可では数値 ID のみを使用します。
