...
- チャンネルごとの上書き（`channels.whatsapp.*`、`channels.telegram.*` など）：上限やストリーミング トグル。 キャップやストリーミングの切り替えを行います キャップやストリーミングの切り替えを行います

完全なスキーマは [設定](/gateway/configuration) を参照してください。

## インバインドされた<unk>

チャンネルは再接続後に同じメッセージを再配信できます。 チャンネルは再接続後に同じメッセージを再配信できます。 チャンネルは、再接続後に同一メッセージを再配信する場合があります。OpenClaw は
...
ゲートウェイでバックエンドされたセッショントランスクリプトを表示するため、それらは真実の源となります。 推奨：異なるコンテキストを避けるために、長時間の
会話に主要なデバイスを1つ使用してください。 Control UI と TUI は常に、
...
メッセージが一貫しています。 これにより、エージェントプロンプトでリアルタイムおよびキュー/ヒストリー
メッセージが一貫しています。
