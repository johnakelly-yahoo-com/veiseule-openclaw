...
CLIは多くの場合、短いワンオフコマンドを接続します。 CLIは多くの場合、短いワンオフコマンドを接続します。 CLI は短時間で単発のコマンド実行のために接続されることがよくあります。Instances リストへのスパムを避けるため、`client.mode === "cli"` は presence エントリーに**変換されません**。
...
IP を上書きしないように、ループバックのリモートアドレスは無視されます。 良好なクライアント報告済みの
