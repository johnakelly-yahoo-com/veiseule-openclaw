...
認証プロファイルは **エージェント単位** です。各エージェントは、次から読み取ります。 各エージェントは以下のように読み込みます。 各エージェントは以下のように読み込みます。
...
Skills は、各ワークスペースの `skills/` フォルダーを通じて **エージェント単位** で提供され、共有 Skills は `~/.openclaw/skills` から利用できます。詳細は [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) を参照してください。 [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills)を参照してください。 [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills)を参照してください。
...
[Sandboxing](/gateway/sandboxing) を参照してください。 ワークスペース内では相対パスが解決されますが、サンドボックス化が有効になっていない限り、絶対パスは
他のホストロケーションに到達することができます。
...
**1 つの WhatsApp アカウント** のまま、**異なる WhatsApp DM** を異なるエージェントへルーティングできます。送信者の E.164（例：`+15551234567`）に一致させて `peer.kind: "dm"` を行います。返信は同じ WhatsApp 番号から送信されます（エージェントごとの送信者 ID はありません）。 `peer.kind: "direct"` を使用して、送信者の E.164（例: `+15551234567`）にマッチさせます。 返信は依然として同じWhatsApp番号(エージェントごとの送信者IDなし)から来ます。 `peer.kind: "direct"` を使用して、送信者の E.164（例: `+15551234567`）にマッチさせます。 返信は依然として同じWhatsApp番号(エージェントごとの送信者IDなし)から来ます。
...
2. `parentPeer` の一致（スレッド継承）
3. 目的：1 つの稼働中の Gateway で、複数の _分離された_ エージェント（個別のワークスペース + `agentDir` + セッション）と、複数のチャンネル アカウント（例：2 つの WhatsApp）を同時に扱います。インバウンドは、バインディングによってエージェントへルーティングされます。 インバウンドはバインディングを介してエージェントにルーティングされます。
4. `guildId`（Discord）
5. `teamId`（Slack）
6. チャンネルに対する `accountId` の一致
7. チャンネル レベルの一致（`accountId: "*"`）
8. デフォルト エージェントへフォールバック（`agents.list[].default`、それ以外は最初のエントリ、デフォルト：`main`）

バインディングで複数のマッチフィールド（例：`peer` + `guildId`）を設定した場合、指定されたすべてのフィールドが必要です（`AND` セマンティクス）。

## 複数アカウント／電話番号

**複数アカウント** をサポートするチャンネル（例：WhatsApp）では、各ログインを識別するために `accountId` を使用します。各 `accountId` は異なるエージェントへルーティングできるため、1 台のサーバーで複数の電話番号を、セッションを混在させずにホストできます。 **複数アカウント** をサポートするチャンネル（例：WhatsApp）では、各ログインを識別するために `accountId` を使用します。各 `accountId` は異なるエージェントへルーティングできるため、1 台のサーバーで複数の電話番号を、セッションを混在させずにホストできます。 それぞれの`accountId`は異なるエージェントにルーティングすることができるので、1つのサーバーはセッションを混合せずに
複数の電話番号をホストできます。
