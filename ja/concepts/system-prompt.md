...
OpenClaw は、すべてのエージェント実行ごとにカスタムのシステムプロンプトを構築します。このプロンプトは **OpenClaw 管理**であり、p-coding-agent のデフォルトプロンプトは使用しません。 OpenClaw は、すべてのエージェント実行ごとにカスタムのシステムプロンプトを構築します。このプロンプトは **OpenClaw 管理**であり、p-coding-agent のデフォルトプロンプトは使用しません。 プロンプトは **OpenClaw-owned** で、p-coding-agent のデフォルトプロンプトは使用しません。
...
- `minimal`: サブエージェント用。**Skills**、**Memory Recall**、**OpenClaw
  ...
  **Messaging**、**Silent Replies**、**Heartbeats** を省略します。Tooling、**Safety**、
  ...
  context available Tooling, **Safety** ,
  Workspace, Sandbox, Current Date & Time (When known), Runtime, and injected
  context available
- `none`: ベースの識別行のみを返します。
...
- `MEMORY.md` および／または `memory.md`（ワークスペースに存在する場合。どちらか一方または両方が挿入される可能性があります）

これらのファイルはすべて、毎ターン**コンテキストウィンドウに挿入**されるため、トークンを消費します。 特に `MEMORY.md` は時間とともに増大し、予期せぬコンテキスト使用量の増加やより頻繁なコンパクションの原因となる可能性があるため、簡潔に保ってください。

> **注:** `memory/*.md` の日次ファイルは自動的には挿入されません。 これらは `memory_search` および `memory_get` ツールを介してオンデマンドでアクセスされるため、モデルが明示的に読み取らない限りコンテキストウィンドウにはカウントされません。

大きなファイルはマーカーで切り捨てられます。 大きなファイルはマーカーで切り捨てられます。 大きなファイルはマーカー付きで切り詰められます。ファイルごとの最大サイズは
`agents.defaults.bootstrapMaxChars`（デフォルト: 20000）で制御されます。欠落しているファイルは、短い欠落ファイルマーカーを注入します。 不足しているファイルは、
短い不足しているファイルマーカーを挿入します。 ファイル全体で挿入されるブートストラップコンテンツの合計は `agents.defaults.bootstrapTotalMaxChars`（デフォルト: 24000）で上限が設定されています。 ファイルが存在しない場合は、短い欠落ファイルマーカーが挿入されます。

サブエージェント セッションでは `AGENTS.md` と `TOOLS.md` のみが挿入されます（他のブートストラップファイルは、サブエージェントのコンテキストを小さく保つため除外されます）。

内部フックは `agent:bootstrap` を介してこのステップを横取りし、注入されるブートストラップファイルを変更または置換できます（例: `SOUL.md` を別のペルソナに差し替える）。

各注入ファイルがどれだけ寄与しているか（生データ vs 注入後、切り詰め、ツールスキーマのオーバーヘッドを含む）を確認するには、`/context list` または `/context detail` を使用してください。[Context](/concepts/context) を参照してください。 [Context](/concepts/context) を参照してください。 [Context](/concepts/context) を参照してください。
...
のみを含みます (動的なクロックやタイムフォーマットは含まれません)。 プロンプトキャッシュの安定性を維持するために、今は **タイムゾーン**
