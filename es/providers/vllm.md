...
OpenClaw también puede **descubrir automáticamente** los modelos disponibles de vLLM cuando optas por usar `VLLM_API_KEY` (cualquier valor funciona si tu servidor no exige autenticación) y no defines una entrada explícita `models.providers.vllm`.
...
2. Opta por usarlo (cualquier valor funciona si no hay autenticación configurada):
...
- Tu servidor requiere una API key real (o quieres controlar los headers).
...
- Comprueba que el servidor es accesible:
