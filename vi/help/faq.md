...
```powershell
# install.ps1 hiện chưa có cờ -Verbose chuyên dụng.
Set-PSDebug -Trace 1
...
`channels.telegram.allowFrom` là **ID người gửi Telegram (người thật)** (dạng số). It is not the bot username.

Trình hướng dẫn khởi tạo chấp nhận đầu vào `@username` và chuyển đổi sang ID số, nhưng cơ chế phân quyền của OpenClaw chỉ sử dụng ID số.
...
- Nếu bạn muốn tích hợp gốc, hãy mở một yêu cầu tính năng hoặc xây dựng một skill nhắm tới các API đó.
