...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). OpenClaw cũng áp dụng giới hạn tổng số ký tự bootstrap được chèn trên tất cả các tệp với `agents.defaults.bootstrapTotalMaxChars` (mặc định `24000` ký tự). 1. `/context` hiển thị kích thước **raw vs injected** và liệu có xảy ra cắt bớt hay không.
