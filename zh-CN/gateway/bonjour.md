...
基于 Tailnet 的连接。 37. 这是尽力而为的机制，**不能**替代 SSH 或
基于 Tailnet 的连接。
...
（"广域 Bonjour"）来保持相同的设备发现用户体验。 40. 你可以通过切换到 **单播 DNS‑SD**（“广域 Bonjour”）并结合 Tailscale 来保持相同的发现 UX。
...
访问，请明确绑定并保持认证启用。 12. 网关 WS 端口（默认 `18789`）默认绑定到回环地址。
...
- Bonjour/mDNS TXT 记录是**未认证的**。 客户端不得将 TXT 视为权威路由信息。
- 客户端应使用已解析的服务端点（SRV + A/AAAA）进行路由。 将 `lanHost`、`tailnetDns`、`gatewayPort` 和 `gatewayTlsSha256` 仅视为提示信息。
- TLS pinning 绝不能允许已公布的 `gatewayTlsSha256` 覆盖之前存储的 pin。
...
`gateway log file: ...`）。查找 `bonjour:` 行，特别是： 42. 网关会写入一个滚动日志文件（在启动时打印为
`gateway log file: ...`）。
...
- **浏览有效但解析失败**：保持机器名称简单（避免表情符号或
  ...
  主机名，因此过于复杂的名称可能会混淆某些解析器。 The service instance name derives from
