...
关于模型选择规则，请参阅 [/concepts/models](/concepts/models)。1) 有关模型选择规则，请参见 [/concepts/models](/concepts/models)。
...
7. OpenClaw 随附 pi‑ai 目录。 OpenClaw 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  ...
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `openclaw.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。 48. CLI 登录流程会将
    令牌存储在网关主机上的鉴权配置文件中。
...
- Hugging Face Inference：`huggingface`（`HUGGINGFACE_HUB_TOKEN` 或 `HF_TOKEN`）— OpenAI 兼容路由；示例模型：`huggingface/deepseek-ai/DeepSeek-R1`；CLI：`openclaw onboard --auth-choice huggingface-api-key`。 参见 [Hugging Face (Inference)](/providers/huggingface)。
...
详见 [/providers/vllm](/providers/vllm)。
