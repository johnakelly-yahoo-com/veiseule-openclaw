...
目标：多个_隔离的_智能体（独立的工作区 + `agentDir` + 会话），加上多个渠道账户（例如两个 WhatsApp）在一个运行的 Gateway 网关中。入站消息通过绑定路由到智能体。 Inbound is routed to an agent via bindings.
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.openclaw/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。 参见 [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills)。
...
3. 支持**多账户**的渠道（如 WhatsApp）使用 `accountId` 来识别每个登录。每个 `accountId` 可以路由到不同的智能体，因此一个服务器可以托管多个电话号码而不混合会话。
4. `guildId`（Discord）
5. `teamId`（Slack）
6. 渠道的 `accountId` 匹配
7. 渠道级匹配（`accountId: "*"`）
...
- 工具允许/拒绝列表是**工具**，不是 Skills。如果 skill 需要运行二进制文件，请确保 `exec` 被允许且二进制文件存在于沙箱中。 28. 如果某个技能需要运行二进制文件，请确保已允许 `exec`，并且该二进制在沙箱中存在。
