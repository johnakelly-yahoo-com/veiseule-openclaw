---
title: "reference/AGENTS.default.md"
...
OpenClaw 为智能体使用专用的工作区目录。默认：`~/.openclaw/workspace`（可通过 `agents.defaults.workspace` 配置）。 默认值：`~/.openclaw/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- `SOUL.md` 定义身份、语气和边界。保持其更新。 保持其为最新状态。
- 如果你更改了 `SOUL.md`，告知用户。
...
- **discord** — Discord 操作：回应、贴纸、投票。使用 `user:<id>` 或 `channel:<id>` 目标（纯数字 id 有歧义）。 Use `user:<id>` or `channel:<id>` targets (bare numeric ids are ambiguous).
