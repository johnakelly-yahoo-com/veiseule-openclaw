...
**节点**是一个配套设备（macOS/iOS/Android/无头），它以 `role: "node"` 连接到 Gateway 网关 **WebSocket**（与操作员相同的端口），并通过 `node.invoke` 暴露命令接口（例如 `canvas.*`、`camera.*`、`system.*`）。协议详情：[Gateway 网关协议](/gateway/protocol)。 7. 协议详情：[Gateway protocol](/gateway/protocol)。
...
为 `role: node` 创建设备配对请求。通过设备 CLI（或 UI）批准。 17. 通过 devices CLI（或 UI）进行批准。
...
将 `exec` 调用转发到**节点主机**。 25. 模型仍然与 **网关** 通信；当选择 `host=node` 时，网关会将 `exec` 调用转发给 **节点主机**。
...
节点主机指向隧道的本地端。 35. 创建一个 SSH 隧道，并让节点主机指向隧道的本地端。
...
Exec 批准是**每个节点主机**的。从 Gateway 网关添加允许列表条目： Add allowlist entries from the gateway:
...
节点暴露 `screen.record`（mp4）。示例： Example:
...
- Node 主机忽略 `PATH` 覆盖。 如果需要额外的 PATH 条目，请配置 node host 服务的环境（或将工具安装在标准位置），而不是通过 `--env` 传递 `PATH`。
...
上或在服务器旁运行最小节点时很有用。 This is useful on Linux/Windows
...
- 在 macOS 上，当配套应用 exec 主机可达时，无头节点主机优先使用它，
  ...
  使用应用，或设置 `OPENCLAW_NODE_EXEC_FALLBACK=0` 禁用回退。 Set `OPENCLAW_NODE_EXEC_HOST=app` to require
