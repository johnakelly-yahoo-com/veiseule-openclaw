...
- iOS/macOS：用户可以在系统提示/设置中选择**使用时**或**始终**。应用可以请求升级，但操作系统可能要求进入设置。 19. 应用可以请求升级权限，但操作系统可能要求前往设置。
...
32. 可选。 可选。macOS 节点通过权限映射报告 `location`；iOS/Android 可能省略它。
...
- iOS：需要始终权限 + 后台位置模式。静默推送可能被限流；预期会有间歇性失败。 7. 静默推送可能会被限流；预计会出现间歇性失败。
...
- 精确："使用精确 GPS 位置。关闭以共享大致位置。" 19. 关闭开关以共享近似位置。”
