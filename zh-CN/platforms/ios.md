...
Gateway 网关在 `local.` 上广播 `_openclaw-gw._tcp`。iOS 应用会自动列出这些。 iOS 应用会自动列出这些。
...
iOS 节点渲染一个 WKWebView canvas。使用 `node.invoke` 来驱动它： Use `node.invoke` to drive it:
...
- 它由 Gateway HTTP 服务器提供（与 `gateway.port` 相同端口，默认 `18789`）。
