...
**提示：** 如果实例创建失败并显示"Out of capacity"，尝试不同的可用性域或稍后重试。免费层容量有限。 Free tier capacity is limited.
...
Use token auth as the default. 使用令牌认证作为默认值。它是可预测的，避免需要任何"不安全认证"的控制 UI 标志。
...
Now that everything is working, lock down the VCN to block all traffic except Tailscale. 现在一切正常工作了，锁定 VCN 以阻止除 Tailscale 之外的所有流量。OCI 的虚拟云网络充当网络边缘的防火墙——流量在到达你的实例之前就被阻止。
...
这会在网络边缘阻止端口 22 上的 SSH、HTTP、HTTPS 和其他所有内容。从现在开始，你只能通过 Tailscale 连接。 From now on, you can only connect via Tailscale.
...
不需要 SSH 隧道。Tailscale 提供： Tailscale provides:
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的_需求_——但你仍应保持操作系统更新，运行 `openclaw security audit`，并验证你没有意外地在公共接口上监听。
...
| IPv6 加固    | Usually not | 取决于你的 VCN/子网设置；验证实际分配/暴露的内容       |
...
某些工具可能没有 ARM 构建。检查： journalctl --user -u openclaw-gateway -n 50

```bash
uname -m  # 应该显示 aarch64
```

大多数 npm 包工作正常。对于二进制文件，寻找 `linux-arm64` 或 `aarch64` 版本。 tailscale serve status
